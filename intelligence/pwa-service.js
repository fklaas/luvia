...
const RELEASE_BUILD=String(globalThis.LuviaKernelVersion?.build||'13.82.60').trim();
...
async function clearOldCaches(){if(!('caches'in window))return[];const keys=await caches.keys(),removed=[];for(const key of keys){if(key.startsWith('luvia-shell-v')&&key!==EXPECTED_CACHE){await caches.delete(key);removed.push(key)}}return removed}
async function register(){if(!('serviceWorker'in navigator))return snapshot();try{await removeWrongRegistrations();registration=await navigator.serviceWorker.register(SW_URL,{scope:SW_SCOPE,updateViaCache:'none'});bindRegistration(registration);await registration.update();if(registration.waiting)activateWaiting(registration);await navigator.serviceWorker.ready;await clearOldCaches();lastError=null;emit();return snapshot()}catch(e){lastError=e.message||String(e);emit();throw e}}
