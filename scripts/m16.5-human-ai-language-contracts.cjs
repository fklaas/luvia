...
function validateLanguageContracts(document){
  assert.equal(document.contractId,'luvia.human-ai-language-contracts.v1');assert.equal(document.version,'1.0.0');assert.equal(document.actions.length,327);
  assert.equal(document.summary.semanticActions,327);assert.equal(document.summary.canonicalGermanActions,327);assert.ok(document.summary.curatedActionIds>=100);assert.ok(document.summary.curatedRules>=120);
  assert.equal(new Set(document.actions.map(item=>item.actionId)).size,327);assert.ok(document.actions.every(item=>item.canonical.de.length===1));
