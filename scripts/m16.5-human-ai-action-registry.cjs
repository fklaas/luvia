...
function materializeNavigationAIParity() {
  ...
  for (const action of registry.actions.filter(item => item.ai?.runtimeRegistered === true)) {
    ...
    const inputContract = inputContracts.contracts[action.ai.actionId];
    if (inputContract) action.inputContract = {
      status: 'READY',
      schemaId: inputContract.schemaId,
      requiredFields: inputContract.required,
      optionalFields: inputContract.optional,
      contextFields: inputContract.context,
    };
  }
...
const SOURCE_MARKER_DECISIONS = Object.freeze({
  ...
  'data-ftc-path-canvas': ['Reisen & Zusammenarbeit', 'STATUS/EINGABE/PROJEKTION'],
  ...
  'data-ftc-route-focus': ['Places & Ortsentdeckung', 'INTERAKTIONSPRIMITIVE'],
  'data-ftc-route-stop': ['Places & Ortsentdeckung', 'STATUS/EINGABE/PROJEKTION'],
  'data-ftc-free-route': ['Reisen & Zusammenarbeit', 'STATUS/EINGABE/PROJEKTION'],
  ...
  'data-ftc-without-suggestions': ['Reisen & Zusammenarbeit', 'INTERAKTIONSPRIMITIVE'],
  'data-ftc-inspiration-pick': ['Reisen & Zusammenarbeit', 'INTERAKTIONSPRIMITIVE'],
  'data-ftc-inspiration-search': ['Reisen & Zusammenarbeit', 'INTERAKTIONSPRIMITIVE'],
  ...
  'data-collapsed': ['Reisen & Zusammenarbeit', 'STATUS/EINGABE/PROJEKTION'],
  'data-ftc-timezone': ['Reisen & Zusammenarbeit', 'STATUS/EINGABE/PROJEKTION'],
  ...
  'data-places-fit-mode': ['Places & Ortsentdeckung', 'AKTIONSKANDIDAT'],
  ...
  'data-places-filter-section': ['Places & Ortsentdeckung', 'AKTIONSKANDIDAT'],
  'data-places-map-browser': ['Places & Ortsentdeckung', 'STATUS/EINGABE/PROJEKTION'],
  'data-places-map-current': ['Places & Ortsentdeckung', 'STATUS/EINGABE/PROJEKTION'],
  'data-places-map-navigate': ['Places & Ortsentdeckung', 'AKTIONSKANDIDAT'],
  'data-places-map-panel': ['Places & Ortsentdeckung', 'STATUS/EINGABE/PROJEKTION'],
  'data-places-map-panel-close': ['Navigation & Oberfläche', 'AKTIONSKANDIDAT'],
  'data-places-map-tool': ['Places & Ortsentdeckung', 'AKTIONSKANDIDAT'],
  'data-places-map-total': ['Places & Ortsentdeckung', 'STATUS/EINGABE/PROJEKTION'],
  'data-places-preview-open': ['Places & Ortsentdeckung', 'AKTIONSKANDIDAT'],
  'data-places-price': ['Places & Ortsentdeckung', 'AKTIONSKANDIDAT'],
  'data-places-subtype': ['Places & Ortsentdeckung', 'AKTIONSKANDIDAT'],
  'data-places-subtype-group': ['Places & Ortsentdeckung', 'STATUS/EINGABE/PROJEKTION'],
});
...
function buildReport(validated = validateRegistry()) {
  ...
  return `# M16.5 Human ↔ AI Action Parity Control Plane\n\n` +
    `Date: 2026-09-03\n\n` +
    `Status: **B0.01–B0.10 PUBLIC COMPLETE ON 13.82.135 / B1 FOUNDATION PUBLIC ON 13.82.136 + TRIP READ ON 13.82.138 + PLACES MUTATIONS ON 13.82.139 + BOOKING LIFECYCLE/PLACE-ROUTE INTEGRITY ON 13.82.143 / RECOVERY BASELINE ON 13.82.147 / 13.82.148–149 REJECTED / PUBLIC MAP/MINI-INFO CHECKPOINT ON 13.82.168, USER ACCEPTANCE OPEN / PRODUCT PARITY CONTINUES THROUGH B1–B5**\n\n` +
    ...
    `5. Preserve the publicly closed B0.10 control-plane release and continue product parity row by row through B1–B5. App \`13.82.135\` proved one exact Plan/Undo path, accepted App \`13.82.138\` proved the Chat-native Trip-selection read, accepted App \`13.82.139\` publicly proved the complete Favorite/Unfavorite/Plan/Unplan confirmation, readback, receipt and Undo protocol, and accepted App \`13.82.143\` proved the Booking lifecycle plus exact Place-route gate. App \`13.82.147\` retains the accepted recovery baseline; Apps \`13.82.148\` and \`13.82.149\` are rejected counterevidence. App \`13.82.168\` is publicly deployed and browser-operated as the uninterrupted-map/preview-morph checkpoint, but remains below surface acceptance until the user accepts the mini-info design and physical drag smoothness. Provider-positive evidence remains open.\n\n` +
