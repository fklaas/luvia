...
function validateParityFailureMatrix(document){
  assert.equal(document.rows.length,333,'every semantic action needs one matrix row');
  assert.equal(new Set(document.rows.map(row=>row.actionId)).size,333,'matrix action IDs must be unique');
  ...
  assert.equal(document.summary.catalogActions,333);assert.equal(document.summary.matrixRows,333);assert.equal(document.summary.dimensions,12);assert.equal(document.summary.protectedChanges,127);assert.equal(document.summary.publicE2eProven,7);assert.equal(document.summary.driftGuard,true);assert.equal(document.summary.ownerExecution,false);
