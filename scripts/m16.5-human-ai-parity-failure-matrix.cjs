...
function validateParityFailureMatrix(document){
  assert.equal(document.rows.length,327,'every semantic action needs one matrix row');
  assert.equal(new Set(document.rows.map(row=>row.actionId)).size,327,'matrix action IDs must be unique');
  ...
  assert.equal(document.summary.catalogActions,327);assert.equal(document.summary.matrixRows,327);assert.equal(document.summary.dimensions,12);assert.equal(document.summary.protectedChanges,124);assert.equal(document.summary.publicE2eProven,3);assert.equal(document.summary.driftGuard,true);assert.equal(document.summary.ownerExecution,false);
