...
function validateSafetyPolicy(document){
  assert.equal(document.contractId,'luvia.human-ai-safety-policy.v1');assert.equal(document.version,'1.0.0');assert.equal(document.policies.length,327);assert.equal(document.summary.policyActions,327);
  assert.equal(new Set(document.policies.map(item=>item.actionId)).size,327);assert.equal(Object.values(document.summary.classifications).reduce((sum,count)=>sum+count,0),327);assert.equal(Object.keys(document.summary.classifications).length,6);
