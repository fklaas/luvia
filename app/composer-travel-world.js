(() => {
  ...
  const VERSION = '2.7.0-island-country-identity';
  ...
  const normalizedCountryName=value=>String(value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();
  function countryForDestination(features,destination,lng,lat){
    const geographic=countryAt(features,lng,lat);if(geographic)return geographic;
    const code=String(destination?.countryCode||'').trim().toUpperCase(),addressParts=String(destination?.formattedAddress||'').split(',').map(part=>part.trim()).filter(Boolean),names=[destination?.country,destination?.countryName,addressParts.at(-1)].map(normalizedCountryName).filter(Boolean);
    return features.find(feature=>feature.properties.code===code||names.some(name=>name===normalizedCountryName(feature.properties.name)))||null;
  }
  ...
  function mount(host,{onPick=()=>{},onView=()=>{},onNavigate=()=>{},onSettled=()=>{},onIntent=()=>{},view:initial={},selection=null,interactive=true,accent='#c95169',reducedMotion=false}={}){
    ...
    async function travelTo(destination){
      const seq=++tourSequence;await ready;if(!alive||seq!==tourSequence)return false;const lat=Number(destination.latitude),lng=Number(destination.longitude),target=countryForDestination(features,destination,lng,lat);if(!target)return false;
    ...
    async function containsDestination(destination){await ready;const lng=Number(destination?.longitude),lat=Number(destination?.latitude);if(!Number.isFinite(lng)||!Number.isFinite(lat))return false;if(view.region&&region())return Boolean(countryAt([region()],lng,lat));if(view.country&&country())return Boolean(countryForDestination([country()],destination,lng,lat));if(view.continent){const found=countryForDestination(features,destination,lng,lat);return found?.properties?.continent===view.continent}return true;}
  ...
  window.LuviaComposerTravelWorld=Object.freeze({version:VERSION,markup,mount,project,unproject,countryAt,countryForDestination,geographicTrace,fitGeography});
})();
