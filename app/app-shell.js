(() => {
  ...
  const runtimeAssetUrl=path=>`${path}?v=${encodeURIComponent(window.LuviaKernelVersion?.build||'13.82.93')}`;
  ...
  function mountNavigationCompassMotion(){
    ...
    const schedule=(needle,first=false)=>{
      ...
      const timer=setTimeout(()=>{
        ...
        const animation=needle.animate(frames,{duration:Math.round(between(1150,2350)),easing:'cubic-bezier(.2,.74,.2,1)'});animations.add(animation);
    ...
    navigationCompassMotionCleanup=()=>{disposed=true;timers.forEach(clearTimeout);timers.clear();animations.forEach(animation=>{try{animation.cancel()}catch{}});animations.clear()};
  }
