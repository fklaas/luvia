(() => {
  ...
  const runtimeAssetUrl=path=>`${path}?v=${encodeURIComponent(window.LuviaKernelVersion?.build||'13.82.95')}`;
  ...
  function refreshNavigationSelection(view=activeView){
    ...
    const selectedView=activeCompassContext==='profile'?null:(compassView||view);
    ...
    root?.querySelectorAll?.('[data-compass-context="profile"]').forEach(button=>{const selected=activeCompassContext==='profile';button.classList.toggle('is-compass-context-active',selected);if(selected)button.setAttribute('aria-current','page');else button.removeAttribute('aria-current')});
  }
