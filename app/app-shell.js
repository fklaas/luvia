(() => {
  ...
  async function show(view='today',options={}){
    ...
    else if(view==='control-center-identity')content='<div id="identity-center"></div>';
    else if(view==='control-center-bookings')content='<div id="booking-control-center"></div>';
    else if(view==='control-center-inbox')content='<div id="booking-inbox"></div>';
    ...
    if(view==='control-center-identity'){try{await window.LuviaIdentityCenter.mount(stage.querySelector('#identity-center'))}catch(error){console.error(error);stage.innerHTML='<section class="lv-card" style="padding:24px"><h2>Identität & Datenschutz</h2><div class="lv-error">Das Identity Center konnte nicht geöffnet werden.</div></section>'}}
    ...
    if(view==='control-center-inbox'){try{await window.LuviaBookingInbox.mount(stage.querySelector('#booking-inbox'))}catch(error){console.error(error);stage.innerHTML='<section class="lv-card" style="padding:24px"><h2>Booking Inbox</h2><div class="lv-error">Die Booking Inbox konnte nicht geöffnet werden.</div></section>'}}
