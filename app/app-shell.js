(() => {
  ...
  const runtimeAssetUrl=path=>`${path}?v=${encodeURIComponent(window.LuviaKernelVersion?.build||'13.82.168.202')}`;
  ...
  function calendarDate(value){
    ...
    const date=new Date(Number(match[1]),Number(match[2])-1,Number(match[3]),12);return Number.isNaN(date.getTime())?null:date;
  }
  const germanMonth=date=>date?.toLocaleDateString('de-DE',{month:'long'})||'';
  function germanTripDateRange(t={}){
    const start=calendarDate(t.startDate||t.start_date),end=calendarDate(t.endDate||t.end_date),full=date=>date?.toLocaleDateString('de-DE',{day:'numeric',month:'long',year:'numeric'})||'';
    if(start&&end&&start.getFullYear()===end.getFullYear()&&start.getMonth()===end.getMonth())return `${start.getDate()}.–${end.getDate()}. ${germanMonth(end)} ${end.getFullYear()}`;
    if(start&&end&&start.getFullYear()===end.getFullYear())return `${start.getDate()}. ${germanMonth(start)} – ${end.getDate()}. ${germanMonth(end)} ${end.getFullYear()}`;
    if(start&&end)return `${full(start)} – ${full(end)}`;
    if(start)return `ab ${full(start)}`;
    if(end)return `bis ${full(end)}`;
    return '';
  }
  const companionColors=Object.freeze(['#5f8ed8','#e96f6a','#2f9b7d','#8f79c8','#e7a83f','#3c8fa5']);
  const safeProfileColor=(value,index=0)=>/^#[0-9a-f]{6}$/i.test(String(value||''))?String(value):companionColors[index%companionColors.length];
  ...
  async function signedOut({authMode=null}={}){
    ...
    const joinRendered=authMode==='recovery'?false:await window.LuviaJoinFlow?.renderIfPending?.(mountRoot,authApi.getState(),profile());
    ...
    const fallbackLogin=(reason='fallback')=>{
      window.LuviaGuidedJourneyEntry?.deactivate?.();
    ...
    try{
      await window.LuviaGuidedJourneyEntry.render(mountRoot,{authMode});
    ...
    requestAnimationFrame(()=>requestAnimationFrame(()=>{
      if(authApi.getState()?.authenticated&&!recoveryRequested())return;
      const entry=mountRoot.querySelector('[data-entry-root]'),active=entry;
      const marker=entry?.querySelector('.public-nav,.lv-public-auth-panel,.public-hero-copy')||entry;
      ...
      // The Landing is intentionally taller than the viewport. Browser Back may restore any
      // valid scroll position, so viewport intersection is not a mount-health signal.
      const visible=Boolean(active?.isConnected&&marker?.isConnected&&rect&&markerRect&&rect.width>10&&rect.height>10&&markerRect.width>10&&markerRect.height>10&&style?.visibility!=='hidden'&&style?.display!=='none'&&markerStyle?.visibility!=='hidden'&&markerStyle?.display!=='none');
  ...
  function mountNavigationCompassMotion(){
    navigationCompassMotionCleanup?.();navigationCompassMotionCleanup=null;
    const needles=[...root?.querySelectorAll?.('.lv-living-nav-ai .lv-living-compass__needle,.lv-nav--compass .lv-living-compass__needle')||[]];
    if(!needles.length||compassMotionReduced())return;
    let disposed=false;const timers=new Set(),animations=new Set(),between=(min,max)=>min+Math.random()*(max-min);
    const schedule=(needle,first=false)=>{
      if(disposed||!needle?.isConnected)return;
      const timer=setTimeout(()=>{
        timers.delete(timer);if(disposed||!needle.isConnected)return;if(document.hidden){schedule(needle,true);return}
        const swingCount=4+Math.floor(Math.random()*4),frames=[{transform:'rotate(0deg)',offset:0}];let direction=Math.random()<.5?-1:1;
        for(let index=0;index<swingCount;index+=1){if(Math.random()>.22)direction*=-1;const angle=direction*Math.round(between(index<2?68:34,index<2?178:132));frames.push({transform:`rotate(${angle}deg)`,offset:Number(((index+1)/(swingCount+1)).toFixed(3))})}
        frames.push({transform:`rotate(${Math.round(between(-14,14))}deg)`,offset:.92},{transform:'rotate(0deg)',offset:1});
        const duration=Math.round(between(1150,2350));
        if(typeof needle.animate==='function'){
          const animation=needle.animate(frames,{duration,easing:'cubic-bezier(.2,.74,.2,1)'});animations.add(animation);
          animation.finished.catch(()=>null).finally(()=>{animations.delete(animation);if(!disposed)schedule(needle,false)});
        }else{
          const segment=Math.max(120,Math.round(duration/Math.max(1,frames.length-1)));let index=0;
          needle.style.transition=`transform ${segment}ms cubic-bezier(.2,.74,.2,1)`;
          const advance=()=>{if(disposed||!needle.isConnected)return;needle.style.transform=frames[index].transform;index+=1;const phase=setTimeout(()=>{timers.delete(phase);if(index<frames.length)advance();else{needle.style.removeProperty('transition');needle.style.removeProperty('transform');if(!disposed)schedule(needle,false)}},segment);timers.add(phase)};
          advance();
        }
      },Math.round(first?between(900,2600):between(5600,18400)));
      timers.add(timer);
    };
    needles.forEach(needle=>schedule(needle,true));
    navigationCompassMotionCleanup=()=>{disposed=true;timers.forEach(clearTimeout);timers.clear();animations.forEach(animation=>{try{animation.cancel()}catch{}});animations.clear();needles.forEach(needle=>{needle.style.removeProperty('transition');needle.style.removeProperty('transform')})};
  }
  ...
  const compassSelectionSettleDuration=()=>compassMotionReduced()?0:620;
  ...
  function clearCompassSelectionNeedle(stage){
    const needle=stage?.querySelector('.lv-plan-compass-needle');needle?.style.removeProperty('transform');stage?.style.removeProperty('--lv-plan-selection-angle');['--lv-compass-direction-angle','--lv-compass-direction-color','--lv-compass-direction-soft','--lv-compass-direction-veil','--lv-compass-direction-on'].forEach(name=>stage?.style.removeProperty(name));stage?.classList.remove('is-direction-settled');if(stage?.dataset.compassNeedleMotion==='selection')delete stage.dataset.compassNeedleMotion;if(stage?.dataset.compassDirectionTone)delete stage.dataset.compassDirectionTone;
  }
  function applyCompassDirectionTone(stage,targetAngle){
    const tone=window.LuviaExperienceContractCoreV1?.resolveCompassDirectionTone?.(targetAngle);if(!stage||!tone)return tone||null;
    Object.entries(tone.cssVariables||{}).forEach(([name,value])=>stage.style.setProperty(name,value));stage.dataset.compassDirectionTone=tone.color;return tone;
  }
  ...
  function playCompassContextNeedle(stage,{entering=false,fromAngle=0,duration=entering?compassContextEntryDuration():compassContextExitDuration()}={}){
    ...
    const first=sign*(48+Math.random()*72),second=-sign*(34+Math.random()*88),third=sign*(22+Math.random()*62);
    const endAngle=entering?0:third;
    const frames=entering
      ?[{transform:`rotate(${fromAngle}deg)`},{transform:`rotate(${second}deg)`,offset:.27},{transform:`rotate(${first*.72}deg)`,offset:.61},{transform:`rotate(${-sign*(18+Math.random()*34)}deg)`,offset:.84},{transform:'rotate(0deg)'}]
      :[{transform:'rotate(0deg)'},{transform:`rotate(${first}deg)`,offset:.24},{transform:`rotate(${second}deg)`,offset:.59},{transform:`rotate(${third}deg)`,offset:.82},{transform:`rotate(${endAngle}deg)`}];
    const animation=needle.animate(frames,{duration,easing:'cubic-bezier(.38,.05,.22,1)',fill:'forwards'});stage.dataset.compassNeedleMotion='context';activeCompassAnimations.add(animation);
    const finished=animation.finished.catch(()=>null).finally(()=>{activeCompassAnimations.delete(animation);try{animation.cancel()}catch{}if(stage.dataset.compassNeedleMotion==='context')delete stage.dataset.compassNeedleMotion});
    return{endAngle,finished};
  }
  ...
  function dock(){
    return `<div class="lv-dock-wrap"><nav class="lv-dock" aria-label="Hauptnavigation">${navigationItems().map(item=>item.action==='assistant'
      ...
      :`<button class="lv-nav ${activeView===item.id?'on':''}" type="button" data-view="${item.id}" data-compass-context="${item.id}" ${activeView===item.id?'aria-current="page"':''}><span class="lv-nav-icon">${navigationIcons[item.id]||''}</span><span class="lv-nav-label">${esc(item.id==='memories'?'Erinnern':item.label)}</span></button>`).join('')}</nav></div>`;
  }
  ...
  function journeyCourse(){
    ...
    const course=hour<10
      ?{bearing:32,code:'NO 032°',title:'Ruhig in den Tag finden',next:'Ersten Impuls entdecken',progress:18}
      :hour<14
        ?{bearing:84,code:'O 084°',title:'Gemeinsam unterwegs',next:'Vor Ort treiben lassen',progress:42}
        :hour<18
          ?{bearing:137,code:'SO 137°',title:'Dem guten Moment folgen',next:'Den Abend öffnen',progress:64}
          :hour<22
            ?{bearing:221,code:'SW 221°',title:'Den Tag ausklingen lassen',next:'Lieblingsmoment bewahren',progress:82}
            :{bearing:318,code:'NW 318°',title:'Erlebtes leise bewahren',next:'Morgen neu ausrichten',progress:94};
    return `<div class="lv-living-course" role="status" aria-label="Reisekurs: ${esc(course.title)}"><span class="lv-living-course-compass" style="--course-bearing:${course.bearing}deg" aria-hidden="true"><svg viewBox="0 0 48 48"><circle cx="24" cy="24" r="20"/><path class="is-route" d="M10 29c6-10 12 8 19-2s8-7 11-9"/><path class="is-needle" d="m24 9 4 15-4 15-4-15Z"/><circle cx="24" cy="24" r="2.5"/></svg></span><span class="lv-living-course-copy"><small>Reisekurs · ${esc(course.code)}</small><b>${esc(course.title)}</b></span><span class="lv-living-course-track" style="--course-progress:${course.progress}%" aria-hidden="true"><i></i><em></em></span><span class="lv-living-course-next"><small>Nächster Impuls</small><b>${esc(course.next)}</b></span></div>`;
  }
  function companionProjection(){
    ...
    const visibleMembers=members.length?members:[profile()];
    return `${visibleMembers.slice(0,3).map((member,index)=>`<span style="--companion-color:${safeProfileColor(member.avatarColor||member.profileColor||member.color,index)}" title="${esc(member.displayName||member.name||'Mitreisende Person')}">${esc(initials(member))}</span>`).join('')}${visibleMembers.length>3?`<span class="is-more">+${visibleMembers.length-3}</span>`:''}`;
  }
  function activeTripMarkup(t){
    const dates=germanTripDateRange(t);return `<span>Aktive Reise</span><strong>${esc(t.title||'Unsere Reise')}</strong><small>${esc(t.destination?.name||'Reiseziel offen')}${dates?` · ${esc(dates)}`:''}</small><div class="lv-living-trip-accent" aria-hidden="true"><i></i><i></i><i></i><i></i></div>`;
  }
  const tripTitle=t=>String(t?.title||t?.name||t?.destination?.name||'Unbenannte Reise');
  function tripSwitchOptions(){
    const state=snap();return state.trips.map(t=>`<option value="${esc(t.id||t.tripId||'')}" ${String(t.id||t.tripId||'')===String(state.activeTripId||'')?'selected':''}>${esc(tripTitle(t))}</option>`).join('');
  }
  function tripSwitchMarkup(){
    const count=snap().trips.length;return `<label class="lv-living-trip-picker" title="Aktive Reise wechseln"><span aria-hidden="true">⇄</span><b>Reise wechseln</b><small>${count} ${count===1?'Reise':'Reisen'}</small><select data-trip-switch aria-label="Aktive Reise wechseln">${tripSwitchOptions()}</select></label>`;
  }
  async function selectTripFromControl(select){
    const id=String(select?.value||'');if(!id||id===String(snap().activeTripId||''))return;
    const command=tripContract()?.commands?.selectActiveTrip||tripContract()?.selectActiveTrip;if(typeof command!=='function')throw new Error('Der Trip Owner kann die aktive Reise gerade nicht wechseln.');
    select.disabled=true;
    try{await Promise.resolve(command(id,{source:'app-shell.trip-switcher',touch:true}));}
  ...
  function refreshNavigationSelection(view=activeView){
    ...
    const routeView=view==='timeline'?'trip':view;
    const selectedView=activeCompassContext==='profile'?null:activeCompassContext==='control-center'?null:(compassView||routeView);
    ...
    root?.querySelectorAll?.('[data-compass-context]').forEach(button=>{const selected=activeCompassContext===button.dataset.compassContext;button.classList.toggle('is-compass-context-active',selected);if(selected)button.setAttribute('aria-current','page');else if(button.dataset.view!==selectedView)button.removeAttribute('aria-current')});
  }
  ...
  async function openPlace(payload){
    ...
    if(payload.overlayOnly){
      if(enabled&&await openTypedPlaceOverlay(payload))return true;
      ...
      if(overlay){
        window.LuviaPlaceDetails.update(overlay,{typeLabel,placeType:payload.type,destination:activeTrip()?.destination?.name||'',place:basePlace,photos:immediatePhotos,intelligence:immediateIntel,lifecycle:{status:seed.lifecycle||seed.status||'discovered',title:`${typeLabel}-Lebenszyklus`}});
        if(providerId){
  ...
  async function addTodaySuggestion(element){
    if(element.dataset.suggestionQuery){
      const draft={kind:'draft-place-discovery',requiresConfirmation:true,query:element.dataset.suggestionQuery,targetDate:element.dataset.suggestionDate||'',startAt:element.dataset.suggestionStart||''};
      if(typeof window.LuviaJourneySuggestions?.open==='function')return window.LuviaJourneySuggestions.open(draft);
      window.LuviaTripPreferenceContextV1?.setDraft?.(draft);
      return show('timeline',{source:'today-suggestion-continuity'});
    }
  ...
  async function handleRuntimeAction(client,action){
    ...
    if(action.type==='runtime.resume'){
      ...
      if(['places','hotels'].includes(activeView)&&window.LuviaPlacesSpatialExperience?.resume?.(activeTrip()))return;
  ...
  function moduleMounts(){
    ...
    moduleMountRegistry.register('hotels',{mount:({target,trip,options})=>window.LuviaAccommodations.mount({roots:[target],trip},{trip,options}),unmount:()=>window.LuviaAccommodations?.unmount?.()});
  ...
  async function unmountCurrent(reason='route-change'){
    if(moduleMountRegistry)await moduleMountRegistry.deactivate({reason});
    window.LuviaProfileOnboarding?.unmount?.();
    window.LuviaFirstTripComposer?.unmount?.();
  }
  const MOUNT_FAILURES=Object.freeze({
    places:['Places','Das Places-Modul konnte nicht geöffnet werden.'],
    hotels:['Hotels & Unterkünfte','Der Hotelbereich konnte nicht geöffnet werden.'],
  ...
  function profileOnboardingReturnTo(value){
    const requested=String(value||'today');
    return !['profile-onboarding','first-trip-composer'].includes(requested)&&navigationContract().get(requested)?requested:'today';
  }
  async function leaveProfileOnboarding(returnTo,{source='profile-onboarding-close'}={}){
    const target=profileOnboardingReturnTo(returnTo);
    window.LuviaProfileOnboarding?.unmount?.();
    activeView=target;shellInitialized=false;lastRenderedTripId=null;
    const intent=navigationContract().createIntent(target,{source,replace:true});
    commitScreenIntent(intent,{replace:true,source});
    root.innerHTML='';
    return render();
  }
  async function showProfileOnboarding(intent,options={}){
    if(!window.LuviaProfileOnboarding?.mount)throw new Error('Luvia Profil-Onboarding ist noch nicht verfügbar.');
    const params=intent?.params||{};
    const returnTo=profileOnboardingReturnTo(params.returnTo||options.returnTo||activeView);
    const mode=params.profileMode==='first-use'||options.mode==='first-use'?'first-use':'edit';
    const step=params.profileStep||options.step||window.LuviaProfileOnboarding.draftStep?.()||'welcome';
    const sequence=++showSequence;
    commitScreenIntent(intent,{...options,replace:options.replace??false});
    await unmountCurrent('profile-onboarding');if(sequence!==showSequence)return;
    window.LuviaTodayExperience?.unbind?.();window.LuviaPremiumMemoriesExperience?.unbind?.();
    activeView='profile-onboarding';shellInitialized=false;lastRenderedTripId=null;root.innerHTML='';
    window.LuviaProfileOnboarding.mount(root,{
      mode,step,returnTo,
      onComplete:()=>leaveProfileOnboarding(returnTo,{source:'profile-onboarding-complete'}),
      onCancel:()=>leaveProfileOnboarding(returnTo,{source:'profile-onboarding-cancel'})
    });
    completeRuntimeStage('shell-ready',{surface:'profile-onboarding',owner:'identity'});
    completeRuntimeStage('modules-ready',{route:'profile-onboarding',mountMode:'fullscreen',owner:'identity'});
  }
  function firstTripReturnTo(value){
    const requested=String(value||'today');
    return !['profile-onboarding','first-trip-composer'].includes(requested)&&navigationContract().get(requested)?requested:'today';
  }
  async function leaveFirstTripComposer(returnTo,{source='first-trip-composer-close'}={}){
    const target=firstTripReturnTo(returnTo);
    window.LuviaFirstTripComposer?.unmount?.();
    activeView=target;shellInitialized=false;lastRenderedTripId=null;
    const intent=navigationContract().createIntent(target,{source,replace:true});
    commitScreenIntent(intent,{replace:true,source});root.innerHTML='';
    return render();
  }
  async function showFirstTripComposer(intent,options={}){
    ...
    completeRuntimeStage('shell-ready',{surface:'first-trip-composer',owner:'trip'});
    completeRuntimeStage('modules-ready',{route:'first-trip-composer',mountMode:'fullscreen',owner:'trip'});
  }
  async function show(view='today',options={}){
    ...
    if(view==='profile-onboarding')return showProfileOnboarding(intent,options);
    if(view==='first-trip-composer')return showFirstTripComposer(intent,options);
    if(activeView==='profile-onboarding')window.LuviaProfileOnboarding?.defer?.();
    if(activeView==='first-trip-composer')window.LuviaFirstTripComposer?.defer?.();
    ...
    else if(view==='timeline'){content=window.LuviaJourneyDayComposer?.renderTimeline?.(t,esc)||'<section class="lv-error">Die Timeline ist gerade nicht verfügbar.</section>';timelineLastHtml=content}
    ...
    if(view==='timeline')requestAnimationFrame(()=>window.LuviaJourneyDayComposer?.bindTimeline?.(stage));
    ...
    root.querySelector('.lv-dock-wrap')?.remove();root.querySelector('.lv-shell')?.insertAdjacentHTML('beforeend',dock());refreshNavigationSelection(view);mountNavigationCompassMotion();
  ...
  function refreshTimelineProjection(){
    if(activeView!=='timeline'||!root?.querySelector('.lv-shell')||timelineRenderFrame)return false;
    timelineRenderFrame=requestAnimationFrame(()=>{
      timelineRenderFrame=0;
      const trip=activeTrip(),stage=root.querySelector('[data-stage]'),surface=stage?.querySelector('[data-journey-timeline]'),nextHtml=trip?window.LuviaJourneyDayComposer?.renderTimeline?.(trip,esc):'';
      if(!surface||!nextHtml){show('timeline',{force:true,animate:false,scroll:false,source:'journey-owner-local-projection-fallback'}).catch(console.error);return}
      if(nextHtml===timelineLastHtml)return;
      const selectedDate=surface.querySelector('[data-timeline-date][aria-pressed="true"]')?.dataset.timelineDate,template=document.createElement('template');template.innerHTML=nextHtml.trim();const nextSurface=template.content.firstElementChild;if(!nextSurface)return;
      surface.replaceWith(nextSurface);timelineLastHtml=nextHtml;window.LuviaJourneyDayComposer?.bindTimeline?.(stage);
      if(selectedDate)nextSurface.querySelector(`[data-timeline-date="${CSS.escape(selectedDate)}"]`)?.click();
    });
    return true;
  }
  function refreshShellHeader(){const t=activeTrip(),p=profile(),shell=root?.querySelector('.lv-shell');if(!t||!shell)return;shell.querySelectorAll('.lv-living-sidebar .lv-trip').forEach(trip=>{trip.innerHTML=activeTripMarkup(t)});shell.querySelectorAll('.lv-living-profile').forEach(trigger=>{trigger.innerHTML=`${avatar(p)}<span><b>${esc(p.displayName||'Profil')}</b><small>Profil &amp; Reisekompass</small></span>`});shell.querySelectorAll('.lv-living-top-profile').forEach(trigger=>{trigger.innerHTML=`${avatar(p)}<i aria-hidden="true"></i>`});shell.querySelectorAll('[data-trip-switch]').forEach(select=>{select.innerHTML=tripSwitchOptions();select.value=String(snap().activeTripId||'')});const companions=shell.querySelector('[data-living-companions]');if(companions)companions.innerHTML=companionProjection();root.querySelector('.lv-dock-wrap')?.remove();shell.insertAdjacentHTML('beforeend',dock(t));refreshNavigationSelection();mountNavigationCompassMotion();}
  function releaseBadge(){const v=window.LuviaKernelVersion||window.LuviaCoreVersion||{core:'4.17.0',build:'13.17.0'};return `${tripSwitchMarkup()}<button class="lv-living-new-trip" type="button" data-create aria-label="Neue Reise anlegen"><span aria-hidden="true">＋</span><b>Neue Reise</b></button><span class="lv-build-badge" title="Luvia Core ${v.core} · Build ${v.build}">${v.build} · Core ${v.core}</span>`;}
  function ready(){const t=activeTrip(),p=profile();if(!t)return noTrips();const tripId=String(t.id||t.tripId||'');lastTripRenderSignature=tripRenderSignature(t);window.LuviaTheme.apply(t);if(root.querySelector('.lv-shell')){completeRuntimeStage('shell-ready',{surface:'app-shell',reused:true});refreshShellHeader();if(lastRenderedTripId!==tripId)return show(activeView,{force:true,animate:false});completeRuntimeStage('modules-ready',{route:activeView,reused:true});return}root.innerHTML=`<div class="lv-shell lv-living-shell"><div class="lv-living-atmosphere" aria-hidden="true"><i></i><i></i><i></i></div><aside class="lv-living-sidebar"><button class="lv-living-brand" type="button" data-view="today" aria-label="Luvia Heute öffnen">${livingCompass('lv-living-brand-compass')}<span><b>LUVIA</b><small>Reisen, die mit dir leben.</small></span></button><div class="lv-trip lv-living-trip">${activeTripMarkup(t)}</div>${sidebarNavigation()}<div class="lv-living-sidebar-spacer"></div><button class="lv-living-quiet-action" type="button" data-view="control-center" data-compass-context="control-center"><span class="lv-living-nav-icon">${navigationIcons.control}</span><span>Control Center</span></button><button class="lv-profile-trigger lv-living-profile" data-profile-open="hub" data-compass-context="profile">${avatar(p)}<span><b>${esc(p.displayName||'Profil')}</b><small>Profil &amp; Reisekompass</small></span></button></aside><section class="lv-living-workspace"><header class="lv-header lv-living-topbar"><button class="lv-living-mobile-brand" type="button" data-view="today" aria-label="Luvia Heute öffnen">${livingCompass('lv-living-mobile-compass')}<b>LUVIA</b></button>${journeyCourse()}<div class="lv-living-top-actions"><div class="lv-living-companions" data-living-companions aria-label="Mitreisende">${companionProjection()}</div>${releaseBadge()}<button class="lv-living-icon-button lv-living-more-button" type="button" data-view="control-center" data-compass-context="control-center" aria-label="Control Center im Living Compass öffnen">•••</button><button class="lv-profile-trigger lv-living-top-profile" data-profile-open="hub" data-compass-context="profile" aria-label="Profileinstellungen öffnen">${avatar(p)}<i aria-hidden="true"></i></button></div></header><main class="lv-stage lv-living-stage" data-stage></main></section></div>`;shellInitialized=true;completeRuntimeStage('shell-ready',{surface:'app-shell',reused:false});return show(activeView,{animate:true})}
  async function render(){const auth=(window.LuviaAuth||window.ParisAuth).getState();if(auth.loading)return bootScreen();if(recoveryRequested())return signedOut({authMode:'recovery'});if(!auth.authenticated)return signedOut();window.LuviaGuidedJourneyEntry?.deactivate?.();document.documentElement.classList.remove('lv-entry-active','lv-public-entry-active');document.body.classList.remove('lv-entry-active','lv-public-entry-active');const s=snap();if(!s.loaded)return bootScreen('Reisen werden geladen …');if(window.LuviaJoinFlow?.pending?.())return window.LuviaJoinFlow.renderIfPending(root,auth,profile());const currentIntent=navigationHistory().currentIntent();const explicitOnboarding=currentIntent?.route==='profile-onboarding';if(explicitOnboarding||window.LuviaProfileOnboarding?.shouldStart?.()){const mode=explicitOnboarding?(currentIntent.params?.profileMode==='first-use'?'first-use':'edit'):'first-use';const intent=explicitOnboarding?currentIntent:navigationContract().createIntent('profile-onboarding',{source:'profile-onboarding-gate',replace:true,params:{profileStep:window.LuviaProfileOnboarding?.draftStep?.()||'welcome',profileMode:mode,returnTo:activeView==='profile-onboarding'?'today':activeView}});return showProfileOnboarding(intent,{replace:!explicitOnboarding,mode})}const explicitFirstTrip=currentIntent?.route==='first-trip-composer';if(explicitFirstTrip||window.LuviaFirstTripComposer?.shouldStart?.()){const mode=explicitFirstTrip?(currentIntent.params?.tripMode==='first-use'?'first-use':'create'):'first-use';const intent=explicitFirstTrip?currentIntent:navigationContract().createIntent('first-trip-composer',{source:'first-trip-composer-gate',replace:true,params:{tripStep:window.LuviaFirstTripComposer?.draftStep?.()||'welcome',tripMode:mode,returnTo:'today'}});return showFirstTripComposer(intent,{replace:!explicitFirstTrip,mode})}if(!s.hasTrips||!s.hasActiveTrip)return noTrips();return ready()}
  ...
  function schedulePwaRegistration(){
    if(pwaRegistrationScheduled)return;
    pwaRegistrationScheduled=true;
    const afterLoad=()=>{
      const start=()=>Promise.resolve(window.LuviaPWA?.register?.()).then(()=>window.LuviaPWA?.warmShell?.()).catch(error=>console.warn('[LuviaPWA]',error));
  ...
  function waitForPlatformPorts(timeoutMs=2500){
    if(window.LuviaPlatformPorts?.get)return Promise.resolve(window.LuviaPlatformPorts);
    if(window.LuviaPlatformPortsReady)return Promise.resolve(window.LuviaPlatformPortsReady).then(ports=>{
      if(!ports?.get)throw new Error('Luvia Platform Ports wurden nicht vollständig registriert.');
      return ports;
    });
    return new Promise((resolve,reject)=>{
      let timer=0;
      const cleanup=()=>{window.removeEventListener('luvia:platform-ports-ready',onReady);if(timer)clearTimeout(timer)};
      const onReady=()=>{
        const ports=window.LuviaPlatformPorts;
        if(!ports?.get)return;
        cleanup();resolve(ports);
      };
      window.addEventListener('luvia:platform-ports-ready',onReady);
      timer=setTimeout(()=>{cleanup();reject(new Error('Luvia Platform Ports konnten nicht rechtzeitig geladen werden.'))},timeoutMs);
    });
  }
  async function bootstrap(){
    ...
    try{
      ...
      const client=await appRuntime().run('platform-ready',async()=>{await waitForPlatformPorts();return window.LuviaSupabaseService.start()},{timeoutMs:5000,detail:{source:'app-shell',capability:'platform-ports+supabase-service'}});
      ...
      if(!subscriptionsBound){
        ...
        unsubscribeTrip=tripContract()?.subscribe?.(s=>{if(!bootComplete)return;const active=s?.activeTrip||null,context=s?.context||{},activeTripId=String(active?.id||active?.tripId||context?.tripId||'')||null;if(!activeTripId)return;const token=++tripSwitchToken;Promise.resolve().then(async()=>{if(token!==tripSwitchToken)return;if(profile().activeTripId!==activeTripId&&authApi.getState().authenticated)await window.LuviaProfileService.setActiveTrip(activeTripId).catch(console.warn);await window.LuviaJourneyContractV1?.commands?.hydrate?.(activeTripId).catch(()=>{});if(token!==tripSwitchToken)return;window.LuviaDestination?.refresh?.();if(window.LuviaCollaboration?.snapshot?.().tripId!==activeTripId)window.LuviaCollaboration?.watchTrip?.(activeTripId).catch?.(console.warn);const nextSignature=tripRenderSignature(active),requiresViewRender=lastRenderedTripId!==activeTripId||nextSignature!==lastTripRenderSignature;lastTripRenderSignature=nextSignature;refreshShellHeader();if(requiresViewRender)await show(activeView,{force:true,animate:false});}).catch(console.error)});
      ...
      bootComplete=true;
      errorScreen(error);
      await window.LuviaBootCoordinator?.reveal?.();
    }
  }
  ...
  function handleHubAction(action,meta={}){
    ...
    if(['today','plan','trip','memories','more','places','hotels','places-lifecycle','timeline','routes','gallery','albums','bookings','control-center','control-center-identity','control-center-bookings','control-center-inbox'].includes(action))return show(action,{focusHeading});
    if(action==='assistant')return show('today').then(()=>window.LuviaAIDashboard?.openChat?.());
    if(action==='new-trip')return window.LuviaFirstTripComposer?.open?.({mode:'create',returnTo:activeView});
    if(action==='profile')return show('more').then(()=>window.LuviaProfileFoundation.open('hub'));
    if(action==='preferences')return window.LuviaProfileOnboarding?.open?.({mode:'edit',returnTo:activeView==='profile-onboarding'?'more':activeView});
  ...
  async function openLivingCompassContext(context='plan',{focus=true,intentSequence=++compassIntentSequence}={}){
    const key=['today','plan','trip','memories','profile','control-center'].includes(context)?context:'plan';
    ...
    planCompassTransition=true;cancelCompassFlights();lastCompassFocus=document.activeElement instanceof HTMLElement?document.activeElement:null;
    currentStage.classList.add('is-context-leaving','is-context-seeking','is-context-pulsing');
    const leavingNeedle=playCompassContextNeedle(currentStage,{duration:compassContextExitDuration()});
    await Promise.all([compassWait(compassContextExitDuration()),leavingNeedle.finished]);
    ...
    const enteringNeedle=playCompassContextNeedle(next,{entering:true,fromAngle:leavingNeedle.endAngle,duration:compassContextEntryDuration()});
    await Promise.all([compassWait(compassContextEntryDuration()),enteringNeedle.finished]);
  ...
  async function leavePlanCompass(destination='today',selected=null,intentSequence=++compassIntentSequence){
    ...
    try{
      if(selected){const angle=Number(selected.dataset.planAngle||0),directAngle=normalizeCompassAngle(angle);applyCompassDirectionTone(stage,directAngle);selected.classList.add('is-selected');const needleMotion=playCompassSelectionNeedle(stage,directAngle);await needleMotion.finished;if(intentSequence===compassIntentSequence&&stage.isConnected){stage.classList.add('is-direction-settled');await compassWait(compassSelectionSettleDuration())}}else stage.classList.add('is-navigating');
  ...
  function bind(){root.addEventListener('click',e=>{
    ...
    if(e.target.closest('[data-retry]'))return bootstrap();if(e.target.closest('[data-create]'))return window.LuviaFirstTripComposer?.open?.({mode:'create',returnTo:activeView||'today'});if(e.target.closest('[data-signout]'))return (window.LuviaAuth||window.ParisAuth).signOut();if(e.target.closest('[data-join-code]'))return window.LuviaJoinFlow?.openCodeEntry?.();if(e.target.closest('[data-invite]'))return openDialog('trip.invite',activeTrip());if(e.target.closest('[data-edit]'))return openDialog('trip.edit',activeTrip());
    const tripSwitchOpen=e.target.closest('[data-trip-switch-open]');if(tripSwitchOpen){e.preventDefault();e.stopPropagation();const select=root.querySelector('.lv-living-topbar [data-trip-switch]');select?.focus?.({preventScroll:true});if(typeof select?.showPicker==='function')try{select.showPicker()}catch{}return}
    ...
    const view=e.target.closest('button[data-view],a[data-view],[role="button"][data-view]')?.dataset.view;if(view){e.preventDefault();e.stopPropagation();++compassIntentSequence;const context=compassContextForView(view),compassStage=currentPlanCompassStage();if(context&&compassStage)return openLivingCompassContext(context,{intentSequence:compassIntentSequence});if(compassStage)return leavePlanCompass(view,null,compassIntentSequence);return show(view)}
  });root.addEventListener('change',e=>{const select=e.target.closest?.('[data-trip-switch]');if(select)selectTripFromControl(select).catch(error=>{console.error('[Luvia Trip Switcher]',error);window.LuviaUIKit?.toast?.(error.message||'Reise konnte nicht gewechselt werden.',{type:'error'})})});root.addEventListener('pointerdown',rememberCompassPointer,true);root.addEventListener('pointerup',releaseCompassPointer,true);root.addEventListener('pointercancel',()=>{compassPointerGesture=null},true);root.addEventListener('keydown',e=>{
