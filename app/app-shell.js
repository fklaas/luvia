(() => {
  ...
  const runtimeAssetUrl=path=>`${path}?v=${encodeURIComponent(window.LuviaKernelVersion?.build||'13.82.74')}`;
  ...
  function schedulePwaRegistration(){
    ...
    const afterLoad=()=>{
      const start=()=>window.LuviaPWA?.register?.().catch(error=>console.warn('[LuviaPWA]',error));
