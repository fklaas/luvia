(() => {
  ...
  const runtimeAssetUrl=path=>`${path}?v=${encodeURIComponent(window.LuviaKernelVersion?.build||'13.82.135')}`;
  ...
  function handleHubAction(action,meta={}){
    ...
    if(['today','plan','trip','memories','more','places','places-lifecycle','timeline','routes','gallery','albums','bookings','control-center','control-center-identity','control-center-bookings','control-center-inbox'].includes(action))return show(action,{focusHeading});
