(() => {
  ...
  const BOOKING_AVAILABILITY_SRC='core/booking/booking-availability.js?v=13.68.6';
  const BOOKING_RESERVATION_CREATE_SRC='core/booking/booking-reservation-create.js?v=13.68.6';
  const BOOKING_RESERVATION_MUTATION_SRC='core/booking/booking-reservation-mutation.js?v=13.68.6';
  const BOOKING_RESERVATION_MUTATION_STATUS_SRC='core/booking/booking-reservation-mutation-status.js?v=13.68.6';
  const BOOKING_RESERVATION_RECOVERY_SRC='core/booking/booking-reservation-recovery.js?v=13.68.6';
  const BOOKING_EMAIL_V2_SRC='core/booking/booking-email-v2.js?v=13.68.6';
  ...
  async function signedOut(){
    ...
    if(await window.LuviaJoinFlow?.renderIfPending?.(root,authApi.getState(),profile()))return;
    const fallbackLogin=(reason='fallback')=>{
      ...
      root.innerHTML=`<main class="lv-auth lv-card" data-luvia-auth-recovery="${reason}"><header class="lv-auth-head"><span class="lv-logo"></span><h1>Willkommen bei Luvia</h1><p>Melde dich an, um deine Reisen auf diesem Gerät zu öffnen.</p></header><div class="lv-auth-slot" data-auth-slot></div></main>`;
      authUI?.renderAuthForm?.(root.querySelector('[data-auth-slot]'),'login');
    };
    if(!window.LuviaGuidedJourneyEntry?.render){fallbackLogin('entry-missing');return;}
    try{window.LuviaGuidedJourneyEntry.render(root)}catch(error){console.error('[LuviaAuthBoot] Public Entry konnte nicht gerendert werden',error);fallbackLogin('entry-render-error');return;}
    requestAnimationFrame(()=>requestAnimationFrame(()=>{
      const entry=root.querySelector('[data-entry-root]'),active=entry?.querySelector('[data-screen].is-active');
      ...
      const visible=Boolean(active&&marker&&rect&&markerRect&&rect.width>10&&rect.height>10&&markerRect.width>10&&markerRect.height>10&&style?.visibility!=='hidden'&&style?.opacity!=='0'&&markerStyle?.visibility!=='hidden'&&markerStyle?.opacity!=='0'&&markerStyle?.display!=='none');
