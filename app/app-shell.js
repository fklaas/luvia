(() => {
  ...
  const runtimeAssetUrl=path=>`${path}?v=${encodeURIComponent(window.LuviaKernelVersion?.build||'13.82.106')}`;
  ...
  function refreshNavigationSelection(view=activeView){
    ...
    const selectedView=activeCompassContext==='profile'?null:activeCompassContext==='control-center'?null:(compassView||view);
  ...
  async function addTodaySuggestion(element){
    if(element.dataset.suggestionQuery){
      ...
      return show('places',{source:'today-preference-suggestion',planningDraft:draft});
    }
