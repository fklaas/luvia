(() => {
  ...
  const runtimeAssetUrl=path=>`${path}?v=${encodeURIComponent(window.LuviaKernelVersion?.build||'13.82.88')}`;
  ...
  function companionProjection(){
    ...
    return `<div class="lv-living-companions" aria-label="Mitreisende">${members.slice(0,3).map((member,index)=>`<span style="--companion-color:${safeProfileColor(member.avatarColor||member.profileColor||member.color,index)}" title="${esc(member.displayName||member.name||'Mitreisende Person')}">${esc(initials(member))}</span>`).join('')}${members.length>3?`<span class="is-more">+${members.length-3}</span>`:''}</div>`;
  }
