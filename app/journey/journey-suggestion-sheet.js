...
const VERSION='1.19.0-clean-card-photo-first';
const cache=new Map();
...
function currentInput(input={}){
  ...
  return{
    ...
    reasons:[...(input.reasons||guidance.reasons||[])],
     snapshot,planningPolicy:{...(dayGuidance.policy||{}),...(input.planningPolicy||{})},weather:input.weather||globalThis.LuviaWeatherContextV1?.current||null,positionContext,contextGate:contextGate||null,source:clean(input.source)||'timeline-suggestion',requestedCount:Number(input.requestedCount)||null,stayDecision:input.stayDecision||null,onSelectionChange:typeof input.onSelectionChange==='function'?input.onSelectionChange:null,excludeProviderPlaceIds:[...(input.excludeProviderPlaceIds||[])].map(clean).filter(Boolean)
  };
}
...
function stayPriceMarkup(place,input){
  ...
  return`<span class="lvjs-stay-price"><strong>${esc(amount)} gesamt</strong><small>${esc(provider)} · Livepreis für ${esc(input.stayDecision?.query?.checkIn||input.targetDate)} bis ${esc(input.stayDecision?.query?.checkOut||'Check-out')} · finale Quote vor Buchung</small></span>`;
}
...
const cuisineLabels=place=>[...(place.cuisineTypes||[]),...(place.types||[])].map(type=>clean(type).toLowerCase()).filter(type=>type&&!['restaurant','food','food establishment','cafe','coffee shop','bakery','establishment','point of interest'].includes(type.replace(/_/g,' '))).map(type=>type.replace(/_restaurant$/,'').replace(/_/g,' ')).filter((type,index,all)=>all.indexOf(type)===index).slice(0,6).join(', ');
...
function providerDetailRows(place,input){
  ...
  add('Kategorie',clean(place.primaryTypeDisplayName?.text||place.primaryTypeLabel||place.primaryType||visualLabel(visual)).replace(/_/g,' '));
  if(['food','cafe'].includes(visual))add('Küche',clean(place.cuisine||place.cuisineType||cuisineLabels(place)));
...
function shellMarkup(input){
  ...
  return`<header class="lvjs-header"><div><span>${hotelMap?'Hotels':placesSearch?'Places entdecken':'Luvia'} · ${esc(input.targetDate)}</span><h2 data-lvjs-heading>${esc(title)}</h2><p>${esc(copy)}</p></div><button type="button" data-lvjs-close aria-label="${mapResults?'Zurück zur Karte':'Vorschläge schließen'}">×</button></header><div class="lvjs-status" data-lvjs-status role="status" aria-live="polite"><span class="lvjs-loader" aria-hidden="true"></span><div><strong>Luvia prüft ${esc(destination)} …</strong><small>Orte werden gesucht, fachlich gefiltert und für alle Reisenden belegbar gewichtet.</small></div></div><div class="lvjs-results" data-lvjs-results hidden></div><footer class="lvjs-footer"><span data-lvjs-ai-state>Places belegt · Luvia ordnet · ihr bestätigt</span><button type="button" class="lvjs-footer-plan" data-lvjs-plan-selected hidden>Zur Timeline hinzufügen</button><button type="button" data-lvjs-spectrum>Alle Richtungen entdecken</button><button type="button" data-lvjs-retry hidden>Erneut prüfen</button></footer>`;
}
...
async function commit(place,form,input,override={}){
  ...
  const bookingPending=Boolean(override.bookingStatus),fields={planned_at:plannedAt,place_name:place.name,notes:bookingPending?'Besuch eingeplant; Buchung oder Reservierung ist noch nicht bestätigt.':'Von Luvia vorgeschlagen und ausdrücklich bestätigt.',metadata:{
...
function paintResults(handle,result,selectedId='',restoredState=null){
  ...
  const sync=()=>{
    ...
    root.querySelectorAll('[data-suggestion-select]').forEach(button=>{
      ...
      if(!scheduler)return;scheduler.hidden=!done;if(!selected)return;
  ...
  results.addEventListener('click',async event=>{
    ...
    if(bookingButton){const id=bookingButton.dataset.lvjsBooking,place=result.choices.find(item=>providerId(item)===id),form=schedulerFor(id);if(!selectedIds.has(id)){selectedIds.add(id);ensurePlan(place);sync()}const item=scheduleFor(selectedPlaces(),plans,result.input).find(row=>providerId(row.place)===id),cardState=form?.querySelector('[data-lvjs-card-state]');try{if(!item?.fits)throw new Error('Der vorgeschlagene Zeitpunkt kollidiert mit eurem Reisetag. Bitte den Timeline-Eintrag später anpassen.');let plannedPlace=place;if(!completed.has(id)){bookingButton.disabled=true;bookingButton.textContent='Timeline wird vorbereitet …';const outcome=await commitOrPropose({...item,bookingStatus:'pending_user_action'},result.input,form);if(outcome.kind==='proposal'){completed.set(id,'proposal');if(cardState){cardState.className='lvjs-card-state is-success';cardState.textContent='Die Gruppenabstimmung läuft. Erst nach Freigabe wird eine Buchung geöffnet.'}sync();return}completed.set(id,'booking-pending');plannedPlace={...place,tripPlaceId:outcome.value?.entity?.tripPlaceId,placeId:outcome.value?.entity?.id};if(cardState){cardState.className='lvjs-card-state is-success';cardState.textContent='In der Timeline als unbestätigt markiert. Der Booking-Status folgt aus der Anbieterantwort.'}sync()}await openBooking(plannedPlace,bookingButton,form,handle,result,stateSnapshot())}catch(error){if(cardState){cardState.hidden=false;cardState.className='lvjs-card-state is-error';cardState.textContent=error?.message||'Booking konnte nicht geöffnet werden.'}bookingButton.disabled=false}return}
