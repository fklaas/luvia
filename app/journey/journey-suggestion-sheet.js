...
function providerDetailMedia(place){
  const image=imageUrl(place),attribution=imageAttribution(place);
  if(!image)return`<div class="lvjs-provider-detail-media is-empty" role="img" aria-label="Kein Foto dieses Ortes verfügbar"><span>${esc(visualIcon(visualCategory(place)))}</span><small>Für diesen Ort ist noch kein sicher zugeordnetes Bild verfügbar.</small></div>`;
  const source=safeHttpUrl(attribution?.sourceUrl),credit=[attribution?.credit,attribution?.license].filter(Boolean).join(' · ')||'Bild aus dem verknüpften Ortseintrag';
  return`<figure class="lvjs-provider-detail-media"><img src="${esc(image)}" alt="${esc(placeDisplayName(place))}" loading="eager" fetchpriority="high" decoding="async"><figcaption>${source?`<a href="${esc(source)}" target="_blank" rel="noopener noreferrer">${esc(credit)}</a>`:esc(credit)}</figcaption></figure>`;
}
