...
function intelligenceMarkup(model,esc){
  const suggestion=model.suggestion;
  return `<article class="lvt-glass lvt-intelligence" data-today-ai-card><header><span>✦</span><div><small>Luvia Intelligence</small><strong data-ai-status>${suggestion?'Aus Profil + Reise abgeleitet':'Denkt im Hintergrund mit'}</strong></div><button type="button" data-ai-brief-refresh aria-label="Vorschlag neu denken">↻</button></header><button class="lvt-intelligence-idea" type="button" ${suggestion?`data-today-suggestion-add data-suggestion-query="${esc(suggestion.query)}" data-suggestion-date="${esc(suggestion.targetDate)}" data-suggestion-start="${esc(suggestion.startAt)}"`:'data-ai-ask-open'}><strong data-ai-headline>${esc(suggestion?.query||'Eure Reise nimmt weiter Form an.')}</strong><span data-ai-message>${esc(suggestion?.reasons?.[0]||'Luvia verbindet Reisekompass, Zeitraum, Journey und bestätigte Places – ohne selbst etwas einzuplanen.')}</span><em>${suggestion?'In Places ansehen':'Mit Luvia entdecken'} ${todayIcon('arrow')}</em></button><div class="lvt-ai-dots" aria-label="Wechselnde Reiseimpulse"><i class="is-active"></i><i></i><i></i></div><footer>KI entwirft · Owner-Cores belegen · ihr bestätigt</footer></article>`;
}
...
function focusedTodayMarkup(model,esc){
  ...
  return `<section class="lvt-premium lvt-premium--living-day lvt-today-focus" data-today-premium data-experience-state="${esc(model.state)}" data-journey-projection="journey.v1-read-only" data-place-projection="places.v1-read-only" data-ai-projection="dashboard.brief-draft" data-photo-resolution="${esc(photo.kind)}" aria-labelledby="lvt-title"><img class="lvt-hero-image" data-today-destination-image src="${esc(photo.url)}" alt="${esc(photo.alt)}" fetchpriority="high"><a class="lvt-photo-credit" data-today-photo-credit href="${esc(photo.source)}" target="_blank" rel="noreferrer">${esc(photo.credit)} ↗</a><div class="lvt-hero-shade" aria-hidden="true"></div><div class="lvt-play-orbits" aria-hidden="true"><i></i><i></i></div><header class="lvt-focus-heading"><span>Heute · ${esc(model.context.label)}</span><h1 id="lvt-title">${esc(model.greeting)} <em>${esc(destination)}</em> rückt näher.</h1><p>${esc(model.context.tripTitle)} · ${esc(model.summary)}</p></header><div class="lvt-focus-top">${weatherMarkup(model,esc)}${avatarMarkup(model,esc)}</div><div class="lvt-focus-bottom">${metricMarkup(model,esc)}${intelligenceMarkup(model,esc)}</div></section>`;
}
...
function bind(container,context={}){
  ...
  const hydrateAi=async(force=false)=>{const card=container.querySelector('[data-today-ai-card]');if(!card||!window.LuviaAI?.run)return;const status=card.querySelector('[data-ai-status]');if(status)status.textContent='Luvia denkt mit OpenAI …';try{const response=await window.LuviaAI.run('dashboard.brief',{currentMoment:{surface:'today',tripId:tripId(context.trip),destination:context.trip?.destination?.name||context.trip?.destinationName||'',reason:'visible-today-brief'}},{fallback:true,force});const data=response?.data||{};const ideas=[{headline:data.headline,message:data.message},...(data.highlights||[]).slice(0,2).map(highlight=>({headline:highlight,message:'Aus eurem Journey-Kontext – geplant wird erst nach Bestätigung.'}))].filter(item=>clean(item.headline));if(!ideas.length)return;if(status)status.textContent=response?.meta?.fallback?'Regelbasiert abgesichert':'OpenAI · Kontext aus Owner-Cores';let index=0;const applyIdea=()=>{const idea=ideas[index%ideas.length];card.querySelector('[data-ai-headline]').textContent=idea.headline;card.querySelector('[data-ai-message]').textContent=idea.message||'';card.querySelectorAll('.lvt-ai-dots i').forEach((dot,dotIndex)=>dot.classList.toggle('is-active',dotIndex===index%Math.min(3,ideas.length)));index++};applyIdea();const cycle=setInterval(applyIdea,8000);disposers.push(()=>clearInterval(cycle))}catch{if(status)status.textContent='Regelbasiert abgesichert'}};
