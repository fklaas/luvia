...
function intelligenceMarkup(model,esc){
  ...
  return `<article class="lvt-glass lvt-intelligence" data-today-ai-card><header><span class="lvt-guidance-mark" aria-hidden="true"><i></i></span><div><small>Luvia</small><strong data-ai-status>${suggestion?'Aus eurem Reisekompass':'Neue Ideen entstehen im Hintergrund'}</strong></div><button type="button" data-ai-brief-refresh aria-label="Vorschläge neu denken">↻</button></header><div class="lvt-ai-slider" data-ai-slider aria-label="KI-Vorschläge für eure Planung"><button class="lvt-intelligence-idea" type="button" ${suggestion?`data-today-suggestion-add data-suggestion-query="${esc(suggestion.query)}" data-suggestion-date="${esc(suggestion.targetDate)}" data-suggestion-start="${esc(suggestion.startAt)}"`:'data-ai-ask-open'}><small>Für eure Reise</small><strong>${esc(title)}</strong><span>Aus Profil, Reisegefühl und Zeitraum abgeleitet. Ihr entscheidet, was daraus wird.</span><em>${suggestion?'In Places prüfen':'Mit Luvia entdecken'} ${todayIcon('arrow')}</em></button></div><footer>KI entwirft · Owner-Cores belegen · ihr bestätigt</footer></article>`;
}
...
function bind(container,context={}){
  ...
  const hydrateAi=async(force=false)=>{const card=container.querySelector('[data-today-ai-card]'),slider=card?.querySelector('[data-ai-slider]');if(!card||!slider||!window.LuviaAI?.run)return;const status=card.querySelector('[data-ai-status]'),destination=clean(context.trip?.destination?.name||context.trip?.destinationName||'eurem Reiseziel'),goal=`Liefere bis zu vier unterschiedliche, konkrete Reiseideen für ${destination}, passend zu Profil, Reisegefühl, Zeitraum und bereits geplanter Journey. Jede searchPlans.query ist ein natürlicher deutscher Kartentitel mit höchstens 48 Zeichen, wiederholt den Ortsnamen nicht und verwendet keine Formulierungen wie konkrete Orte, passende Ideen oder vollständige Erklärungssätze.`;if(status)status.textContent='Luvia schaut für euch voraus …';try{const [brief,planning]=await Promise.all([window.LuviaAI.run('dashboard.brief',{currentMoment:{surface:'today',tripId:tripId(context.trip),destination,reason:'visible-today-brief'}},{fallback:true,force}),window.LuviaAI.run('discovery.plan',{domain:'places',contract:{domain:'places',query:goal,includedTypes:[],excludedTypes:[],featureRequirements:{evidenceRequired:true},labels:{context:['today-dashboard',destination],priorities:['profile-and-trip-fit','different-experience-types','short-natural-card-titles']},freeText:goal},currentMoment:{surface:'today',domain:'places',tripId:tripId(context.trip),destination,reason:'visible-planning-suggestions'}},{fallback:true,force})]);const plan=planning?.data||{},seen=new Set();let ideas=(plan.searchPlans||[]).map(item=>({query:clean(item?.query),headline:ideaTitle(item?.query,destination)})).filter(item=>item.query.length>=5&&item.headline&&!seen.has(item.headline.toLowerCase())&&seen.add(item.headline.toLowerCase()));if(planning?.meta?.fallback)ideas=ideas.slice(0,1);else ideas=ideas.slice(0,4);if(!ideas.length)return;if(status)status.textContent=planning?.meta?.fallback?'Sicherer Vorschlag aus eurem Reisekompass':'Mit OpenAI aus eurem Reisekompass';slider.innerHTML=ideas.map((idea,index)=>`<button class="lvt-intelligence-idea" type="button" data-today-suggestion-add data-suggestion-query="${fallbackEsc(idea.query)}" title="${fallbackEsc(idea.query)}"><small>Idee ${String(index+1).padStart(2,'0')}</small><strong>${fallbackEsc(idea.headline)}</strong><span>Aus Profil, Reisegefühl und Zeitraum abgeleitet. Ihr entscheidet, was daraus wird.</span><em>In Places prüfen ${todayIcon('arrow')}</em></button>`).join('');slider.classList.toggle('has-multiple',ideas.length>1)}catch{if(status)status.textContent='Sicherer Vorschlag aus eurem Reisekompass'}};
