...
function intelligenceMarkup(model,esc){
  ...
  return `<article class="lvt-glass lvt-intelligence" data-today-ai-card><header>${todayCompass('lvt-intelligence-compass')}<div><small>Luvia</small><strong data-ai-status>${suggestion?'Luvia prüft drei reale Möglichkeiten':'Neue Ideen entstehen im Hintergrund'}</strong></div><button type="button" data-ai-brief-refresh aria-label="Vorschläge neu prüfen">↻</button></header><div class="lvt-ai-slider" data-ai-slider aria-label="Luvias Vorschläge für eure Planung"><button class="lvt-intelligence-idea" type="button" ${suggestion?`data-today-suggestion-add data-suggestion-query="${esc(suggestion.query)}" data-suggestion-date="${esc(suggestion.targetDate)}" data-suggestion-start="${esc(suggestion.startAt)}"`:'data-ai-ask-open'}><small>Nächster möglicher Moment</small><strong>${esc(title)}</strong><span>Profil, Reisegefühl, Zeitraum und bestätigter Tagesplan fließen zusammen. Ihr entscheidet.</span><em>${suggestion?'Drei Vorschläge ansehen':'Mit Luvia entdecken'} ${todayIcon('arrow')}</em></button></div><footer>Places belegt die Fakten · Luvia ordnet · ihr bestätigt</footer></article>`;
}
