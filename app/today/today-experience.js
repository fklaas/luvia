...
function ideaTitle(value,destination=''){
  ...
  const place=clean(destination).split(',')[0];
  if(place)title=title.replace(new RegExp(`^${escapePattern(place)}(?:\\s*[-–—:·]|\\s+)+`,'i'),'');
  title=title
    .replace(/\s+(?:konkrete|passende|besondere|authentische)\s+(?:orte|ideen|ausflüge|erlebnisse)\b.*$/i,'')
    .replace(/\s+für\s+(?:einen?|eine|eure?|die)\s+(?:entspannten?|besonderen?|authentischen?|ruhigen?)\b.*$/i,'')
    .replace(/\s+im\s+(?:frühling|sommer|herbst|winter|januar|februar|märz|april|mai|juni|juli|august|september|oktober|november|dezember)\b.*$/i,'')
    .replace(/\s+und\s+/gi,' & ')
    .replace(/\s+/g,' ')
    .trim();
  if(!title)title=clean(value);
  if(title.length>62){const shortened=title.slice(0,59).replace(/\s+\S*$/,'').trim();title=`${shortened||title.slice(0,59).trim()}…`}
  return title;
}
...
function journeyEntries(journey={}){
  journey=journey||{};
  const buckets=[journey.day?.entries,journey.currentDay?.entries,...(journey.days||[]).map(day=>day?.entries),journey.entries,journey.events].filter(Array.isArray);
...
function todayCompass(className=''){
  const base='assets/brand/luvia-living-compass/layers';
  return`<span class="lvt-official-compass ${className}" aria-hidden="true"><img src="${base}/face.svg" alt=""><img class="is-needle" src="${base}/two-ended-needle.svg" alt=""><img src="${base}/hub.svg" alt=""></span>`;
}
...
function liveMapMarkup(model,esc){
  ...
  return `<section class="lvt-live-map" aria-label="Eure heutige Timeline" data-owner="journey.v1"><header><span><i aria-hidden="true"></i><span><small>Heute live</small><strong>Euer Tag bewegt sich</strong></span></span><button type="button" data-view="timeline">Timeline öffnen ${todayIcon('arrow')}</button></header><div class="lvt-route-flow">${items.map((item,index)=>`<span class="is-${esc(item.kind)}"><i>${todayIcon(item.kind==='done'?'check':item.kind==='now'?'route':item.kind==='open'?'spark':'pin')}</i><b>${esc(item.time)}</b><small>${esc(item.title)}</small></span>${index<items.length-1?'<b aria-hidden="true"></b>':''}`).join('')}</div><footer><span>${todayIcon('route')} ${esc(model.context.tripTitle)}</span><span>${todayIcon('cloud')} ${gaps?`${gaps} ${gaps===1?'offenes Zeitfenster':'offene Zeitfenster'}`:'Tag vollständig geplant'}</span><span>${todayIcon('shield')} Vorschlag bleibt Entwurf</span></footer></section>`;
}
...
function intelligenceMarkup(model,esc){
  const suggestion=model.suggestion;
  const destination=clean(model.trip?.destination?.name||model.trip?.destinationName||model.context.destination);
  const title=ideaTitle(suggestion?.query||'Eine Idee, die zu euch passt',destination);
  return `<article class="lvt-glass lvt-intelligence" data-today-ai-card><header>${todayCompass('lvt-intelligence-compass')}<div><small>Luvia</small><strong data-ai-status>${suggestion?'Luvia prüft passende reale Möglichkeiten':'Neue Ideen entstehen im Hintergrund'}</strong></div><button type="button" data-ai-brief-refresh aria-label="Vorschläge neu prüfen">↻</button></header><div class="lvt-ai-slider" data-ai-slider aria-label="Luvias Vorschläge für eure Planung"><button class="lvt-intelligence-idea" type="button" ${suggestion?`data-today-suggestion-add data-suggestion-query="${esc(suggestion.query)}" data-suggestion-date="${esc(suggestion.targetDate)}" data-suggestion-start="${esc(suggestion.startAt)}"`:'data-ai-ask-open'}><small>Nächster möglicher Moment</small><strong>${esc(title)}</strong><span>Profile, Reisegefühl, Zeitraum, Wetter, Wege und bestätigte Pläne fließen zusammen. Ihr entscheidet.</span><em>${suggestion?'Vorschläge ansehen':'Mit Luvia entdecken'} ${todayIcon('arrow')}</em></button></div><footer>Places belegt die Fakten · Luvia ordnet · ihr bestätigt</footer></article>`;
}
function journeyEntryMarkup(model,esc){
  const d=model.dashboard,count=d.entries.length;
  return `<nav class="lvt-journey-entry" aria-label="Deine Reise direkt weiterführen"><span><small>Deine Reise</small><strong>${esc(model.context.tripTitle)}</strong><em>${esc(d.countdown.label)} · ${count} ${count===1?'Moment':'Momente'} geplant</em></span><div class="lvt-journey-actions"><button type="button" data-trip-switch-open>Reise wechseln</button><button type="button" data-view="timeline">Tag öffnen</button><button type="button" data-view="trip">Anpassen ${todayIcon('arrow')}</button><button class="lvt-journey-help" type="button" data-ai-ask-open aria-label="Luvia um Hilfe für diese Reise bitten">✦</button></div></nav>`;
}
...
function focusedTodayMarkup(model,esc){
  ...
  const headline=model.context.phase==='during'
    ?['Heute gehört ',' euch.']
    :model.context.phase==='after'
      ?['Was in ',' bleibt.']
...
function sequenceMarkup(model,esc){
  ...
  return `<div class="lvt-day-sequence" aria-label="Heutige Timeline">${moments.map((item,index)=>`<span class="is-${esc(item.kind)}"><i>${esc(item.time)}</i><b>${esc(item.title)}</b>${index<moments.length-1?'<em aria-hidden="true"></em>':''}</span>`).join('')}</div>`;
}
...
function bind(container,context={}){
  ...
  const hydrateAi=async(force=false)=>{const card=container.querySelector('[data-today-ai-card]'),slider=card?.querySelector('[data-ai-slider]'),suggestions=window.LuviaJourneySuggestions;if(!card||!slider||!suggestions?.load)return;const status=card.querySelector('[data-ai-status]');if(status)status.textContent='Luvia prüft reale Möglichkeiten …';try{const guidance=window.LuviaTripPreferenceContextV1?.dayGuidance?.(window.LuviaJourneyContractV1?.reads?.snapshot?.({trip:context.trip})||{})?.suggestion||{};const result=await suggestions.load(guidance,{force}),choices=(result.choices||[]).slice(0,3);if(!choices.length)return;if(status)status.textContent=result.stale?'Letzter belegter Vorschlagsstand':result.ai?.ranking?'Mit OpenAI persönlich geordnet':'Persönlich und nachvollziehbar geordnet';slider.innerHTML=choices.map((place,index)=>{const query=clean(place.name),reason=clean(place.aiReasons?.[0]||place.recommendation?.reasons?.[0]||result.input?.reasons?.[0]||'Passt als anderer Moment zu eurem bestätigten Tagesplan.');return`<button class="lvt-intelligence-idea" type="button" data-today-suggestion-add data-suggestion-query="${fallbackEsc(query)}" data-suggestion-date="${fallbackEsc(result.input?.targetDate||'')}" data-suggestion-start="${fallbackEsc(result.input?.startAt||'')}" title="${fallbackEsc(query)}"><small>Vorschlag ${String(index+1).padStart(2,'0')}</small><strong>${fallbackEsc(query)}</strong><span>${fallbackEsc(reason)}</span><em>Alle drei vergleichen ${todayIcon('arrow')}</em></button>`}).join('');slider.classList.toggle('has-multiple',choices.length>1)}catch(error){if(status)status.textContent='Vorschläge laden, sobald Places erreichbar ist';slider.innerHTML=`<button class="lvt-intelligence-idea" type="button" data-view="timeline"><small>Nichts wurde verändert</small><strong>Timeline bleibt vollständig nutzbar.</strong><span>${fallbackEsc(error?.message||'Luvia kann gerade keine belegbaren Orte laden.')}</span><em>Timeline öffnen ${todayIcon('arrow')}</em></button>`}};
