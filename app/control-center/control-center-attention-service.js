...
let refreshEpoch=0,sessionExitPending=false;
const authPort=()=>window.LuviaPlatformPorts?.get?.('AuthSessionPort')||window.LuviaIdentityPlatformWebPorts?.AuthSessionPort||null;
const sessionReady=()=>{const auth=authPort()?.snapshot?.();return Boolean(!sessionExitPending&&auth?.authenticated&&!auth?.loading)};
...
async function refresh(){
 ...
 if(!sessionReady()){state.items=[];state.loading=false;state.error=null;state.updatedAt=new Date().toISOString();emit();return snapshot();}
...
function resetForSessionExit(){sessionExitPending=true;refreshEpoch+=1;state.items=[];state.loading=false;state.error=null;state.updatedAt=new Date().toISOString();return emit()}
function refreshWhenReady(){if(sessionReady())refresh().catch(()=>{})}
function runtimeAction(event){const type=event.detail?.type;if(type==='session.deactivate')resetForSessionExit();else if(type==='session.activate'||type==='session.switch')sessionExitPending=false;}
window.addEventListener('luvia:logout',resetForSessionExit);window.addEventListener('luvia:runtime-action',runtimeAction);window.addEventListener('luvia:booking-changed',refreshWhenReady);window.addEventListener('luvia:control-center-travel-identity-changed',refreshWhenReady);
