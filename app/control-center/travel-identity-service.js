...
function snapshot(){
 const store=window.LuviaTripStore?.snapshot?.()||{trips:[],activeTrip:null};
 const context=window.LuviaTripContext?.getSnapshot?.()||{};
 const travel=window.LuviaTravelContext?.snapshot?.()||{};
 const active=summarizeTrip(context.trip||store.activeTrip);
 const now=new Date();
 const upcoming=(store.trips||[]).filter(t=>clean(t.id||t.tripId)!==active?.id).map(summarizeTrip).filter(Boolean).filter(t=>{const d=dateValue(t.startDate);return d&&d>=new Date(now.getFullYear(),now.getMonth(),now.getDate())}).sort((a,b)=>dateValue(a.startDate)-dateValue(b.startDate))[0]||null;
...
window.addEventListener('luvia:trips-changed',emit);window.addEventListener('luvia:travel-context-changed',emit);
