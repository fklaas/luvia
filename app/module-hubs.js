(() => {
  ...
  const COMPASS_CONTEXTS=Object.freeze({
    today:Object.freeze({title:'Was braucht euer Tag gerade?',description:'Luvia verbindet das Naheliegende, ohne euch in Menüs festzuhalten.',features:Object.freeze([
      feature('Entdecken','Places · gerankte Orte','search','places'),feature('Tagesbogen','Journey · jetzt und als Nächstes','route','timeline'),feature('Buchungen','Booking · Status und Antworten','ticket','bookings'),feature('Hinweise','Wetter und Konflikte','bell','attention','foundation'),feature('Wallet','Belege · offline','wallet','wallet','reserved'),feature('Erinnern','Memory Cards und Stories','memory','memories'),feature('Gemeinsam','Rollen und Entscheidungen','users','participants','reserved'),feature('Luvia fragen','Intelligence · planen und handeln','spark','assistant','foundation')
    ...
    plan:Object.freeze({title:'Welche Richtung soll die Planung nehmen?',description:'Alle Planen-Funktionen bleiben auffindbar; nur die passende Richtung steht im Vordergrund.',features:Object.freeze([
      feature('Places','Suchen, Kategorien, Evidenz','search','places'),feature('Meine Orte','Gespeichert, geplant, besucht','heart','places-lifecycle','foundation'),feature('Timeline','Zum Tagesbogen hinzufügen','calendar','timeline'),feature('Booking','Anfragen, Antworten, Belege','ticket','bookings'),feature('Checklisten','Vorbereitung und unterwegs','check','checklists','reserved'),feature('Budget','Gemeinsame Reiseausgaben','wallet','budget','reserved'),feature('Routen','Etappen und externe Navigation','route','routes','foundation'),feature('Wetter','Kontext statt separater Kachel','cloud','weather','foundation')
    ...
    trip:Object.freeze({title:'Wo möchtet ihr eure Reise öffnen?',description:'Tagesdaten, Menschen und Owner-Cores werden nur im passenden Moment sichtbar.',features:Object.freeze([
      feature('Tagesübersicht','Heute und als Nächstes','sun','today'),feature('Timeline','Alle Tage und Momente','calendar','timeline'),feature('Teilnehmer','Rollen, Votes, Einladungen','users','participants','reserved'),feature('Besuchte Orte','Places-Lifecycle','pin','places-lifecycle','foundation'),feature('Live-Momente','Foto, Ton oder Notiz','camera','gallery','foundation'),feature('Reisedaten','Ziel, Zeitraum, Reisefarbe','map','trip-settings'),feature('Wallet','Dokumente und Belege offline','wallet','wallet','reserved'),feature('Booking','Status und Antworten','ticket','bookings')
    ...
    memories:Object.freeze({title:'Wie soll diese Reise weiterleben?',description:'Media bleibt Asset-Owner; Memory macht daraus bewusst ausgewählte Geschichten.',features:Object.freeze([
      feature('Story Studio','Kapitel, Beiträge, Publish','memory','memories'),feature('Memory Cards','Vorderseite, Rückseite, Ton','camera','memories'),feature('Fotoalben','Cover, Auswahl, Beiträge','heart','albums'),feature('Mediathek','Fotos, Videos, Upload Queue','camera','gallery'),feature('Moment bewahren','Foto, Ton, Video, Notiz','spark','gallery','foundation'),feature('Erinnerungskarte','Orte und Day Keys','map','memory-map','foundation'),feature('Mitwirkende','Gemeinsam, privat, consentiert','users','participants','reserved'),feature('Reviews','Erlebt, verifiziert, Reputation','star','reviews','reserved')
  ...
  function compassStage(contextKey='plan',trip){const context=COMPASS_CONTEXTS[contextKey]||COMPASS_CONTEXTS.today,stats=tripStats(trip);return `<section class="lv-plan-compass-stage" data-plan-compass-stage data-living-compass-stage data-compass-context="${esc(contextKey)}" aria-labelledby="lv-plan-compass-title">
    ...
    <div class="lv-plan-constellation">
      ...
      <nav class="lv-plan-directions" aria-label="Funktionen in ${esc(contextKey)}">${context.features.map((item,index)=>{const angle=-90+index*(360/context.features.length);return `<button type="button" class="lv-plan-direction" data-hub-action="${esc(item.action)}" data-compass-label="${esc(item.title)}" data-maturity="${esc(item.maturity)}" data-plan-angle="${angle}" style="--direction-angle:${angle}deg;--direction-counter:${-angle}deg;--direction-index:${index}" aria-label="${esc(item.title)}: ${esc(item.description)} · ${esc(item.maturity)}"><i>${planIcon(item.icon)}</i><span><strong>${esc(item.title)}</strong><small>${esc(item.description)}</small></span></button>`}).join('')}</nav>
    </div>
