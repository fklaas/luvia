...
async function recommend(options={}){
  ...
  const providerRequest=async(query,strictDestination)=>{
    ...
    if(options.fastPath===true&&cached&&Date.now()-cached.loadedAt<PROVIDER_CACHE_MS){
      const places=cached.places.filter(place=>!rejected.has(providerId(place))).map(place=>({...place,providerObservedAt:place.providerObservedAt||new Date(cached.loadedAt).toISOString(),providerFactsCached:true}));
    ...
    try{
      ...
      const places=rawPlaces.filter(place=>!rejected.has(providerId(place)));
