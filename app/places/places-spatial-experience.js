(() => {
  ...
  const state={
    ...
    requestToken:0,lifecycleToken:0,renderToken:0,networkUnsubscribe:null,planningHandle:null,lastSearchAt:null
  };
  ...
  function reason(place,index){
    const supplied=(place.aiReasons||place._luviaReasons||[]).map(clean).find(Boolean);
  ...
  function render(){
    ...
    bind();
    requestAnimationFrame(()=>{if(renderToken===state.renderToken)mountMap(view)});
  }
  ...
  function markerButton(marker){
    ...
    button.addEventListener('click',()=>select(marker.providerPlaceId,true));
    return button;
  }
  function mountMap(view){
    ...
    try{
      ...
      map.on('load',()=>{
        ...
        container.closest('.lv-places-spatial__map')?.classList.add('is-ready');
        setMapMessage(`${view.markers.length} koordinatenverifizierte Orte auf der Karte.`,`ready`);
        setTimeout(()=>map.resize(),100);
      });
      map.on('error',event=>{
        if(!map.loaded()||event?.error)setMapMessage('Kartendaten konnten nicht vollständig geladen werden. Die Ergebnisliste bleibt verfügbar.','unavailable');
      });
    }catch{
      setMapMessage('Die Karte konnte nicht aufgebaut werden. Die Ergebnisliste bleibt verfügbar.','unavailable');
    }
  }
  function destroyMap(){
  ...
  function bind(){
    ...
    root.querySelector('[data-places-back-plan]')?.addEventListener('click',()=>globalThis.LuviaApp?.show?.('plan'));
  ...
  async function mount(root,trip){
    ...
    state.root=root;state.trip=trip;state.visibleLimit=INITIAL_VISIBLE_RESULTS;state.status='loading';state.error=null;state.filters={openNow:false,rated:false};state.filterOpen=false;state.details.clear();state.images.clear();state.saved.clear();
    const contract=placesContract();
