...
(() => {
  ...
  function create({context,select,open,notify}){
    ...
    async function candidates(category){
      const c=ctx(),preference=globalThis.LuviaTripPreferenceContextV1?.snapshot?.()||{},profilePreferences=preference.profilePreferences||{},tripComposition=c.trip?.composition||preference.tripComposition||{},key=JSON.stringify([c.trip?.id,c.geography,category,profilePreferences,tripComposition]);if(cache.has(key))return cache.get(key);
      const request=places().recommend({trip:c.trip,destination:c.geography,destinationContext:c.geography,strictDestination:true,maxDistanceMeters:c.geography.searchRadiusMeters||3000,category,query:category==='culture'?'Kultur':category==='nature'?'Natur':'Restaurant',userQuery:'',subjectText:'',providers:['auto'],fastPath:true,fastQueryLimit:1,parallelFastQueries:false,limit:30,candidateLimit:30,profilePreferences,profileContext:preference,tripComposition,requirePreferenceEvidence:true}).then(result=>result?.places||result?.data?.places||[]).catch(error=>{cache.delete(key);throw error});cache.set(key,request);return request;
    }
  ...
  globalThis.LuviaTripMapExperiences=Object.freeze({version:'1.1.0',create,freeWindow,fitWindow,groupMatches,samePlannedPlace,personReason,instant,localDate,point});
})();
