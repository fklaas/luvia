...
## M16.5 Block 1 Booking Time and Place-Route Integrity — Local Integration Candidate 13.82.143
...
- Local evidence: **the real AI Runtime, Action Contract, Ledger, Booking Lifecycle Policy, public Owner boundary and shared consumer projection pass the visible 390 px five-action lifecycle fixture. The separate visible Place/Booking integrity fixture is 4/4 green, and focused source/unit tests preserve both the concrete cross-venue negative cases and positive exact OpenTable/TheFork/ticket/Hotel provider-property cases. The Modify fixture still reaches 5/5 and visibly shows `Neues Datum: 15.06.2027` plus `Neue Uhrzeit: 19:30 Uhr` for the public `.142` model shape**.
...
- Release state: **focused tests plus the visible local lifecycle 5/5 and Place/Booking integrity 4/4 runs are green. The `booking-route-resolve` Edge Function 2.7 Candidate is local only and has not been deployed. Full Safe Regression, runtime commit, immutable Worker version, Stable/Immutable byte comparison, signed-in public Acceptance/Step-17 result and exact `.143 → .139` rollback remain pending; `.143` is not accepted before those gates pass**.
- Scope and fixed continuation: **no DB, RLS, Provider credential, Secret, Main or Production change is claimed. First publish and publicly accept `.143`, including the Edge 2.7 route resolver; then prove real restaurant and activity provider routes; then build the bright Demo-design Hotel surface over the existing Booking Owner; only then activate Hotel/Affiliate providers; afterward close P09/P10, the remaining Step-17 rows and Blocks 2–5 in order**.
...
- Action matrix: **the four proven rows `places.favorite`, `places.unfavorite`, `places.plan` and `places.unplan` are now `PUBLIC_E2E_PASS`; the canonical inventory remains 329 actions, 23 typed runtime actions, 124 protected writes, 898 source markers and 2,725 generated failure evals. Public-E2E rows rise truthfully from 3 to 7**.
...
- Remaining B1 boundary: **the current fixed order is `.143` public Place/Booking-integrity Acceptance; real Restaurant/Activity provider routes; the bright Demo-design Hotel surface over the existing Booking Owner; Hotel/Affiliate activation; P09/P10; remaining Step-17 rows; then Blocks 2–5**.
