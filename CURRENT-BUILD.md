...
## M16.5 Map Navigation + Booking Submit Boundary — Local Integration candidate 13.82.154

- Compass-map correction: **both productive marker paths now rotate through twelve tones from the complete Luvia Compass spectrum instead of collapsing to one trip colour. The active `Passend` mode is reapplied to every new viewport response before its atomic marker replacement, so its pressed state and the actually visible evidence-backed pins remain aligned**.
...
- Current gate: **focused Place-detail, unified Journey sheet, explicit external-handoff, Booking route, lifecycle and admission tests pass. Fresh `.154` bundles are built and the complete controlled Safe Regression is 201/201 PASS including NFR-0 3/3; clean release commit, Integration deployment, operated exact Restaurant route, mobile layout proof, byte proof and exact rollback remain required before acceptance**.
