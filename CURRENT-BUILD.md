...
- Runtime target: **App 13.82.79 / Core 4.82.79 / Integration Preview only**.
...
- Cache-upgrade correction: **Service Worker precaching retains the complete shell list but runs through a strict four-request worker pool and one shared opened cache handle. A controlled client whose active worker does not match the document build starts the bounded update from the early PWA runtime, using a build-qualified worker URL; only the exact expected waiting build is activated. Because that browser is already parsing the current-build document and current-build `?v=` assets, the automatic controller handoff preserves this document instead of forcing a second 250-resource reload. Explicit/manual worker activation keeps the guarded one-reload policy. Fresh uncontrolled clients still defer registration until page load/idle**.
- Local visible evidence: **real cursor clicks at 1440 x 900 and 390 x 844 opened the closed Compass, the three primary directions, all six Luvia worlds and the final “Ein ruhiger Anfang” canvas with the complete new short product introduction. Canvas Back and Browser Back return coherently; reload replays the intro and restores a closed Compass under the active `luvia-shell-v13.82.79`; mobile horizontal overflow is absent. Keyboard Enter/Space, Reduced Motion, theme selection and the other five canvases were also visibly exercised. Hardware `Input.dispatchTouchEvent` is unavailable in the integrated browser, so no new physical-device touch claim is made for this public-Landing slice**.
- Controlled stale-client evidence: **on the same 390 x 844 origin, a live `.78` controller was upgraded by the `.79` document without a second document reload. The current document reached `load` in 9.257 s and the complete closed-Compass state plus active `.79` controller/no waiting worker in 11.082 s; 2.2 s later only `luvia-shell-v13.82.79` remained. A subsequent real cursor click opened all three primary choices with no horizontal overflow**.
...
- Rejected stale-controller recovery evidence: **App 13.82.74 bounded the install pool and passed its immutable E2E, byte parity and controlled `.72 -> .74` upgrade fixture. On the real Stable origin, however, a browser still controlled by the rejected `.73` worker remained at `readyState: loading` for more than 25 seconds because `.74` deferred registration until the page load that the stale controller prevented. Stable was immediately rolled back to `.72`; `.74` receives no functional/public acceptance. `.79` adds the early, build-qualified controlled-client recovery path, preserves the already-current document during automatic controller handoff, shares one opened shell-cache handle across parallel fetches and removes any stale cache recreated by late old-worker writes after load**.
...
- Main / Production: **locked and verified unchanged before publication: Main `c4b6d1740ad04c291d5e27d8d18b3a32e5ed87ba`; Production deployment `578f13fc-8193-4988-88cf-93c94362fcc3`, version `0d26706b-8b79-4e05-b3b6-6c6314cc597c`; no database/schema/RPC/RLS/bucket migration, Edge Function, secret or manual Cloudflare configuration change**.
- Candidate rollback: **redeploy the immediately previous accepted Integration App 13.82.72 version `9bdfd528-d5b2-4773-8a0f-949cc6e37fe0` from deployment `852438a1-1bc8-45f5-9875-f26e59db16c9`; code/assets only, no data rollback**.
