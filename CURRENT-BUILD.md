...
## M16.5 Owner-first Intelligence USP Slices — Integration Candidate 13.82.126
...
- Candidate state: **local implementation and targeted Evals are present. `.125` exposed a mixed-build Stable client because `luvia-runtime-loader.mjs` fell through the Service Worker's generic cache-first branch; `.126` adds `.mjs` to the version-aware strategy and reloads controlled upgrades under the new controller. Full Safe Regression, public Step-17 CUA, commit, immutable deployment and byte provenance are not claimed by this paragraph and must be recorded before release**.
