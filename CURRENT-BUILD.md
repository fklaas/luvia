...
- Runtime target: **App 13.82.80 / Core 4.82.80 / Integration Preview only**.
- Status: **Compass-first public journey candidate complete; the corrected toxic-worker recovery, immutable publication and exact deployed-byte E2E are in verification. This is not the complete M16.5 Product-Surface or Design-Freeze acceptance**.
...
- Cache-upgrade correction: **the tiny build-qualified recovery runtime now executes in the document head before the first blocking stylesheet, so a controlled client can replace a stale worker while the current document is still parsing. Worker installation fetches only a deliberately small critical visible shell through the bounded four-request pool; the complete application shell warms explicitly only after visible load and idle. The automatic expected-worker handoff preserves the already-current document, cleans stale caches again after late old-worker writes and tolerates browsers that block session-storage access. Explicit/manual activation retains the guarded one-reload policy**.
