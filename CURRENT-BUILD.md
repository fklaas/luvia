# Cuisine filters and shared map pins — candidate 13.82.168.30

Explicit cuisine union before provider pagination; shared map pointer handling; unified pin silhouette, stacking and profile-match outline. Acceptance pending.
...
## M16.5 Preview Morph + Hotel Map Parity — Public Integration checkpoint 13.82.167 / user acceptance open
...
- Release evidence: **fresh `.167` runtime bundles, generated visual/action inventories, the 330-row parity matrix, NFR-0 3/3 and the complete controlled Safe Regression are 201/201 PASS. Local and public 390 × 844 real-pointer proof covers Places and Hotels: one Pin stages one exact image preview, preview tap morphs open, and upward drag settles the same provider entity. Commit `568d6e4c0796f87d3af377ce9e480386aea6892f` is deployed exclusively to Integration as Worker version `b2ab956a-729f-402a-8d3d-56778614d61a` at 100 %. Ten release-critical public assets are byte-identical to the clean Git archive**.
- Acceptance boundary: **`.167` is a publicly operated engineering checkpoint, not the completed map design and not a user-accepted Design Freeze. The user explicitly reports that the map and compact mini-info preview still need refinement and that the upward finger interaction must feel smoother and more physically continuous. The next slice remains inside this same Places/Hotels map system; no B2 package or unrelated feature starts before this interaction is visibly accepted**.
- Production truth: **Production remains App/Core `13.82.49 / 4.82.49` on Worker version `cc7d58fc-a5f8-4d5c-a63f-95782e34eabe` at 100 %. Main, Production, DB/RLS, Edge functions, provider credentials and user data were not changed by `.167`**.
