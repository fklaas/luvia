...
- Runtime target: **App 13.82.72 / Core 4.82.72 / Integration Preview only**.
- Status: **cohesive Landing/Auth foundation published on Integration and bounded visible public E2E PASS; this is not the complete M16.5 Product-Surface or Design-Freeze acceptance**.
...
- Journey-first entry: **the public first screen is now a bright, softly masked coastal memory surface instead of a form-first or dark full-bleed login. All primary start actions lead to a dedicated large Living Compass threshold; a deliberate click opens the two signposted paths “Weiterreisen” and “Meine Reise beginnen”, which then hand off to the single productive login/register owner**.
...
- Local visible evidence: **real left-click desktop and 390 x 844 responsive-browser flows from the bright hero through the large Compass opening and both signposts into minimal registration/login PASS; the deployable iframe renders the productive signed-in shell without bad script paths. Login/Register switching, Escape, Browser Back, reload, invalid and fixture-backed valid recovery states, Reduced Motion and a clean console also PASS. Hardware `Input.dispatchTouchEvent` is unavailable in the integrated browser, so no physical-device touch claim is made for this slice**.
...
- Public visible evidence: **on the exact App 13.82.72 immutable bytes, real left clicks opened the large Compass and the “Meine Reise beginnen” signpost on desktop and 390 x 844 responsive mobile. Both signposts remained sharp, ARIA expanded/visibility state was coherent, the mobile document had no horizontal overflow, the three-field registration and Compass chapter appeared, and console warnings/errors remained 0. The stable origin served the same `.72` assets; an existing authenticated stable-origin session correctly restored the signed-in product instead of replaying the public Landing**.
- First clean `.72` runtime publication: **Integration version `9bdfd528-d5b2-4773-8a0f-949cc6e37fe0`, deployment `852438a1-1bc8-45f5-9875-f26e59db16c9`; stable `https://integration-luvia.njwnrvwbv5.workers.dev/`; immutable `https://9bdfd528-integration-luvia.njwnrvwbv5.workers.dev/`; 16 / 16 release-critical source/stable/immutable assets byte-identical**.
...
- Explicitly still open: **persisted Compass-led Profile/preferences onboarding, first-trip onboarding, the remaining M16.5 Product Surface Matrix, complete one-to-one visual parity and joint Design Freeze. No complete M16.5 functional/design acceptance is granted by this bounded Landing/Auth release**.
...
- Main / Production: **locked and verified unchanged: Main `c4b6d1740ad04c291d5e27d8d18b3a32e5ed87ba`; Production deployment `578f13fc-8193-4988-88cf-93c94362fcc3`, version `0d26706b-8b79-4e05-b3b6-6c6314cc597c`; no database/schema/RPC/RLS/bucket migration, Edge Function, secret or manual Cloudflare configuration change**.
- Rollback: **redeploy Integration App 13.82.65 version `672b3a94-e25d-47bc-97d3-baf903d1c971` from deployment `30f7b880-e7b4-4bd2-874d-a0b834ac75b8`; code/assets only, no data rollback**.
