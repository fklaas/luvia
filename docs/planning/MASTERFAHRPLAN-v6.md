...
App .86 verwendet für den positiven Profil-Ernährungsnachweis dieselbe streng belegte Kaskade aus HERE, begrenztem Google und Foursquare. HERE liefert in 8/15 km um Scharbeutz 0 explizite vegetarische Orte; Foursquare endet mit 429 ohne Credits. Google Places API (New) ist laut Nutzerbestätigung bereits aktiv. Das unveränderte GOOGLE_PLACES_API_KEY-Secret ist in Supabase vorhanden und wird vom Gateway gelesen. Ein zunächst gespeicherter 403-Cooldown wurde kontrolliert zurückgesetzt; der danach zugelassene echte Aufruf an places.googleapis.com endete erneut mit HTTP 403, providerStatus PERMISSION_DENIED und Dienst places.googleapis.com. Damit sind fehlendes Secret, internes Budget und Legacy-Endpoint ausgeschlossen. Übrig bleiben API-/Anwendungseinschränkung des bestehenden Keys, Key-Projekt-Zuordnung und Billing dieses Projekts. Alle zeigt 50 reale Essen-&-Trinken-Pins, Passend bleibt beim vegetarischen Profil ehrlich 0/0 und ein unbelegtes Steakhaus ausgeschlossen. Das interne Google-Budget bleibt 4/min, 25/Tag und 800/Monat bei einem laut Nutzerangabe verfügbaren Google-Tageskontingent von 1.000. Der erneute vollständige Safe-Regression-Lauf nach dem Diagnose-Slice ist mit 227/227 grün.

**Nächster Abschlussnachweis:** Im Google-Cloud-Projekt des bestehenden Supabase-Schlüssels die API-Beschränkung auf Places API (New), die Anwendungseinschränkung für serverseitige Supabase-Aufrufe, die exakte Projektzugehörigkeit und aktives Billing prüfen. Danach den öffentlichen Health-Probe ohne PERMISSION_DENIED sowie mindestens einen realen vegetarischen Scharbeutz-Treffer mit identischer Provider-ID und verständlichem Grund in Places, Timeline und AI Chat sichtbar abnehmen.
...
Karte, Timeline-Vorschläge und AI Chat konsumieren denselben places.v1-Filtervertrag und denselben providerbelegten Alle-/Passend-Bestand. Separat gefundene Ernährungsbelege werden nur über identische Providerreferenz oder exakt gleichen normalisierten Namen innerhalb 25 Metern verbunden. Der negative Steakhouse-Fall und positive Google-, HERE- und Foursquare-Fixtures sind automatisiert grün. Auf öffentlicher Integration liefert Alle 50 Essen-&-Trinken-Pins und Passend für das vegetarische Profil ehrlich 0/0. Der neue öffentliche Google-Realprobe belegt bei vorhandenem Supabase-Secret und freiem internem Budget weiterhin HTTP 403 / PERMISSION_DENIED für places.googleapis.com.

**Nächster Abschlussnachweis:** Nach Korrektur der bestehenden Google-Key-Beschränkung beziehungsweise Key-Projekt-/Billing-Zuordnung denselben realen positiven Ernährungsort in Places, Timeline-Vorschlag und AI Chat mit gleicher Provider-ID, gleichem Reisezielradius und verständlichem Passend-Grund sichtbar belegen.
