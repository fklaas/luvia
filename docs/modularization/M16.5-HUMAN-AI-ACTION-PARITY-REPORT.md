# M16.5 Human ↔ AI Action Parity Control Plane

Date: 2026-09-03

Status: **B0.01–B0.10 PUBLIC COMPLETE ON 13.82.135 / B1 FOUNDATION PUBLIC ON 13.82.136 + TRIP READ ON 13.82.138 + PLACES MUTATIONS ON 13.82.139 + BOOKING LIFECYCLE/PLACE-ROUTE INTEGRITY ON 13.82.143 / RECOVERY BASELINE ON 13.82.147 / 13.82.148–149 REJECTED / PUBLIC MAP/MINI-INFO CHECKPOINT ON 13.82.168, USER ACCEPTANCE OPEN / PRODUCT PARITY CONTINUES THROUGH B1–B5**

Source: `Luvia-vollstaendiges-Benutzeraktionsinventar-2026-09-01.xlsx` · SHA-256 `42E4B9D2115EE6CF38E3B7E9EDA1148AAACCB63CF14C8780C9EDA8A67CE48E46` · Integration snapshot `13.82.168`.

## Plain-language position

The complete reviewed inventory is now a machine-readable release control plane. It records what a person can do, which Owner must perform it, whether the AI can reach the same path and exactly which contract work remains open. It does not make missing capabilities available by declaration.

## Binding counts

| Measure | Count |
| --- | ---: |
| Semantic user actions | 330 |
| Available/conditional product actions | 319 |
| Public E2E pass | 7 |
| Registered/partial | 64 |
| Missing AI parity | 223 |
| Native chat controls | 14 |
| Presentation-only / not required | 10 |
| Demo / not applicable | 11 |
| Unavailable or reserved outcomes | 24 |
| Audited active data-* markers | 1004 |
| Existing runtime action IDs | 27 |

## Category ledger

| Category | Total | Public pass | Registered/partial | Missing | Native/N/A |
| --- | ---: | ---: | ---: | ---: | ---: |
| Buchung & Anbieterkommunikation | 40 | 0 | 12 | 28 | 0 |
| Erinnerungen & Geschichten | 35 | 0 | 2 | 33 | 0 |
| Gerät, System & Offline | 10 | 0 | 0 | 10 | 0 |
| Journey, Timeline & Tagesplan | 30 | 1 | 6 | 23 | 0 |
| Luvia AI Chat | 16 | 0 | 0 | 0 | 16 |
| Medien & Galerie | 22 | 0 | 1 | 21 | 0 |
| Navigation & Oberfläche | 30 | 0 | 19 | 4 | 6 |
| Öffentliche Landing-Demos | 11 | 0 | 0 | 0 | 11 |
| Places & Ortsentdeckung | 38 | 4 | 3 | 29 | 2 |
| Profil, Vorlieben & Privatsphäre | 34 | 0 | 9 | 25 | 0 |
| Reisen & Zusammenarbeit | 38 | 2 | 6 | 30 | 0 |
| Veranstaltungen & Kalender | 8 | 0 | 6 | 2 | 0 |
| Zugang, Konto & Sicherheit | 18 | 0 | 0 | 18 | 0 |

## Honest contract gaps

- Owner method audit open: **0** actions.
- Public contract binding proven locally, AI route still open: **246** action rows.
- Runtime owner binding present but public proof open: **38** action rows.
- Public owner path proven: **7** action rows.
- Typed action input contract open: **218** actions.
- Typed metadata contract ready for the existing 27 runtime actions: **73** mapped human-action rows.
- Bounded runtime enforcement is active for **27/27** runtime actions. Navigation, Places, Events, Booking, Journey, Trip, Memory and Identity reject missing or contradictory input before Ledger creation or Owner invocation. Navigation is restricted to registered routes; writes retain visible confirmation and receipts; Booking R3 commands retain unknown-outcome reconciliation. Raw prompts, Story text and concrete preference values are omitted from the Action Ledger where only bounded references are required.

## Generated parity and failure evidence

- Matrix rows: **330/330** semantic actions after deterministic regeneration.
- Required dimensions per row: **12** — contract, compiler, permission,
  confirmation, idempotency, receipt, recovery, Undo, multilingual, typo,
  multi-intent and denial.
- Generated explicit failure evals: **2733**.
- Current evidence states: **7** public E2E proofs, **81** local AI paths,
  **217** truthful manual Owner paths, **14** blocked rows and **11**
  non-product rows.
- CI drift protection hashes the six source contracts and compares the checked-in
  matrix byte-for-byte with a fresh deterministic generation. Changed or new
  actions, missing confirmation/idempotency evidence and unsupported public-pass
  claims fail the release suite.

## Release gate

A new or changed active `data-*` marker makes the registry test fail until the source audit and semantic-action decision are deliberately revised. Every state-changing row requires a separate explicit confirmation and idempotency decision. A row can become `PUBLIC_E2E_PASS` only when the existing runtime action, public Owner contract/method and visible Integration evidence agree.

## Immediate continuation

1. Owner-method inventory is locally complete; bind the remaining human actions into typed AI routes and visible confirmations.
2. Keep all 27 typed runtime actions fail-closed while public AI routes are completed domain by domain.
3. Keep the generated 330-row, 12-dimension failure-eval matrix green while action routes are completed domain by domain.
4. Keep the visible local register, consumer-chat and parity/failure gates green from their canonical files.
5. Preserve the publicly closed B0.10 control-plane release and continue product parity row by row through B1–B5. App `13.82.135` proved one exact Plan/Undo path, accepted App `13.82.138` proved the Chat-native Trip-selection read, accepted App `13.82.139` publicly proved the complete Favorite/Unfavorite/Plan/Unplan confirmation, readback, receipt and Undo protocol, and accepted App `13.82.143` proved the Booking lifecycle plus exact Place-route gate. App `13.82.147` retains the accepted recovery baseline; Apps `13.82.148` and `13.82.149` are rejected counterevidence. App `13.82.168` is publicly deployed and browser-operated as the uninterrupted-map/preview-morph checkpoint, but remains below surface acceptance until the user accepts the mini-info design and physical drag smoothness. Provider-positive evidence remains open.

## B1 registry delta retained after visible/public release evidence

The universal Booking candidate replaces the restaurant-specific runtime
registration with canonical `booking.place.open`, preserves the compatibility
alias and adds bounded `navigation.route.open`, `booking.stay.search` and exact `booking.stay.offer.open` runtime actions. All 20
human navigation outcomes, including the new Hotels area, now map to that
single allow-listed Owner command; Hotel search reaches the public Booking Owner and fails closed without live provider evidence. A selected offer opens only when Provider-, Property-, Offer-, Rate-, Quote- and URL identity agree. The source runtime action set is **24**.
The deterministic registry and input-contract artifact are regenerated and
locally green; the 330-row parity/failure matrix remains a release gate for every following domain slice. Accepted App `13.82.139` promotes exactly four publicly operated Places mutation rows for seven total public passes. The rejected `.148` and `.149` candidates and local `.150` candidate do not inflate public evidence before an operated public run.
