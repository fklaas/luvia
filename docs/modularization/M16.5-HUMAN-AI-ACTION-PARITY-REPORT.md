...
Source: `Luvia-vollstaendiges-Benutzeraktionsinventar-2026-09-01.xlsx` · SHA-256 `42E4B9D2115EE6CF38E3B7E9EDA1148AAACCB63CF14C8780C9EDA8A67CE48E46` · Integration snapshot `13.82.168`.
...
| Measure | Count |
| --- | ---: |
| Semantic user actions | 333 |
| Available/conditional product actions | 322 |
| Public E2E pass | 7 |
| Registered/partial | 67 |
| Missing AI parity | 223 |
| Native chat controls | 14 |
| Presentation-only / not required | 10 |
| Demo / not applicable | 11 |
| Unavailable or reserved outcomes | 24 |
| Audited active data-* markers | 1014 |
| Existing runtime action IDs | 30 |

## Category ledger

| Category | Total | Public pass | Registered/partial | Missing | Native/N/A |
| --- | ---: | ---: | ---: | ---: | ---: |
| Buchung & Anbieterkommunikation | 40 | 0 | 12 | 28 | 0 |
| Erinnerungen & Geschichten | 35 | 0 | 2 | 33 | 0 |
| Gerät, System & Offline | 10 | 0 | 0 | 10 | 0 |
| Journey, Timeline & Tagesplan | 33 | 1 | 9 | 23 | 0 |
...
| Places & Ortsentdeckung | 38 | 4 | 3 | 29 | 2 |
...
- Public contract binding proven locally, AI route still open: **249** action rows.
- Runtime owner binding present but public proof open: **38** action rows.
- Public owner path proven: **7** action rows.
- Typed action input contract open: **218** actions.
- Typed metadata contract ready for the existing 30 runtime actions: **76** mapped human-action rows.
- Bounded runtime enforcement is active for **30/30** runtime actions. Navigation, Places, Events, Booking, Journey, Trip, Memory and Identity reject missing or contradictory input before Ledger creation or Owner invocation. Navigation is restricted to registered routes; writes retain visible confirmation and receipts; Booking R3 commands retain unknown-outcome reconciliation. Raw prompts, Story text and concrete preference values are omitted from the Action Ledger where only bounded references are required.

## Generated parity and failure evidence

- Matrix rows: **333/333** semantic actions after deterministic regeneration.
...
- Generated explicit failure evals: **2764**.
- Current evidence states: **7** public E2E proofs, **84** local AI paths,
  **217** truthful manual Owner paths, **14** blocked rows and **11**
  non-product rows.
...
2. Keep all 30 typed runtime actions fail-closed while public AI routes are completed domain by domain.
3. Keep the generated 333-row, 12-dimension failure-eval matrix green while action routes are completed domain by domain.
