...
`core/collaboration/*` is reserved for `feature/collaboration-core`, while the current presence/activity service remains compatibility until M18.1 proves Membership Truth. `core/attention/*`, `core/travel-wallet/*`, `core/reviews/*` and `core/admin/*` are reserved for their matching owner streams and must not be created as active runtime roots without an approved Core slice.
