...
## Release `.104`: vollständige Kategorie-/Küchenmatrix und ausgewählte exakte Medien

App **13.82.168.104** / Core **4.82.223** veröffentlicht Runtime-Commit **5e8f37985eb5ae76c0247ecfb11410bcc107c677** als Worker **7cda6e20-3b33-442a-b5e6-1c2da8256faf**. Gateway **v233** meldet Software **4.64.36** und Places **4.38.15-exact-google-media-category-matrix**. Die Migration **20260906043000** trennt Google-Suche und Place Photo in atomare Budgets von jeweils 25 pro Tag, 800 pro Monat und 4 pro Minute. Fotos werden ausschließlich für den ausgewählten Ort angefordert; Ziehen, Zoomen und breite Ergebnislisten lösen keinen Google-Fotoread aus.

Die öffentliche Matrix besteht **33/33** automatische Pfade: 13 Places-Kategorien plus Stays und 19 Landesküchen. 14 Küchen liefern im Scharbeutz-Radius belegte Treffer; Thai, Koreanisch, Mexikanisch, Nahöstlich und Libanesisch bleiben ehrlich leer. Median **1.284 ms**, P95 **3.434 ms**, Maximum **4.099 ms**. Sichtbar bestehen Food **48 Alle / 9 Passend**, Shopping **46 / 11**, der vegetarische Ausschluss von Kleines Steakhouse, der Chinesisch-Treffer **Hay-Cheng**, 46 Shopping-Pins nach Zoom, eine weiterhin bedienbare Karte und der ausgewählte Pin im Vordergrund. **Snykrode** lädt sein exakt verknüpftes Wikimedia-Bild in **1342 x 736** Pixeln.

Google Text Search (New) antwortet weiterhin **403 PERMISSION_DENIED** von **places.googleapis.com**. Damit ist der Key vorhanden und erreicht den Dienst, besitzt im Google-Projekt aber noch nicht die erforderliche serverseitige Berechtigung. Foursquare antwortete zuletzt **429** und wird automatisch 15 Minuten abgekühlt. Diese externen Zustände werden nicht als Bildtreffer ausgegeben; die UI bleibt ehrlich bildlos oder nutzt nur einen exakt verknüpften freien Ortsbildpfad.

Safe Regression **233/233**, NFR-0 **3/3** und öffentliche Byteidentität **30/30** sind grün. Das Archiv **luvia-integration-13.82.168.104-5e8f3798-public-bytes.zip** hat **89.026.357 Bytes**, SHA-256 **713C216C53C71AE4A9D90B1BFEA8CC1679B1B5ABE6B05181646663F4AF34A8D6**. Immutable: **https://7cda6e20-integration-luvia.njwnrvwbv5.workers.dev**. Exakter App-Rollback: `npx wrangler versions deploy 075622ac-defa-4cef-8e20-43684302b7e8@100 --name integration-luvia --message "Rollback App 13.82.168.104 exact media matrix to accepted App 13.82.168.103" --yes`. Main und Production blieben unverändert.
