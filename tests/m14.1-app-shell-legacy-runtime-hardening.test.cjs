...
assert.match(shell,/runtimeAssetUrl=path=>`\$\{path\}\?v=\$\{encodeURIComponent\(window\.LuviaKernelVersion\?\.build\|\|'\d+\.\d+\.\d+(?:\.\d+)?'\)\}`/,'runtime assets must follow the current Kernel build with a semantic release fallback');
