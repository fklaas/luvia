...
for(const asset of [actionCore,ledgerCore,actionRuntime,dashboard]){
  assert.ok(index.includes(`${asset}?v=13.82.168.82`),`index misses M16 runtime asset ${asset}`);
...
assert.match(version,/core:'4\.82\.204',build:'13\.82\.168\.82',name:'M16\.5 Places and Stays Quality'/);
