...
for(const asset of [actionCore,ledgerCore,actionRuntime,dashboard]){
  assert.ok(index.includes(`${asset}?v=13.82.121`),`index misses M16 runtime asset ${asset}`);
...
assert.ok(worker.includes("const CACHE='luvia-shell-v13.82.121'"));
assert.match(version,/core:'4\.82\.121',build:'13\.82\.121',name:'M16\.5 Steps 15–18 Global AI & Owner-first Productization'/);
