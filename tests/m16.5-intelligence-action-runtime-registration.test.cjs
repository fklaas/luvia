...
for(const asset of [actionCore,ledgerCore,actionRuntime,dashboard]){
  assert.ok(index.includes(`${asset}?v=13.82.143`),`index misses M16 runtime asset ${asset}`);
...
assert.ok(worker.includes("const CACHE='luvia-shell-v13.82.143'"));
assert.match(version,/core:'4\.82\.143',build:'13\.82\.143',name:'M16\.5 Block 1 Semantic Places Mutations'/);
