...
for(const asset of [actionCore,ledgerCore,actionRuntime,dashboard]){
  assert.ok(index.includes(`${asset}?v=13.82.168.58`),`index misses M16 runtime asset ${asset}`);
...
assert.match(version,/core:'4\.82\.180',build:'13\.82\.168\.58',name:'M16\.5 Places and Stays Quality'/);
