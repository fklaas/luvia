...
for(const asset of [actionCore,ledgerCore,actionRuntime,dashboard]){
  assert.ok(index.includes(`${asset}?v=13.82.167`),`index misses M16 runtime asset ${asset}`);
...
assert.ok(worker.includes("const CACHE='luvia-shell-v13.82.167'"));
assert.match(version,/core:'4\.82\.167',build:'13\.82\.167',name:'M16\.5 Places Hotel Recovery'/);
