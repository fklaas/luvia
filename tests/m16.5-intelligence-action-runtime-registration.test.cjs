...
for(const asset of [actionCore,ledgerCore,actionRuntime,dashboard]){
  assert.ok(index.includes(`${asset}?v=13.82.73`),`index misses M16 runtime asset ${asset}`);
...
assert.ok(worker.includes("const CACHE='luvia-shell-v13.82.73'"));
assert.match(version,/core:'4\.82\.73',build:'13\.82\.73',name:'M16\.5 Landing Auth Foundation'/);
