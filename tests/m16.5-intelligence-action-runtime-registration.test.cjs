...
for(const asset of [actionCore,ledgerCore,actionRuntime,dashboard]){
  assert.ok(index.includes(`${asset}?v=13.82.58`),`index misses M16 runtime asset ${asset}`);
...
assert.ok(worker.includes("const CACHE='luvia-shell-v13.82.58'"));
assert.match(version,/core:'4\.82\.58',build:'13\.82\.58',name:'M16\.5 Living Compass Recovery'/);
