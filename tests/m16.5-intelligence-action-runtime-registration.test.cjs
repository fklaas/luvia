...
for(const asset of [actionCore,ledgerCore,actionRuntime,dashboard]){
  assert.ok(index.includes(`${asset}?v=13.82.168.20`),`index misses M16 runtime asset ${asset}`);
...
for(const test of [
  'tests/m16.5-block1-booking-submit-owner.test.cjs',
  'tests/m16.5-block1-semantic-booking-lifecycle.test.cjs',
...
assert.match(version,/core:'4\.82\.168',build:'13\.82\.168\.20',name:'M16\.5 Places Category Query Split'/);
