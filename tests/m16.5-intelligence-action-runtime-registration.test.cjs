...
for(const asset of [actionCore,ledgerCore,actionRuntime,dashboard]){
  assert.ok(index.includes(`${asset}?v=13.82.168.173`),`index misses M16 runtime asset ${asset}`);
...
assert.ok(worker.includes("const CACHE='luvia-shell-v13.82.168.173-local-recovery'"));
assert.match(version,/core:'4\.82\.292',build:'13\.82\.168\.173',name:'M16\.5 Places and Stays Quality'/);
