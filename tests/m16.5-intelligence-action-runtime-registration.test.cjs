...
for(const asset of [actionCore,ledgerCore,actionRuntime,dashboard]){
  assert.ok(index.includes(`${asset}?v=13.82.125`),`index misses M16 runtime asset ${asset}`);
...
assert.ok(worker.includes("const CACHE='luvia-shell-v13.82.125'"));
assert.match(version,/core:'4\.82\.125',build:'13\.82\.125',name:'M16\.5 Owner-first Intelligence USP Slices'/);
