...
for(const asset of [actionCore,ledgerCore,actionRuntime,dashboard]){
  assert.ok(index.includes(`${asset}?v=13.82.134`),`index misses M16 runtime asset ${asset}`);
...
assert.ok(worker.includes("const CACHE='luvia-shell-v13.82.134'"));
assert.match(version,/core:'4\.82\.134',build:'13\.82\.134',name:'M16\.5 Block 0 Human-AI Parity Control Plane'/);
