...
for(const asset of [actionCore,ledgerCore,actionRuntime,dashboard]){
  assert.ok(index.includes(`${asset}?v=13.82.61`),`index misses M16 runtime asset ${asset}`);
...
assert.ok(worker.includes("const CACHE='luvia-shell-v13.82.61'"));
assert.match(version,/core:'4\.82\.61',build:'13\.82\.61',name:'M16\.5 Living Compass Recovery'/);
