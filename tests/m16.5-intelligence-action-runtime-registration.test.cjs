...
for(const asset of [actionCore,ledgerCore,actionRuntime,dashboard]){
  assert.ok(index.includes(`${asset}?v=13.82.100`),`index misses M16 runtime asset ${asset}`);
...
assert.ok(worker.includes("const CACHE='luvia-shell-v13.82.100'"));
assert.match(version,/core:'4\.82\.100',build:'13\.82\.100',name:'M16\.5AA Preference Resolution \+ Compass Activation'/);
