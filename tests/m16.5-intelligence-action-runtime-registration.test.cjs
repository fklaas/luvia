...
for(const asset of [actionCore,ledgerCore,actionRuntime,dashboard]){
  assert.ok(index.includes(`${asset}?v=13.82.97`),`index misses M16 runtime asset ${asset}`);
...
assert.ok(worker.includes("const CACHE='luvia-shell-v13.82.97'"));
assert.match(version,/core:'4\.82\.97',build:'13\.82\.97',name:'M16\.5X Shell Freeze Orbit and Control Course'/);
