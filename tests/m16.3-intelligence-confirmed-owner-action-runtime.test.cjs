...
(async()=>{
  ...
  const routeBuffer=runtime.prepare('journey.entry.schedule',{tripId:'trip-1',entryId:'event:photo-1',name:'Strandmoment',startAt:'2026-08-25T10:30:00.000Z',durationMinutes:45,routeBufferMinutes:14,previousRouteBufferMinutes:10,changeKind:'route-buffer',routeMode:'zu Fuß',routeSource:'HERE Routing',routeDataAge:'vor 4 Min. beobachtet',routeLiveStatus:'Live-Daten belegt',expectedRouteEvidenceSignature:'fnv1a-route',expectedRevision:'event-rev-1'},{userGesture:true,idempotencyKey:'journey-route-buffer-once'});
  assert.equal(routeBuffer.result.evidence.preview.changes.routeBufferMinutes,14);assert.equal(routeBuffer.result.evidence.preview.before.routeBufferMinutes,10);assert.equal(routeBuffer.result.evidence.preview.routeEvidence.source,'HERE Routing');
  await runtime.execute('journey.entry.schedule',{}, {ledgerId:routeBuffer.ledgerId,userGesture:true,confirmed:true});const routeBufferCall=calls.filter(call=>call[0]==='journey-entry-schedule').at(-1);assert.equal(routeBufferCall[2].changeKind,'route-buffer');assert.equal(routeBufferCall[2].routeBufferMinutes,14);assert.equal(routeBufferCall[2].expectedRouteEvidenceSignature,'fnv1a-route');assert.equal(routeBufferCall[2].operationId,'journey-route-buffer-once');
