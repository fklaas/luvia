...
assert.equal(manifest.currentBuild,'13.82.141');
assert.equal(manifest.policy.minimumRetainedBuilds,3);
assert.deepEqual(manifest.retainedBuilds.map(item=>item.build),['13.82.121','13.82.122','13.82.123','13.82.124','13.82.125','13.82.126','13.82.127','13.82.128','13.82.129','13.82.130','13.82.135','13.82.136','13.82.137','13.82.138','13.82.139','13.82.140']);
...
assert.match(loader,/luvia-runtime-precontext-13\.82\.141\.bundle\.js/);
assert.match(loader,/luvia-runtime-postcontext-13\.82\.141\.bundle\.js/);
assert.doesNotMatch(loader,/13\.82\.(?:12[1-9]|13[05678]|140)\.bundle\.js/,'the current loader may not execute a compatibility bundle');
assert.match(worker,/luvia-runtime-precontext-13\.82\.141\.bundle\.js/);
assert.doesNotMatch(worker,/APP_SHELL\.push\(scoped\('app\/luvia-runtime-(?:precontext|postcontext)-13\.82\.(?:12[1-9]|13[05678]|140)/,'compatibility bundles must not inflate the current service-worker precache');
...
console.log('Retained split builds: 13.82.121–13.82.130 plus 13.82.135 / 13.82.136 / 13.82.137 / 13.82.138 / 13.82.139 / 13.82.140');
