...
assert.equal(manifest.currentBuild,'13.82.168.103');
...
assert.match(loader,/const MODULE_LOAD_ATTEMPTS = 2;/,'dynamic runtime modules need one bounded recovery attempt');
assert.match(loader,/async function importModule\(relativeUrl\)/,'dynamic imports need a dedicated retry boundary');
assert.match(loader,/await import\(source\.href\)/,'a retried module import must use the distinct attempt URL');
assert.match(loader,/importModule\(`\.\/adapters\/platform-port-adapters\.mjs/,'the physical platform port import must use the retry boundary');
assert.match(loader,/importModule\(`\.\.\/luvia-trip-context\.js/,'the Trip context import must use the retry boundary');
