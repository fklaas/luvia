...
(async()=>{
  ...
  try{
    ...
    await page.waitForFunction(()=>window.LuviaKernelVersion?.build==='13.82.55'&&Boolean(window.LuviaPWA));
    ...
    await page.waitForFunction(()=>window.LuviaKernelVersion?.build==='13.82.55'&&Boolean(navigator.serviceWorker.controller));

    const cachesAfterUpdate=await page.evaluate(()=>caches.keys());
    assert.ok(cachesAfterUpdate.includes('luvia-shell-v13.82.55'),'current release cache is missing');
    assert.equal(cachesAfterUpdate.includes('luvia-shell-v13.17.0'),false,'stale fixed cache survived recovery');

    await context.setOffline(true);
    await page.reload({waitUntil:'domcontentloaded'});
    await page.waitForFunction(()=>window.LuviaKernelVersion?.build==='13.82.55'&&document.querySelector('#app')?.children.length>0);
    const offlineCss=await page.evaluate(()=>fetch('app/module-hubs.css?v=13.82.55').then(response=>response.text()));
