...
(async()=>{
  ...
  try{
    ...
    await page.waitForFunction(()=>window.LuviaKernelVersion?.build==='13.82.72'&&Boolean(window.LuviaPWA));
    await page.evaluate(()=>window.LuviaPWA.register());
    await page.waitForFunction(()=>Boolean(navigator.serviceWorker.controller));
    await page.reload({waitUntil:'domcontentloaded'});
    await page.waitForFunction(()=>window.LuviaKernelVersion?.build==='13.82.72'&&Boolean(navigator.serviceWorker.controller));
    ...
    assert.ok(cachesAfterRegister.includes('luvia-shell-v13.82.72'),'current release cache is missing');
    ...
    assert.ok(cachesAfterMaintenance.includes('luvia-shell-v13.82.72'),'explicit maintenance removed the active release cache');
    ...
    await page.reload({waitUntil:'domcontentloaded'});
    await page.waitForFunction(()=>window.LuviaKernelVersion?.build==='13.82.72'&&document.querySelector('#app')?.children.length>0);
    const offlineCss=await page.evaluate(()=>fetch('app/module-hubs.css?v=13.82.72').then(response=>response.text()));
