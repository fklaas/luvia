...
(async()=>{
  ...
  try{
    ...
    await page.waitForFunction(()=>window.LuviaKernelVersion?.build==='13.82.147'&&Boolean(window.LuviaPWA));
    await page.evaluate(()=>window.LuviaPWA.register());
    await page.waitForFunction(()=>Boolean(navigator.serviceWorker.controller));
    await page.reload({waitUntil:'domcontentloaded'});
    await page.waitForFunction(()=>window.LuviaKernelVersion?.build==='13.82.147'&&Boolean(navigator.serviceWorker.controller));
    ...
    assert.ok(cachesAfterRegister.includes('luvia-shell-v13.82.147'),'current release cache is missing');
    ...
    assert.ok(cachesAfterMaintenance.includes('luvia-shell-v13.82.147'),'explicit maintenance removed the active release cache');
    ...
    await page.reload({waitUntil:'domcontentloaded'});
    await page.waitForFunction(()=>window.LuviaKernelVersion?.build==='13.82.147'&&document.querySelector('#app')?.children.length>0);
    const offlineCss=await page.evaluate(()=>fetch('app/module-hubs.css?v=13.82.147').then(response=>response.text()));
    ...
    assert.equal(offlineController.build,'13.82.147','offline document is controlled by a stale worker build');
