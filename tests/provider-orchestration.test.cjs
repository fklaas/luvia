...
(async()=>{
 ...
 calls=[];globalThis.fetch=async(url,init)=>{calls.push(String(url));if(String(url).includes('/rpc/'))return ok({allowed:true});return ok({results:[],items:[{id:'here:Chinese',resultType:'place',title:'Chinese fixture',position:{lat:54,lng:10},categories:[{name:'Restaurant'}],foodTypes:[{name:'Chinese',primary:true}]},{id:'here:SecondaryChinese',resultType:'place',title:'German fixture with broad cuisine metadata',position:{lat:54,lng:10},categories:[{name:'Restaurant'}],foodTypes:[{name:'German',primary:true},{name:'Chinese',primary:false}]},{id:'here:Steak',resultType:'place',title:'Unrelated steak fixture',position:{lat:54,lng:10},categories:[{name:'Restaurant'}],foodTypes:[{name:'Steak House',primary:true}]}]})};
