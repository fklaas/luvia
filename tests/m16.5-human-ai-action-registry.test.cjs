...
assert.equal(validated.registry.summary.aiCoverage.REGISTERED_PARTIAL, 64);
assert.equal(validated.registry.summary.aiCoverage.NATIVE_CHAT, 14);
assert.equal(validated.runtimeActions.length, 27);
assert.equal(Object.keys(validated.inputContracts.contracts).length, 27);
assert.equal(validated.registry.summary.inputContracts.READY, 73);
...
assert.equal(validated.sourceAudit.markers.length, 1004);
...
console.log('330 semantic actions · 27 runtime actions · 1004 active source markers: PASS');
console.log('27 typed runtime input contracts and 73 mapped human-action rows: PASS');
console.log('27/27 runtime actions guarded before Ledger or Owner execution: PASS');
