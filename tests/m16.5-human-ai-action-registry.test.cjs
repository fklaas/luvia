...
assert.equal(validated.registry.summary.inputContracts.READY, 66);
assert.equal(validated.registry.summary.ownerBinding.PUBLIC_CONTRACT_BOUND, 245);
...
assert.equal(validated.sourceAudit.markers.length, 898);
...
console.log('329 semantic actions · 23 runtime actions · 898 active source markers: PASS');
console.log('23 typed runtime input contracts and 66 mapped human-action rows: PASS');
console.log('23/23 runtime actions guarded before Ledger or Owner execution: PASS');
console.log('245 Navigation/Auth/Identity/Collaboration/Intelligence/Trip/Journey/Booking/Places/Media/Memory/Event/Platform actions bound to verified public Owner methods: PASS');
