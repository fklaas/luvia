...
assert.match(moduleSource,/onViewportResults:\(rows,descriptor\)=>\{state\.activeViewport=descriptor;state\.results=filteredHotelRows\(spatial\?\.decoratePreferences\?\.\(rows,state\.trip,\{category:'accommodation',query:state\.query\}\)\|\|rows\);state\.searchMap=new Map/,'live Hotel pins must retain their geographic scope and atomically apply local filters and preferences');
