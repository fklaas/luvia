...
assert.match(moduleSource,/onViewportResults:rows=>\{state\.results=filteredHotelRows\(spatial\?\.decoratePreferences\?\.\(rows,state\.trip,\{category:'accommodation',query:state\.query\}\)\|\|rows\);state\.searchMap=new Map/,'live Hotel pins must replace and preference-decorate the selectable map projection atomically after exact local filters');
