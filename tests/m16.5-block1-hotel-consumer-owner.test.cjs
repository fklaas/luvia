...
for(const token of ["VERSION='5.1.0-bright-search-truth'",'luvia-hotels-bright-v3','data-hotel-search-status','Unterkünfte konnten nicht geladen werden','data-place-retry','reads.searchStayOffers','commands.openStayOffer','userGesture:true','Dieses Angebot öffnen','Datumsformat: TT.MM.JJJJ','Keine Livepreis-Quelle verbunden','kein Marktbestpreis',"placeType:'accommodation'",'LuviaPlaceExperience.plannedPanel','LuviaPlaceExperience.discovery','LuviaPlaceCollections.favoritePanel','LuviaPlaceUI.card'])assert.ok(moduleSource.includes(token),`Hotel consumer owner token missing: ${token}`);
