...
const ctx=vm.createContext({console,Date,Map,Set,Promise,setTimeout,clearTimeout,URL,URLSearchParams,AbortSignal,Deno:{env:{get:name=>['GEOAPIFY_API_KEY','FOURSQUARE_API_KEY'].includes(name)?'test':''}},document:{documentElement:{classList:{contains:()=>false}}}});ctx.window=ctx;
ctx.FOURSQUARE_API_VERSION='2025-06-17';ctx.FOURSQUARE_PRO_FIELDS=['fsq_place_id','name','location','latitude','longitude'];ctx.FOURSQUARE_SEARCH_FIELDS=[...ctx.FOURSQUARE_PRO_FIELDS,'distance','photos'];ctx.FOURSQUARE_DETAILS_FIELDS=[...ctx.FOURSQUARE_PRO_FIELDS,'photos'];ctx.boundedFoursquareError=()=>({code:'TEST',message:'test'});ctx.normalizeFoursquarePlace=(p,{evidenceKind='place-search'}={})=>({id:`fsq:${p.fsq_place_id}`,providerPlaceId:`fsq:${p.fsq_place_id}`,name:p.name,location:{latitude:p.latitude,longitude:p.longitude},photos:(p.photos||[]).map(photo=>({uri:photo.url})),providerRefs:{foursquare:p.fsq_place_id},evidence:[{provider:'foursquare',kind:evidenceKind}]});
...
(async()=>{
 ...
 const calls=[],detailFetch=async url=>{calls.push(new URL(url));return{ok:true,status:200,json:async()=>({features:[{properties:{feature_type:'details',place_id:'abc',name:'Real Place',lat:54.02,lon:10.75,categories:['catering.restaurant'],wiki_and_media:{image:'https://example.org/real-place.jpg'}}}]})}};ctx.fetch=detailFetch;
 ...
 ctx.fetch=async url=>{const u=new URL(url);if(u.pathname.endsWith('/places/search'))return{ok:true,status:200,json:async()=>({results:[{fsq_place_id:'exact',name:'Real Place',latitude:54.0204,longitude:10.7504,photos:[{url:'https://example.org/foursquare-exact.jpg'}]},{fsq_place_id:'wrong',name:'Nearby Other',latitude:54.0201,longitude:10.7501,photos:[{url:'https://example.org/wrong.jpg'}]}]})};throw new Error(`Unexpected exact-media call ${u}`)};
 const media=await ctx.provider.enrichExactFoursquareMedia({id:'here:real',name:'Real Place',location:{latitude:54.02,longitude:10.75},photos:[],providerRefs:{here:'real'},evidence:[{provider:'here',kind:'place-details'}]});assert.equal(media.photos[0].uri,'https://example.org/foursquare-exact.jpg');assert.equal(media.providerRefs.here,'real');assert.equal(media.providerRefs.foursquare,'exact');assert.equal(media.evidence.length,2,'exact photo evidence augments provider identity instead of erasing it');
 ctx.fetch=async url=>{const u=new URL(url);if(u.pathname.endsWith('/places/search'))return{ok:true,status:200,json:async()=>({results:[{fsq_place_id:'photo-endpoint',name:'Exact Photo Place',latitude:54.0202,longitude:10.7502}]})};if(u.pathname.endsWith('/places/photo-endpoint/photos'))return{ok:true,status:200,json:async()=>[{url:'https://example.org/dedicated-photo.jpg'}]};throw new Error(`Unexpected dedicated photo call ${u}`)};
 const dedicated=await ctx.provider.enrichExactFoursquareMedia({id:'geoapify:photo',name:'Exact Photo Place',location:{latitude:54.02,longitude:10.75},photos:[],providerRefs:{geoapify:'photo'}});assert.equal(dedicated.photos[0].uri,'https://example.org/dedicated-photo.jpg','the dedicated current Places photo endpoint hydrates an exact selected place');
 ctx.fetch=async()=>({ok:true,status:200,json:async()=>({results:[{fsq_place_id:'wrong',name:'Different Place',latitude:54.02,longitude:10.75,photos:[{url:'https://example.org/wrong.jpg'}]}]})});
 const rejected=await ctx.provider.enrichExactFoursquareMedia({id:'tomtom:real',name:'Real Place',location:{latitude:54.02,longitude:10.75},photos:[],providerRefs:{tomtom:'real'}});assert.equal(rejected.photos.length,0,'a nearby image with a different place name must never be adopted');
 ctx.fetch=detailFetch;
