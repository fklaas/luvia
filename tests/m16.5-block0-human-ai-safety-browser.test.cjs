...
assert.equal(policy.summary.policyActions,329);assert.equal(Object.keys(policy.summary.classifications).length,6);assert.ok(policy.summary.consentGated>0);assert.ok(policy.summary.providerGated>0);
