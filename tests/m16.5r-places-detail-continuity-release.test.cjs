...
assert.ok(version.includes("core:'4.82.168'")&&version.includes("build:'13.82.168.21'"),'kernel build must be the geoapify cache-bust release');
assert.match(version,/name:'M16\.5 Local Places and Trip Recovery'/);
for(const asset of [
...
])assert(index.includes(`${asset}?v=13.82.168.21`),`active M16.5R cache key missing for ${asset}`);
assert(worker.includes("const CACHE='luvia-shell-v13.82.168.21-local-recovery'"));
...
console.log('App / Core / shell cache: 13.82.168.21 / 4.82.168 / luvia-shell-v13.82.168.21-local-recovery');
