...
files.set(versionPath,read(versionPath).replace(/build:'[^']+'/,"build:'13.82.168.1819'"));run();
assert.equal((read(indexPath).match(/src="app\/luvia-runtime-loader.mjs/g)||[]).length,1,'version upgrade must replace, never append');
assert.ok(read(indexPath).includes('data-luvia-runtime-bundle="13.82.168.1819"'));
