...
assert.equal(core.contractId,'intelligence.human-ai-safety-policy.v1');assert.equal(document.policies.length,327);assert.equal(document.summary.policyActions,327);assert.equal(Object.keys(document.summary.classifications).length,6);
assert.equal(new Set(document.policies.map(item=>item.actionId)).size,327);assert.ok(document.policies.every(item=>item.naturalLanguageConfirmsMutation===false&&item.ownerExecution===false));
