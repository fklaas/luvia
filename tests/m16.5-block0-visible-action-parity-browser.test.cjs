...
assert.equal(registry.actions.length,329,'the visible Block 0 gate requires the complete 329-action registry');
...
assert.match(fixture,/data\?\.summary\?\.semanticActions!==329/,'the visible gate must fail closed on an incomplete registry');
...
assert.match(fixture,/B0\.03 · 23 von 23 Eingabeverträgen aktiv/);
...
assert.match(fixture,/23 von 23 sind aktiv/);
assert.match(fixture,/runtimeEnforced\?\.length!==23/);
...
assert.match(fixture,/Sie behauptet ausdrücklich nicht, dass alle 329 Aktionen bereits im Chat ausführbar sind/);
