...
assert.equal(registry.actions.length,327,'the visible Block 0 gate requires the complete 327-action registry');
...
assert.match(fixture,/data\?\.summary\?\.semanticActions!==327/,'the visible gate must fail closed on an incomplete registry');
...
assert.match(fixture,/B0\.03 · 21 von 21 Eingabeverträgen aktiv/);
...
assert.match(fixture,/21 von 21 sind aktiv/);
assert.match(fixture,/runtimeEnforced\?\.length!==21/);
...
assert.match(fixture,/Sie behauptet ausdrücklich nicht, dass alle 327 Aktionen bereits im Chat ausführbar sind/);
