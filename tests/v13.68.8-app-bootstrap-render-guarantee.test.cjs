...
ok(shell.includes("version:'13.82.140'"),'LuviaApp version missing');
...
ok(index.includes('app/app-shell.js?v=13.82.140'),'index shell cache bust missing');
ok(index.includes('app/public-entry.js?v=13.82.140'),'index public entry cache bust missing');
ok(index.includes('app/luvia-runtime-loader.mjs?v=13.82.140-split9'),'split runtime loader missing');
ok(/luvia-shell-v13\.82\.140(?:-runtime\d+)?/.test(sw),'service worker cache missing');
ok(version.includes("core:'4.82.140'")&&version.includes("build:'13.82.140'"),'kernel version missing');
