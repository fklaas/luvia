...
ok(shell.includes("version:'13.82.124'"),'LuviaApp version missing');
...
ok(index.includes('app/app-shell.js?v=13.82.124'),'index shell cache bust missing');
ok(index.includes('app/public-entry.js?v=13.82.124'),'index public entry cache bust missing');
ok(index.includes('app/luvia-runtime-loader.mjs?v=13.82.124-split7'),'split runtime loader missing');
ok(/luvia-shell-v13\.82\.124(?:-runtime\d+)?/.test(sw),'service worker cache missing');
ok(version.includes("core:'4.82.124'")&&version.includes("build:'13.82.124'"),'kernel version missing');
