...
ok(shell.includes("version:'13.82.160'"),'LuviaApp version missing');
...
ok(index.includes('app/app-shell.js?v=13.82.160'),'index shell cache bust missing');
ok(index.includes('app/public-entry.js?v=13.82.160'),'index public entry cache bust missing');
ok(index.includes('app/luvia-runtime-loader.mjs?v=13.82.160-split9'),'split runtime loader missing');
ok(/luvia-shell-v13\.82\.160(?:-runtime\d+)?/.test(sw),'service worker cache missing');
ok(version.includes("core:'4.82.160'")&&version.includes("build:'13.82.160'"),'kernel version missing');
