...
ok(shell.includes("version:'13.82.168.171'"),'LuviaApp version missing');
...
ok(index.includes('app/app-shell.js?v=13.82.168.171'),'index shell cache bust missing');
ok(index.includes('app/public-entry.js?v=13.82.168.171'),'index public entry cache bust missing');
ok(index.includes('app/luvia-runtime-loader.mjs?v=13.82.168.171-local-recovery'),'split runtime loader missing');
ok(sw.includes("const CACHE='luvia-shell-v13.82.168.171-local-recovery'"),'service worker cache missing');
ok(version.includes("core:'4.82.290'")&&version.includes("build:'13.82.168.171'"),'kernel version missing');
