...
test('public diagnostics are bounded and gateway owns a layered Pro-field fallback',async()=>{
  ...
  assert.match(source,/v2\.16\.4-osm-instance-failover/,'gateway search cache must invalidate after the bounded OSM continuity contract enters the public surface');
  assert.match(source,/version:'4\.38\.5-osm-instance-failover'/);
