...
test('public diagnostics are bounded and gateway owns a layered Pro-field fallback',async()=>{
  ...
  assert.match(source,/v2\.16\.13-exact-google-media-category-matrix/,'gateway search cache must invalidate after exact selected-place Google media and the complete category matrix enter the public surface');
assert.match(source,/version:'4\.38\.15-exact-google-media-category-matrix'/);
  ...
  assert.match(source,/priority:'bounded_dietary_evidence_and_exact_selected_media'/);
