...
test('public diagnostics are bounded and gateway owns a layered Pro-field fallback',async()=>{
  ...
  assert.match(source,/v2\.16\.8-canonical-dedupe/,'gateway search cache must invalidate after the cached OSM category transport enters the public surface');
  assert.match(source,/version:'4\.38\.9-persistent-category-canonical-dedupe'/);
