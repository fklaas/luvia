...
assert.match(version,/core:'4\.82\.259'/);
assert.match(version,/build:'13\.82\.168\.140'/);
assert.match(version,/name:'M16\.5 Places and Stays Quality'/);
assert.match(version,/channel:'integration-preview'/);
assert.match(worker,/const CACHE='luvia-shell-v13.82.168.140-local-recovery'/);
assert.equal(index.includes('?v=13.82.49'),false,'active entry must not retain the previous runtime cache key');
assert.ok(index.includes('app/app-shell.js?v=13.82.168.140'));
assert.ok(index.includes('app/today/today-experience.js?v=13.82.168.140'));
assert.ok(index.includes('core/experience/experience-contract-core.js?v=13.82.168.140'));
...
console.log('App / Core: 13.82.168.140 / 4.82.259');
