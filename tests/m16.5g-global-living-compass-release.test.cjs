...
assert.match(version,/core:'4\.82\.292'/);
assert.match(version,/build:'13\.82\.168\.173'/);
assert.match(version,/name:'M16\.5 Places and Stays Quality'/);
assert.match(version,/channel:'integration-preview'/);
assert.match(worker,/const CACHE='luvia-shell-v13.82.168.173-local-recovery'/);
assert.equal(index.includes('?v=13.82.49'),false,'active entry must not retain the previous runtime cache key');
assert.ok(index.includes('app/app-shell.js?v=13.82.168.173'));
assert.ok(index.includes('app/today/today-experience.js?v=13.82.168.173'));
assert.ok(index.includes('core/experience/experience-contract-core.js?v=13.82.168.173'));
...
console.log('App / Core: 13.82.168.173 / 4.82.292');
