...
window.LuviaDestination={getActive:()=>({name:'Scharbeutz',displayName:'Scharbeutz, Deutschland',country:'Deutschland',countryCode:'DE',placeId:'geoapify:scharbeutz',timezone:'Europe/Berlin',center:{lat:54.0266,lng:10.7547}}),list:()=>[]};
...
assert.strictEqual(api.runtimeVersion,'2.1.1-destination-timezone');
...
(async()=>{
  ...
  assert.deepStrictEqual(JSON.parse(JSON.stringify(cachedDestination)),{owner:'places',contractId:'places.v1',placeId:'geoapify:scharbeutz',name:'Scharbeutz',formattedAddress:'Scharbeutz, Deutschland',country:'Deutschland',countryCode:'DE',latitude:54.0266,longitude:10.7547,timezone:'Europe/Berlin',source:'destination-cache'});
  ...
  assert.deepStrictEqual(JSON.parse(JSON.stringify(destination)),{owner:'places',contractId:'places.v1',placeId:'copenhagen',name:'Kopenhagen',formattedAddress:'Kopenhagen, Dänemark',country:'Dänemark',countryCode:'DK',latitude:55.6761,longitude:12.5683,timezone:''});
