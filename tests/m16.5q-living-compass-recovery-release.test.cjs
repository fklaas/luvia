...
assert.match(worker,/const CACHE='luvia-shell-v13\.82\.55'/);
assert.equal(index.includes('?v=13.82.54'),false,'active entry retains the revoked candidate cache key');
for(const asset of ['intelligence/kernel/version.js','intelligence/pwa-service.js','app/app-shell.js','app/module-hubs.js','app/module-hubs.css','app/places/places-spatial-experience.js','app/places/places-spatial-experience.css'])assert.ok(index.includes(`${asset}?v=13.82.55`),`M16.5Q cache key missing for ${asset}`);
...
console.log('App / Core / shell cache: 13.82.55 / 4.82.55 / luvia-shell-v13.82.55');
