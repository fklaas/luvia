...
(async()=>{
  ...
  assert.match(discoverySource,/options\.fastPath===true\?Math\.min\(8,/,'trip breadth may issue at most eight bounded parallel query variants while ordinary first paint still requests one');
  ...
  assert.match(indexSource,/vendor\/maplibre\/maplibre-gl-5\.12\.0\.js\?v=13\.82\.168\.196" defer/,'the same-origin map renderer must preserve execution order without blocking HTML parsing');
