...
(async()=>{
  ...
  assert.match(runtimeLoaderSource,/identity-platform-web-adapter\.js[\s\S]*platform-port-adapters\.mjs[\s\S]*media-storage-web-adapter\.mjs[\s\S]*supabase-2\.112\.4\.js[\s\S]*luvia-runtime-precontext-13\.82\.167\.bundle\.js[\s\S]*luvia-trip-context\.js[\s\S]*luvia-runtime-postcontext-13\.82\.167\.bundle\.js/,'the physical ports and vendor boundary must be ready before Trip state, its web binding, and all product consumers execute in contract order');
  ...
  assert.match(indexSource,/vendor\/maplibre\/maplibre-gl-5\.12\.0\.js\?v=13\.82\.167" defer/,'the same-origin map renderer must preserve execution order without blocking HTML parsing');
