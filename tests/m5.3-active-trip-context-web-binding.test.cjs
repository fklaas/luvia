...
async function main() {
  ...
  assert.ok(
    binding.includes(
      "./core/trips/active-trip-context.mjs?v=13.82.36"
    ),
  ...
  assert.strictEqual(
    count(
      index,
      '<script type="module" src="luvia-trip-context.js?v=13.82.36"></script>'
    ),
    1,
  ...
  assert.strictEqual(
    count(
      index,
      '<script src="luvia-trip-context.js?v=13.82.36"></script>'
    ),
    0,
  ...
  const tripStoreIndex =
    index.indexOf(
      'core/trips/trip-store.js?v=13.82.36'
    );

  const tripContextIndex =
    index.indexOf(
      'luvia-trip-context.js?v=13.82.36'
    );

  const travelContextIndex =
    index.indexOf(
      'core/context/travel-context-service.js?v=13.82.36'
    );

  const contractIndex =
    index.indexOf(
      'core/platform/trip-contract-adapter.js?v=13.82.36'
    );

  const bootIndex =
    index.indexOf(
      'core/runtime/boot-coordinator.js?v=13.82.36'
    );
