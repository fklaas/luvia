...
(async()=>{
  ...
  const plannerContract=h.sandbox.LuviaIntelligenceContractV1;h.sandbox.LuviaIntelligenceContractV1={...plannerContract,reads:{...plannerContract.reads,planTripPlaceSearch:async()=>{throw Object.assign(new Error('timeout'),{code:'OPENAI_TIMEOUT'});}}};
  const semanticRetry=await h.api.readTripAnchorPlaces(h.state,{...projected,travelOrder:{...projected.travelOrder,mustDo:[],experiences:{wishes:['Escape Room oder Bowling']}}},{});
  check(()=>assert.equal(semanticRetry.anchorSearch.complete,false,'A semantic-only experience remains retryable when its AI search plan times out'));
  const mixedRetry=await h.api.readTripAnchorPlaces(h.state,{...projected,travelOrder:{...projected.travelOrder,mustDo:['Hafen'],experiences:{wishes:['Escape Room oder Bowling']}}},{});
  check(()=>assert.equal(mixedRetry.anchorSearch.complete,false,'A successful must-do fallback cannot hide an unresolved semantic experience after the planner timeout'));
