...
(async()=>{
  ...
  check(()=>assert.equal(structuredAnchors.some(item=>item.query==='Shopping in Einkaufszentren'),false,'Soft experience wishes stay semantic and never become hard fallback anchors'));
