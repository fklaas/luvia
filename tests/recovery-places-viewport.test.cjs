...
(async()=>{
 ...
 const st=ctx.recovery.state;st.surface='places';st.trip={id:'same-trip',destinationName:'Scharbeutz',destinationLat:54.02,destinationLng:10.75};st.category='food';st.categories=[{key:'food'}];st.results=[row('geoapify:cached'),row('tomtom:cached'),row('here:cached')];st.lastSearchAt=new Date().toISOString();ctx.recovery.saveCached();assert.equal(savedCache.size,1);st.results=[];assert.equal(ctx.recovery.loadCached(),true);assert.equal(st.results.length,3,'same-trip resume restores the complete free-provider cohort');
 ...
 st.searchRadiusMeters=10000;assert.match(ctx.recovery.categoryCohortKey(),/"radius":10000/,'explicit 10 km search has its own category-cohort identity');st.searchRadiusMeters=0;
