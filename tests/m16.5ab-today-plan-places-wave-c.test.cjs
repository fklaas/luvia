...
assert.match(journeyComposer,/Reisepuls/,'Timeline keeps one compact Reisepuls instead of the rejected multi-card pulse rail');
...
assert.match(suggestionSheet,/maximumSuggestions/,'the visible suggestion count must remain bounded by the selected travel pace');
...
assert.match(suggestionSheet,/`Für \$\{strongest\.isCurrent\?'dich':strongest\.name\}:/,'each suggestion must explain which covered traveler it fits without the rejected stock phrase');
assert.match(suggestionSheet,/noch nicht belastbar/,'missing provider evidence must be stated instead of inventing a fit for a traveler');
...
assert.match(suggestionSheet,/function exactTimelineConflict\(/,'every card-local plan must preview existing-entry and route conflicts before confirmation');
assert.match(suggestionSheet,/Ankunfts-\/Orientierungspuffer/,'route checks must include a realistic arrival and orientation buffer');
assert.match(suggestionSheet,/buffer=Math\.min\(20,rawBuffer\)/,'the visible arrival/orientation buffer must remain realistic even when the internal uncertainty tail is wider');
assert.match(suggestionSheet,/Öffnungszeiten sind veraltet oder widersprüchlich/,'stale provider facts must not be asserted as safe opening truth');
assert.match(suggestionSheet,/isGroup\?'Warum für euch\?':'Warum für dich\?'/,'traveler-fit evidence must remain available as a correctly addressed collapsible card layer');
assert.match(suggestionSheet,/function liveDayBalance\(/,'multi-selection must calculate at most one focused live day-balance correction');
assert.match(suggestionSheet,/const ordered=\[\.\.\.places\]\.sort/,'per-card dates and times must define the live chronological suggestion order');
assert.match(suggestionSheet,/function weatherForDate\(/,'same-day and dated provider forecasts must be resolved before weather claims are shown');
...
assert.match(suggestionSheet,/imageUrl:clean\(place\.image\?\.url\|\|place\.photoUri\|\|place\.imageUrl\|\|place\.photos\?\.\[0\]\?\.uri\)/,'the chosen verified candidate image must stay with its Timeline entry');
