...
const {sync}=require('../scripts/sync-planning-status.cjs');
assert.deepEqual(sync().changed,[],'All active plans and handoffs must match the canonical current status, next step and package evidence');
assert.ok(plan.packages.some(row=>row.id===plan.currentWork.nextStep.package),'The next step must belong to a known package');
