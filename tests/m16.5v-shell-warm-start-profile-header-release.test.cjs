...
assert.match(index, /window\.addEventListener\('pageshow',event=>\{if\(!event\.persisted\)return;root\.classList\.add\('lv-boot-warm'\);root\.dataset\.luviaBoot='ready'/);
assert.match(index, /document\.getElementById\('app'\)\?\.setAttribute\('aria-busy','false'\)/);
...
assert.match(boot, /progress\('cloud-snapshot'/,'background synchronization must not replace the visible ready boot phase');
