...
async function run(){
  ...
  const supportTimeout=harness();const supportBase=supportTimeout.sandbox.LuviaAI.run;supportTimeout.sandbox.LuviaAI.run=async(capability,request,options)=>{if(capability==='trip.compose'&&request.planningContract?.repairMetadataOnly){const error=Object.assign(new Error('timeout'),{code:'OPENAI_TIMEOUT'});throw error;}const response=await supportBase(capability,request,options);if(capability==='trip.compose')response.data.backupOptions=[];return response;};await supportTimeout.api.loadAiDayDraft(supportTimeout.state);check(()=>assert.equal(supportTimeout.state.aiDraft.status,'ready','Optional Plan-B enrichment cannot discard an already complete trip after a timeout'));check(()=>assert.ok(supportTimeout.state.aiDraft.itinerary.warnings.some(item=>/Plan-B-Alternativen/.test(item))));
  const repairedSupportTimeout=harness(),repairCandidates=Array.from({length:32},(_,i)=>repairedSupportTimeout.candidate(['food','culture','activities','water'][i%4],i+1)),repairInput={days:repairedSupportTimeout.api.itineraryDays(repairedSupportTimeout.state),destination:repairedSupportTimeout.state.data.destination,candidates:repairCandidates,brief:{travelOrder:{categories:[{category:'activities',label:'Aktivitäten',importance:'required'}],mustDo:['Ein aktives Erlebnis'],exclusions:['Keine Parkfüller'],rhythm:{freeTimePercent:25}},policy:{}}},repairOriginal=await repairedSupportTimeout.sandbox.LuviaIntelligenceContractV1.reads.composeTripItinerary(repairInput),repairBase=repairedSupportTimeout.sandbox.LuviaAI.run,oldRepairIds=repairOriginal.days[1].entries.map(item=>item.providerPlaceId),keptRepairId=oldRepairIds[0],removedRepairId=oldRepairIds[1],keptRepairName=repairCandidates.find(item=>item.providerPlaceId===keptRepairId).name,removedRepairName=repairCandidates.find(item=>item.providerPlaceId===removedRepairId).name,sortedRepairIds=repairCandidates.map(item=>item.providerPlaceId).sort(),refFor=id=>`p${sortedRepairIds.indexOf(id)+1}`;repairedSupportTimeout.sandbox.LuviaAI.run=async(capability,request,options)=>{if(capability==='trip.compose-day-repair'){const policy=request.days[0],catalog=[request.candidateCatalog.find(item=>item.providerPlaceId===refFor(keptRepairId)),...request.candidateCatalog.filter(item=>![refFor(keptRepairId),refFor(removedRepairId)].includes(item.providerPlaceId))].filter(Boolean),start=Number(policy.notBefore.slice(0,2))*60+Number(policy.notBefore.slice(3)),entries=catalog.slice(0,policy.minimumMoments).map((place,index)=>{const minutes=start+index*180;return{providerPlaceId:place.providerPlaceId,time:`${String(Math.floor(minutes/60)).padStart(2,'0')}:${String(minutes%60).padStart(2,'0')}`,durationMinutes:90,reason:'Gezielt repariert.'}});return{ok:true,meta:{fallback:false},data:{days:[{date:policy.date,theme:'Gezielt repariert',balance:{energy:'balanced',freeTimePurpose:'Bewusste Pause'},freeTime:policy.role==='full'?[{start:'17:00',end:'18:00',purpose:'Erholung'}]:[],entries}],confidence:.94}};}if(capability==='trip.compose'&&request.planningContract?.repairMetadataOnly)throw Object.assign(new Error('timeout'),{code:'OPENAI_TIMEOUT'});return repairBase(capability,request,options);};const repairStale={...copy(repairOriginal),travelPromise:{summary:`Besuch bei ${removedRepairName}.`,commitments:[`${removedRepairName} bleibt fest eingeplant.`],deliberateFreeTime:'Alte Pause.',exclusions:[]},uncoveredRequirements:[`Öffnung von ${keptRepairName} prüfen.`,`Öffnung von ${removedRepairName} prüfen.`,'Präferenzen später eingeladener Mitreisender abstimmen.']},repaired=await repairedSupportTimeout.sandbox.LuviaIntelligenceContractV1.reads.composeTripItinerary({...repairInput,existingItinerary:repairStale,repairDayDates:[repairInput.days[1].date]});check(()=>assert.doesNotMatch(repaired.travelPromise.summary,new RegExp(removedRepairName),'A repaired route never retains a stale promise when supporting enrichment times out'));check(()=>assert.ok(repaired.uncoveredRequirements.includes(`Öffnung von ${keptRepairName} prüfen.`),'A still-planned Place keeps its open requirement after a route repair'));check(()=>assert.ok(repaired.uncoveredRequirements.includes('Präferenzen später eingeladener Mitreisender abstimmen.'),'Repair-independent open requirements survive supporting enrichment timeouts'));check(()=>assert.ok(!repaired.uncoveredRequirements.some(item=>item.includes(removedRepairName)),'Stale Place-bound requirements are removed after that Place is repaired'));check(()=>assert.equal(repaired.supportingDetailsErrorCode,'OPENAI_TIMEOUT'));repairedSupportTimeout.sandbox.LuviaAI.run=repairBase;const recoveredSupport=await repairedSupportTimeout.sandbox.LuviaIntelligenceContractV1.reads.composeTripItinerary({...repairInput,existingItinerary:repaired,repairDayDates:[repairInput.days[2].date]});check(()=>assert.equal(recoveredSupport.supportingDetailsErrorCode,null,'A successful supporting refresh clears the prior transient timeout'));check(()=>assert.ok(!recoveredSupport.warnings.some(item=>/Plan-B-Alternativen.*nächsten Prüfung/.test(item)),'A successful supporting refresh clears its obsolete retry warning'));
  ...
  check(()=>assert.equal(anchors.state.aiDraft.anchorSearch.version,5,'Old searches are renewed when structured must-do fallbacks gain durable category ownership'));
  ...
  const stale=copy(originalPlan),oldPlaceId=stale.days[0].entries[0].providerPlaceId;
  stale.bookingOrder=[{rank:1,subject:'Bisherigen Hafen klären',providerPlaceId:oldPlaceId,state:'open',reason:'Alter Hafen',blocks:[],evidenceRefs:[]}];
  stale.uncertaintyMap=[{subject:'Alter Hafen',kind:'booking',state:'open',providerPlaceIds:[oldPlaceId]}];
  details.sandbox.dayRepairOverride=(capability,request)=>({ok:true,meta:{fallback:false},data:{days:[{...copy(stale.days[0]),entries:request.candidateCatalog.slice(-stale.days[0].entries.length).map((place,index)=>({...copy(stale.days[0].entries[index]),providerPlaceId:place.providerPlaceId}))}]}});
  details.sandbox.modelCalls=[];
  details.sandbox.LuviaAI.run=async(capability,request,options)=>{const response=await detailsRun(capability,request,options);if(request.planningContract?.rebuildAfterDayRepair){const currentId=request.fixedItinerary.days[0].entries[0].providerPlaceId;response.data.bookingOrder=[{rank:1,subject:'Aktuellen Hafen klären',providerPlaceId:currentId,state:'open',reason:'Neuer Hafen',blocks:[],evidenceRefs:[]}];response.data.uncertaintyMap=[];}return response;};
  const repairedDetails=await details.sandbox.LuviaIntelligenceContractV1.reads.composeTripItinerary({...detailsInput,existingItinerary:stale,repairDayDates:[stale.days[0].date]});
  check(()=>assert.notEqual(repairedDetails.days[0].entries[0].providerPlaceId,oldPlaceId));
  check(()=>assert.deepEqual(copy(repairedDetails.days.slice(1)),copy(stale.days.slice(1)),'Repairing supporting details preserves all other day routes'));
  check(()=>assert.equal(repairedDetails.bookingOrder[0].providerPlaceId,repairedDetails.days[0].entries[0].providerPlaceId,'Booking dependency follows the repaired harbour, not the replaced Place'));
  check(()=>assert.ok(!repairedDetails.uncertaintyMap.some(item=>item.providerPlaceIds.includes(oldPlaceId)),'Empty rebuilt uncertainties cannot restore obsolete Place references'));
  check(()=>assert.equal(details.sandbox.modelCalls.filter(call=>call.request.planningContract?.rebuildAfterDayRepair).length,1,'All dependent metadata is rebuilt once even when existing backup options remain valid'));
  check(()=>assert.equal(details.sandbox.modelCalls.find(call=>call.request.planningContract?.rebuildAfterDayRepair).options.tier,'fast'));
  ...
  const fixed=harness();fixed.sandbox.LuviaFirstTripComposer.mount(fixed.root,{resume:false,step:'welcome'});const fixedState=fixed.api.state();fixedState.modeChosen=true;fixedState.data.entryMode='ai';fixedState.data.inspirationCompleted=false;fixedState.data.inspirationBrief='Wir möchten sieben Tage nach Valencia in Spanien, ab Münster.';
  const fixedPlace={owner:'places',contractId:'places.v1',placeId:'verified-valencia',name:'València',formattedAddress:'València, Spanien',countryCode:'ES',latitude:39.47,longitude:-.38,timezone:'Europe/Madrid'};
  fixed.sandbox.LuviaPlacesContractV1.reads.suggestDestinations=async()=>({suggestions:[{placeId:fixedPlace.placeId}]});fixed.sandbox.LuviaPlacesContractV1.reads.getDestination=async()=>fixedPlace;
  fixed.sandbox.modelOverride=capability=>capability==='discovery.plan'?{ok:true,meta:{fallback:false},data:{searchPlans:[{query:'Valencia, Spanien',reason:'Euer gewähltes Reiseziel bleibt Valencia.'}],reasoningSummary:'Wir planen in Valencia.'}}:null;
  await fixed.api.inspire(fixedState);
  check(()=>assert.equal(fixedState.data.destination.placeId,'verified-valencia','A fixed named destination is geocoded through Places and applied directly'));
  check(()=>assert.equal(fixed.api.currentFlow(fixedState)[fixedState.index],'dates','A fixed destination proceeds to dates without a five-city selection'));
  check(()=>assert.equal(fixed.sandbox.modelCalls.filter(call=>call.capability==='discovery.plan').length,1,'No second paid round to fill five ideas for a fixed city'));
  check(()=>assert.equal(fixedState.data.requestBrief,fixedState.data.inspirationBrief,'The complete original wish survives direct destination resolution'));
  check(()=>assert.equal(fixed.tripCalls.length,0));
  const hypothesisInput={...detailsInput,userRequest:'Shopping und ein Strandtag',poolQuality:{candidateCount:30,distinctAreas:5,largestAreaShare:.2,specificRequests:[{query:'Mercado Central',count:0}],missingExperiences:['Mercado Central']}};
  details.sandbox.modelCalls=[];const hypothesisPlan=await details.sandbox.LuviaIntelligenceContractV1.reads.composeTripItinerary(hypothesisInput);
  const compositionRequest=details.sandbox.modelCalls.find(call=>call.capability==='trip.compose'&&!call.request.planningContract?.repairMetadataOnly).request;
  check(()=>assert.equal(compositionRequest.planningContract.poolQuality.missingExperiences,undefined,'Generated search hypotheses cannot become mandatory composition wishes'));
  check(()=>assert.equal(compositionRequest.userRequest,'Shopping und ein Strandtag'));
  details.sandbox.auditOverride=()=>({ok:true,meta:{fallback:false},data:{readyForReview:true,score:90,headline:'Vollständiger Entwurf mit offenen Live-Prüfungen.',promiseAssessment:{kept:true,summary:'Alle Planungswünsche sind vertreten.',missedCommitments:[]},dimensions:[],issues:[{code:'ROUTE_UNVERIFIED',severity:'attention',dayDate:'',providerPlaceIds:[],message:'Fahrpläne für 2027 bleiben zu prüfen.',suggestedRepair:'Aktuelle Fahrpläne vor der Reise prüfen.'}],repairInstructions:[],strengths:[],confidence:.9}});
  const livePending=await details.sandbox.LuviaIntelligenceContractV1.reads.auditTripItinerary({...hypothesisInput,itinerary:hypothesisPlan}),auditRequest=details.sandbox.modelCalls.at(-1).request;
  check(()=>assert.equal(livePending.readyForReview,true,'An honest provisional route does not invalidate an otherwise complete draft'));
  check(()=>assert.equal(livePending.issues[0].severity,'attention'));
  check(()=>assert.equal(auditRequest.poolQuality.specificRequests,undefined,'The audit cannot promote an AI market hypothesis into a user wish'));
  check(()=>assert.equal(auditRequest.auditScope.providerQueriesAreRequirements,false));
  check(()=>assert.equal(auditRequest.auditScope.liveVerificationRequiredBeforeBooking,true));
  const mustDoNight={...nightData,data:{...nightData.data,goals:[{type:'culture',label:'Kultur'}],hardConstraints:[{key:'mustDo',value:'Ein lebendiger Abend mit Nachtleben',label:'Nachtleben'}],softPreferences:[{key:'budgetLevel',value:'balanced',label:'Mittleres Budget'}]}};
  const projectedNight=project({...details.state.data,tripPreferences:{...details.state.data.tripPreferences,movementStyle:'city_transit'}},mustDoNight);
