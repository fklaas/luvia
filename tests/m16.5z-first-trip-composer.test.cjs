...
for(const marker of ['pathSceneMarkup','mountPathScene','data-ftc-path-canvas','mountSeasonCanvas','destinationInsideScope','containsDestination','setTowns','preserveWorld','lx-day-journey'])assert.ok(composer.includes(marker),`missing cinematic Composer marker: ${marker}`);
...
assert.match(composer,/startRadius=origin\.radius\*\.82/,'Every moving route must originate at the visible Compass rim');
assert.match(composer,/angle=Math\.atan2\(dy,dx\)/,'Each Compass route must carry an oriented moving aircraft');
