...
for(const marker of ['pathSceneMarkup','mountSeasonCanvas','destinationInsideScope','containsDestination','setTowns','preserveWorld','lx-day-journey'])assert.ok(composer.includes(marker),`missing cinematic Composer marker: ${marker}`);
