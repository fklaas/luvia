...
assert.match(css,/\.ftc-host/);assert.match(css,/scrollbar-width:none/);assert.match(css,/@media\(max-width:560px\)/);assert.match(css,/@media\(prefers-reduced-motion:reduce\)/);
...
assert.match(creator,/function deterministicCode/);assert.match(creator,/matchingTrip\(idempotencyKey,joinCode\)/);assert.match(creator,/firstTripComposer:/);
