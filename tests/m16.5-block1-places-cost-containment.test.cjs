...
assert.match(gateway,/v2\.16\.8-canonical-dedupe/,'gateway cache must invalidate after the cached OSM category transport enters the public surface');
assert.match(gateway,/version:'4\.38\.9-persistent-category-canonical-dedupe'/,'gateway health version must expose the bounded OSM category and dietary evidence contract');
