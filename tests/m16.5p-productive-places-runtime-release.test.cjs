...
assert.ok(version.includes("build:'13.82.168.142'"),'kernel build must be the geoapify cache-bust release');
...
for(const asset of [
  ...
  'app/app-shell.js'
])assert.ok(index.includes(`${asset}?v=13.82.168.142`),`active entry release key missing for ${asset}`);
