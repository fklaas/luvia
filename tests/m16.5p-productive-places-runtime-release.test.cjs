...
assert.match(version,/core:'4\.82\.97'/);
assert.match(version,/build:'13\.82\.97'/);
assert.match(version,/name:'M16\.5X Shell Freeze Orbit and Control Course'/);
assert.match(version,/channel:'integration-preview'/);
assert.match(worker,/const CACHE='luvia-shell-v13\.82\.97'/);
...
for(const asset of [
  ...
  'app/app-shell.js'
])assert.ok(index.includes(`${asset}?v=13.82.97`),`active entry release key missing for ${asset}`);
...
console.log('App / Core: 13.82.97 / 4.82.97');
