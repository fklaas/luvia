...
assert.match(version,/core:'4\.82\.64'/);
assert.match(version,/build:'13\.82\.64'/);
assert.match(version,/name:'M16\.5 Living Compass Recovery'/);
assert.match(version,/channel:'integration-preview'/);
assert.match(worker,/const CACHE='luvia-shell-v13\.82\.64'/);
...
for(const asset of [
  ...
  'app/app-shell.js'
])assert.ok(index.includes(`${asset}?v=13.82.64`),`active entry release key missing for ${asset}`);
...
console.log('App / Core: 13.82.64 / 4.82.64');
