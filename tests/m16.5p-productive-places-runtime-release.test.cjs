...
assert.ok(version.includes("build:'13.82.168.21'"),'kernel build must be the geoapify cache-bust release');
assert.match(version,/name:'M16\.5 Local Places and Trip Recovery'/);
assert.match(version,/channel:'integration-preview'/);
assert.ok(worker.includes("const CACHE='luvia-shell-v13.82.168.21-local-recovery'"));
...
for(const asset of [
  ...
  'app/app-shell.js'
])assert.ok(index.includes(`${asset}?v=13.82.168.21`),`active entry release key missing for ${asset}`);
