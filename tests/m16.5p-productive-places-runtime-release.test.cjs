...
assert.match(version,/core:'4\.82\.111'/);
assert.match(version,/build:'13\.82\.111'/);
assert.match(version,/name:'M16\.5AB Verified Booking Route Gate'/);
assert.match(version,/channel:'integration-preview'/);
assert.match(worker,/const CACHE='luvia-shell-v13\.82\.111'/);
...
for(const asset of [
  ...
  'app/app-shell.js'
])assert.ok(index.includes(`${asset}?v=13.82.111`),`active entry release key missing for ${asset}`);
...
console.log('App / Core: 13.82.111 / 4.82.111');
