...
assert.match(version,/core:'4\.82\.98'/);
assert.match(version,/build:'13\.82\.98'/);
assert.match(version,/name:'M16\.5Y Identity Compass Onboarding'/);
assert.match(version,/channel:'integration-preview'/);
assert.match(worker,/const CACHE='luvia-shell-v13\.82\.98'/);
...
for(const asset of [
  ...
  'app/app-shell.js'
])assert.ok(index.includes(`${asset}?v=13.82.98`),`active entry release key missing for ${asset}`);
...
console.log('App / Core: 13.82.98 / 4.82.98');
