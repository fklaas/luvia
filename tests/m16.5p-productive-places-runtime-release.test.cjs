...
assert.match(version,/core:'4\.82\.106'/);
assert.match(version,/build:'13\.82\.106'/);
assert.match(version,/name:'M16\.5AB Today Places Recovery'/);
assert.match(version,/channel:'integration-preview'/);
assert.match(worker,/const CACHE='luvia-shell-v13\.82\.106'/);
...
for(const asset of [
  ...
  'app/app-shell.js'
])assert.ok(index.includes(`${asset}?v=13.82.106`),`active entry release key missing for ${asset}`);
...
console.log('App / Core: 13.82.106 / 4.82.106');
