...
assert.match(version,/core:'4\.82\.66'/);
assert.match(version,/build:'13\.82\.66'/);
assert.match(version,/name:'M16\.5 Landing Auth Foundation'/);
assert.match(version,/channel:'integration-preview'/);
assert.match(worker,/const CACHE='luvia-shell-v13\.82\.66'/);
...
for(const asset of [
  ...
  'app/app-shell.js'
])assert.ok(index.includes(`${asset}?v=13.82.66`),`active entry release key missing for ${asset}`);
...
console.log('App / Core: 13.82.66 / 4.82.66');
