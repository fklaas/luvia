...
async function beginSelectionNeedleTrace(page){
  await page.evaluate(()=>{
    const stage=document.querySelector('[data-stage] > .lv-view-host:not(.lv-route-previous) [data-plan-compass-stage]'),needle=stage?.querySelector('.lv-plan-compass-needle');
    if(!stage||!needle)throw new Error('selection needle trace requires a live Compass stage');
    const angle=()=>{const matrix=new DOMMatrixReadOnly(getComputedStyle(needle).transform);return Math.atan2(matrix.b,matrix.a)*180/Math.PI};
    const trace={lastIdle:angle(),samples:[],startedAt:null};window.__m165qSelectionNeedleTrace=trace;
    const sample=time=>{if(!needle.isConnected||trace.samples.length>90)return;const current=angle();if(stage.classList.contains('is-navigating')){if(trace.startedAt===null)trace.startedAt=time;trace.samples.push({ms:time-trace.startedAt,angle:current});if(time-trace.startedAt>=790)return}else trace.lastIdle=current;requestAnimationFrame(sample)};
    requestAnimationFrame(sample);
  });
}
async function assertSelectionNeedleTrace(page){
  const trace=await page.evaluate(()=>window.__m165qSelectionNeedleTrace),samples=trace?.samples||[];assert.ok(samples.length>=20,`mobile selection needle produced too few rendered frames: ${samples.length}`);
  const distance=(a,b)=>Math.abs(((a-b+180)%360+360)%360-180),first=samples[0].angle,last=samples.at(-1).angle;
  assert.ok(distance(first,trace.lastIdle)<=4,`selection needle snapped away from its rendered idle position: ${JSON.stringify({idle:trace.lastIdle,first})}`);
  assert.ok(distance(first,-90)>55,`selection needle skipped the visible direct journey to its target: ${first}`);
  assert.ok(distance(last,-90)<2.5,`selection needle did not settle at the chosen point: ${last}`);
  const transitions=samples.slice(1).map((sample,index)=>({step:sample.angle-samples[index].angle,elapsed:sample.ms-samples[index].ms})),steps=transitions.map(entry=>entry.step),rounded=new Set(samples.map(sample=>Math.round(sample.angle*10)/10));
  assert.ok(rounded.size>=12,`selection needle exposed too few intermediate positions: ${rounded.size}`);
  assert.ok(steps.every(step=>step<=.8),`selection needle reversed instead of taking the direct signed path: ${Math.max(...steps)}`);
  const maxVelocity=Math.max(...transitions.filter(entry=>entry.elapsed>0).map(entry=>Math.abs(entry.step)/entry.elapsed));assert.ok(maxVelocity<.65,`selection needle changed with snap-like angular velocity: ${maxVelocity}`);
}
...
(async()=>{
  ...
  try{
    ...
    await desktop.getByRole('button',{name:'Erinnern',exact:true}).first().click();await expectCompass(desktop,'Wie soll diese Reise weiterleben?');
    await desktop.getByRole('button',{name:'Kompass schließen und zu Heute zurückkehren'}).click();await heading(desktop,'Heute');
    await desktop.getByRole('button',{name:/Profil(?:einstellungen)? öffnen/,exact:true}).click();await expectCompass(desktop,'Was möchtest du für dich ausrichten?');assert.equal(await desktop.locator('.lv-e2e-dialog').count(),0,'the header avatar must open the Profile Compass instead of the legacy Profile overlay');
    await desktop.getByRole('button',{name:'Kompass schließen und zu Heute zurückkehren'}).click();await heading(desktop,'Heute');
    ...
    const placesReturn=await browser.newPage({viewport:{width:1440,height:900}});placesReturn.setDefaultTimeout(8000);await placesReturn.goto(FIXTURE,{waitUntil:'networkidle'});await expectCompass(placesReturn,'Welche Richtung soll die Planung nehmen?');await placesReturn.getByRole('button',{name:/^Places:/}).click();await heading(placesReturn,'Places');await placesReturn.getByRole('button',{name:'Living Compass öffnen',exact:true}).click();await expectCompass(placesReturn,'Welche Richtung soll die Planung nehmen?');assert.equal(new URL(placesReturn.url()).searchParams.get('screen'),'places','the productive Places Compass control must restore a fully interactive Plan constellation without rewriting the committed Places URL');await placesReturn.close();console.log('productive Places Compass return control / no route-host click replay: PASS');
    ...
    const directions=[['Places','Places','places'],['Meine Orte','Meine Orte','places-lifecycle'],['Timeline','Timeline','timeline'],['Booking','Buchungen','bookings'],['Checklisten','Checklisten','plan'],['Budget','Budget','plan'],['Routen','Route in Google Maps öffnen.','routes'],['Wetter','Wetterkontext','plan']];
    ...
    for(const touchCase of [
      {context:'plan',label:/^Places:/,screen:'places',target:'Places',traceNeedle:true},
      {context:'plan',label:/^Booking:/,screen:'bookings',target:'Buchungen'},
      {context:'trip',label:/^Live-Momente:/,screen:'plan',target:'Diesen Moment bewusst bewahren.',capture:true},
      {context:'memories',label:/^Moment bewahren:/,screen:'plan',target:'Diesen Moment bewusst bewahren.',capture:true},
      {context:'memories',label:/^Mediathek:/,screen:'gallery',target:'Galerie'}
    ]){
      ...
      if(touchCase.context==='memories'){await mobile.getByRole('button',{name:'Erinnern',exact:true}).first().tap();await expectCompass(mobile,'Wie soll diese Reise weiterleben?')}
      if(touchCase.traceNeedle)await beginSelectionNeedleTrace(mobile);const touchLogPromise=physicalTouchWithDrift(mobile,mobile.getByRole('button',{name:touchCase.label}));const touchLog=await touchLogPromise;if(touchCase.traceNeedle){await mobile.waitForTimeout(650);await assertSelectionNeedleTrace(mobile)}await heading(mobile,touchCase.target);
      assert.ok(touchLog.some(entry=>entry.type==='pointerup'),'a physical drifting touch must retain pointerup ownership');assert.equal(touchLog.some(entry=>entry.type==='pointercancel'),false,'a Compass tap must not be ceded to viewport scrolling');assert.equal(new URL(mobile.url()).searchParams.get('screen'),touchCase.screen,`${touchCase.label} physical touch must reach ${touchCase.screen}`);if(touchCase.capture){assert.equal(await mobile.locator('[data-compass-focus="capture"]').count(),1,'Live-Momente and Moment bewahren must open the accepted Media → Memory focus');assert.equal(await mobile.locator('#gallery-module').count(),0,'capture intent must never be substituted by the Fotogalerie')};await context.close();
    }
    console.log('physical mobile touch drift through Places / Booking / Live-Momente / Moment bewahren / Mediathek: PASS');
    console.log('mobile selection needle continuity / direct path / smooth frame progression: PASS');
