...
(async()=>{
  ...
  try{
    ...
    await desktop.getByRole('button',{name:/^Places:/}).click();assert.equal(await desktop.locator('[data-plan-compass-stage]').evaluate(stage=>stage.style.getPropertyValue('--lv-plan-selection-angle')),'-90deg','the needle must take the direct angle to Places without decorative revolutions');await desktop.locator('.lv-route-previous[aria-hidden="true"]').waitFor({state:'attached'});assert.equal(await desktop.locator('.lv-route-previous[aria-hidden="true"] :focus').count(),0,'an outgoing route host must never retain focused descendants while aria-hidden');assert.equal(await desktop.locator('.lv-route-previous').getAttribute('inert'),'','the outgoing route host must be inert during transition');await heading(desktop,'Places');
