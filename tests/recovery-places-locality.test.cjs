...
check('Empty search states coverage and offers explicit alternatives',()=>{const s=ctx.recovery.state;s.trip={destination:{...geo,name:'Scharbeutz'}};s.status='empty';s.filters.cuisines=['chinese_restaurant'];assert.match(ctx.recovery.emptySearchMessage(),/5-km-Umkreis um Scharbeutz/);assert.match(ctx.recovery.emptySearchMessage(),/Küchenangaben können fehlen/);assert.doesNotMatch(ctx.recovery.emptySearchActions(),/data-places-expand-radius/);assert.match(ctx.recovery.emptySearchActions(),/data-places-broaden-cuisine/);s.activeViewport={};assert.match(ctx.recovery.emptySearchMessage(),/sichtbaren Kartenausschnitt/);assert.doesNotMatch(ctx.recovery.emptySearchActions(),/data-places-expand-radius/);s.activeViewport=null;s.filters.cuisines=[]});
