...
check('Only explicit search scope can extend the destination radius',()=>{ctx.recovery.state.searchRadiusMeters=5000;assert.equal(ctx.recovery.tripGeography({destination:geo}).searchRadiusMeters,5000);ctx.recovery.state.searchRadiusMeters=10000;assert.equal(ctx.recovery.tripGeography({destination:geo}).searchRadiusMeters,10000);ctx.recovery.state.searchRadiusMeters=0});
check('Empty search states disclose staged explicit alternatives',()=>{const s=ctx.recovery.state;s.trip={destination:{...geo,name:'Scharbeutz'}};s.status='empty';s.filters.cuisines=['chinese_restaurant'];assert.match(ctx.recovery.emptySearchMessage(),/5-km-Umkreis um Scharbeutz/);assert.match(ctx.recovery.emptySearchMessage(),/Küchenangaben können fehlen/);assert.match(ctx.recovery.emptySearchActions(),/data-target-radius="10000"/);assert.match(ctx.recovery.emptySearchActions(),/Im 10-km-Umkreis suchen/);assert.match(ctx.recovery.emptySearchActions(),/data-places-broaden-cuisine/);s.searchRadiusMeters=10000;assert.match(ctx.recovery.emptySearchMessage(),/10-km-Umkreis um Scharbeutz/);assert.doesNotMatch(ctx.recovery.emptySearchActions(),/data-places-expand-radius/);s.activeViewport={};assert.match(ctx.recovery.emptySearchMessage(),/sichtbaren Kartenausschnitt/);assert.doesNotMatch(ctx.recovery.emptySearchActions(),/data-places-expand-radius/);s.activeViewport=null;s.searchRadiusMeters=0;s.filters.cuisines=[]});
