...
const m165fCss=shellCss.slice(shellCss.indexOf('M16.5F'));
...
assert.match(dockBlock,/data-ai-ask-open[^>]*data-luvia-experience-component="livingCompass"/,'Compass must open the real Intelligence surface');
...
assert.match(shell,/class="lv-brand"[^>]*data-view="today"/,'official brand must return to Today');
assert.match(shell,/class="lv-logo"/,'App Shell must retain the replaceable official logo mount');
assert.match(shell,/version:'13\.82\.50'/,'Consumer candidate version is missing');
