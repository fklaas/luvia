...
assert.match(sheet,/paintResults\(handle,result,selectedId\)/);
...
assert.match(css,/@keyframes lvjs-sheet-in/);
assert.match(bookingCss,/Verified-email fallback canvas/);
assert.match(bookingCss,/@keyframes lv-booking-inline-in/);
assert.match(bookingCss,/\.lv-booking-canvas\{overflow-y:auto;overscroll-behavior:contain;scrollbar-width:none\}/);
assert.match(bookingCss,/\.lv-booking-actions\{position:sticky;bottom:0/);
