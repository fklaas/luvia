...
assert.equal(contract.runtimeVersion,'1.0.1');
for(const prompt of [
...
])assert.equal(contract.routeIntent(prompt)?.actionId,'journey.day.read',`day intent was not routed: ${prompt}`);
...
assert.match(fs.readFileSync(path.resolve(__dirname,'../core/ai/ai-action-runtime.js'),'utf8'),/const VERSION='1\.0\.1'/);
