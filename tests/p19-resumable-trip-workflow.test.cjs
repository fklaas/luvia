...
function contracts(){
  ...
  assert.match(composer,/\['ready','brief-ready','brief-loading','loading','error'\]\.includes\(state\.aiDraft\?\.status\)/,'terminal planning evidence must survive reload without silently restarting paid work');checks++;
  assert.match(composer,/retryEscalated=state\.aiDraft\?\.errorCode==='AI_WORKFLOW_BUDGET_EXHAUSTED'\|\|Number\(state\.aiDraft\?\.qualityAttempts\|\|0\)>=3[\s\S]*?qualityAttempts:retryEscalated\?3:0,retryEscalated,error:null,errorCode:null/,'a manual retry must preserve reusable evidence and route an exhausted plan directly into one strongest-tier holistic repair');checks++;
