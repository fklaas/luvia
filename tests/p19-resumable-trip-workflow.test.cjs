...
function contracts(){
  ...
  assert.match(composer,/const resumingTargetedRepair=Boolean\(itinerary\)&&repairDayDates\.length===1/,'a preserved single failed day must enter targeted repair even after receiving a new workflow id');checks++;
  assert.match(composer,/repairWholeTrip=Boolean\(itinerary\)&&repairDayDates\.length>1,repairBase=repairWholeTrip\?null:itinerary,repairDates=repairWholeTrip\?\[\]:repairDayDates/,'several failed days must be repaired in one holistic bounded composition instead of one model call per day');checks++;
  assert.match(composer,/state\.aiDraft=\{\.\.\.state\.aiDraft,status:'idle',audit:null,qualityAttempts:0,error:null\}/,'a manual retry must discard the failed audit budget while retaining reusable brief, places and itinerary evidence');checks++;
