...
function contracts(){
  ...
  assert.match(registry,/'planning\.dialogue'[\s\S]*?maxOutputTokens:4000/);assert.match(registry,/'trip\.compose'[\s\S]*?maxOutputTokens:5200,reasoningEffort:'none'/);assert.match(registry,/'trip\.audit'[\s\S]*?maxOutputTokens:3500/);checks+=3;
  ...
  assert.match(composer,/candidateBreadthWavesStarted:wave,candidatePoolTarget:poolPlan\.target[\s\S]*candidateBreadthWavesCompleted:wave/,'every scalable candidate breadth wave must be checkpointed before and after bounded provider work');checks++;
  assert.match(composer,/readAiPlaces\(state,\{categories:refillCategories,refresh:true,focused:true,excludedProviderPlaceIds:result\.places\.map\(providerId\),breadthWave:wave,targetCount:shortage\}\)/,'a thin long-trip catalog must receive exclusion-aware, offset multi-query breadth waves');checks++;
