...
async function providerResume(){
  ...
  assert.equal(starts.length,3);assert.equal(starts[0].payload.idempotencyKey,starts[1].payload.idempotencyKey);assert.equal(starts[1].payload.idempotencyKey,starts[2].payload.idempotencyKey);assert.match(starts[0].payload.idempotencyKey,/^trip-plan-v3:/);assert.deepEqual(starts[0].payload.context,starts[1].payload.context);assert.equal(starts[0].payload.input.days[0].entries[0].evidence.refs[0].id,'evidence-valencia-1');assert.equal(starts[0].payload.workflowId,workflow.id);assert.equal(starts[0].payload.retryFailed,false,'foreground polling must read a failed job instead of silently paying for another attempt');assert.equal(reads,1);checks+=9;
}
...
function contracts(){
  ...
  assert.match(registry,/'planning\.dialogue'[\s\S]*?maxOutputTokens:2800,reasoningEffort:'none'/);assert.match(registry,/'trip\.compose'[\s\S]*?maxOutputTokens:5200,reasoningEffort:'none'/);assert.match(registry,/'trip\.audit'[\s\S]*?maxOutputTokens:2500/);checks+=3;
  ...
  assert.match(composer,/const checkpointCandidates=result\.places\.map\(retainedPoolCandidate\);await checkpointTripWorkflow\(state,'candidates',\{brief:clone\(brief\),candidates:clone\(checkpointCandidates\)/,'the resumable checkpoint stores the full verified reserve');checks++;
  ...
  assert.match(composer,/repairWholeTrip=Boolean\(itinerary\)&&!repairDayDates\.length&&state\.aiDraft\.retryEscalated===true,composeQualityAttempt=repairWholeTrip\|\|resumingTargetedDeepRepair\?3:attempt\+1/,'dated repairs preserve valid days, including when multiple days need repair');checks++;
