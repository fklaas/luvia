...
function contracts(){
  ...
  assert.match(openai,/requestTimeoutMs/);assert.match(openai,/'trip\.compose':65_000/);assert.match(openai,/signal:controller\.signal/);assert.match(openai,/OPENAI_TIMEOUT/);checks+=4;
  assert.match(jobs,/TRIP_JOB_LEASE_TIMEOUT_MS=90_000/);assert.ok((jobs.match(/Date\.now\(\)-TRIP_JOB_LEASE_TIMEOUT_MS/g)||[]).length===2,'both stale-job recovery paths must use the bounded server lease');checks+=2;
  ...
  assert.match(registry,/'planning\.dialogue'[\s\S]*?maxOutputTokens:4000/);assert.match(registry,/'trip\.compose'[\s\S]*?maxOutputTokens:9000/);assert.match(registry,/'trip\.audit'[\s\S]*?maxOutputTokens:3500/);checks+=3;
  ...
  assert.match(composer,/localCandidates=Array\.isArray\(resumed\?\.places\)[\s\S]*?:localCandidates,cachedCoverage=/,'a fresh workflow must reuse the locally persisted Places catalog before paying for another discovery pass');checks++;
  assert.match(composer,/candidateCountRefillAttempted:true,candidateTarget/,'a count-based candidate refill must be checkpointed before and after bounded provider work');checks++;
  assert.match(composer,/readAiPlaces\(state,\{categories:refillCategories,refresh:true,focused:true\}\)/,'a nearly sufficient long-trip catalog must receive one focused multi-query refill');checks++;
  ...
  assert.match(composer,/resumingEscalatedRepair=Boolean\(itinerary\)&&state\.aiDraft\.retryEscalated===true,resumingTargetedRepair=Boolean\(itinerary\)&&repairDayDates\.length===1&&!resumingEscalatedRepair/,'a preserved single failed day stays targeted while an exhausted user-authorized retry enters the strong holistic lane');checks++;
  assert.match(composer,/repairWholeTrip=Boolean\(itinerary\)&&\(repairDayDates\.length>1\|\|state\.aiDraft\.retryEscalated===true\),composeQualityAttempt=repairWholeTrip\?3:attempt\+1/,'several failed days and exhausted retries must use one strongest-tier holistic composition instead of one model call per day');checks++;
  ...
  assert.match(composer,/AI_PLANNING_PHASES=Object\.freeze/);assert.match(composer,/data-ftc-ai-elapsed/);assert.match(composer,/Das dauert länger als üblich/);checks+=3;
