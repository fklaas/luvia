...
assert.equal(core.listRoutes().length,19);
assert.equal(core.get('hotels').mount.targetId,'accommodations-module');
assert.equal(core.get('hotels').mount.owner,'booking');
assert.equal(core.get('timeline').mount.owner,'journey');
assert.equal(core.get('timeline').mount.mode,'inline');
