...
async function proxyPlaceElements(center:{latitude:number;longitude:number},radius:number,category:string,strictType:string){
  ...
  const response=await providerFetch('openstreetmap','search',1,endpoint,{method:'POST',headers:{'Content-Type':'application/json','X-Luvia-Provider-Token':tokenValue},body:JSON.stringify({latitude:center.latitude,longitude:center.longitude,radius,category,strictType}),signal:AbortSignal.timeout(10000)});
  const body=await response.json().catch(()=>({}));
