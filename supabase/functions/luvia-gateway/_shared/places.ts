...
if(action==='places.text-search'){
  ...
  if(wantsLegacy&&!processed.length){
    ...
    if(providers.includes('google')&&getKey()){
      ...
      catch(error:any){fallbackReason=isQuotaError(error)?'google_quota':'google_failed';providerErrors.push({provider:'google',message:error?.message||String(error),code:error?.code||'PROVIDER_ERROR',status:Number(error?.status)||null,reason:error?.reason||null})}
