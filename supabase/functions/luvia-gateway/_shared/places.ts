...
const HEALTH_PROBES=Object.freeze({
  ...
  'chinese-scharbeutz':Object.freeze({query:'Chinesisch Restaurant',destination:Object.freeze({name:'Scharbeutz',countryCode:'DE',location:Object.freeze({latitude:54.0214,longitude:10.7536}),canonicalCity:Object.freeze({name:'Scharbeutz'}),searchRadiusMeters:5000}),options:Object.freeze({providers:Object.freeze(['auto']),category:'food',includedType:'chinese_restaurant',includedTypes:Object.freeze(['chinese_restaurant']),strictPlaceType:'chinese_restaurant',strictTypeFiltering:true,maxDistanceMeters:5000})}),
  'spanish-scharbeutz':Object.freeze({query:'Spanisch Restaurant',destination:Object.freeze({name:'Scharbeutz',countryCode:'DE',location:Object.freeze({latitude:54.0214,longitude:10.7536}),canonicalCity:Object.freeze({name:'Scharbeutz'}),searchRadiusMeters:5000}),options:Object.freeze({providers:Object.freeze(['auto']),category:'food',includedType:'spanish_restaurant',includedTypes:Object.freeze(['spanish_restaurant']),strictPlaceType:'spanish_restaurant',strictTypeFiltering:true,maxDistanceMeters:5000,forceRefresh:true})}),
...
function exactMediaCandidate(place:any,candidate:any){return Boolean(exactMediaName(place?.name)&&exactMediaName(candidate?.name)===exactMediaName(place?.name)&&candidate?.location&&place?.location&&(distanceMeters(candidate.location,place.location)??Infinity)<=120)}
...
async function geoapifyPlacesSearch(textQuery:string,destination:any,options:any,restriction:any,bias:any){
  ...
  const matchesCuisineEvidence=(place:any)=>!cuisines.length||cuisines.some((type:string)=>place?.types?.includes(type));
...
if(action==='places.text-search'){
  ...
  if(wantsLegacy&&!processed.length){
    ...
    if(useFoursquare){attempted.push('foursquare');try{foursquarePlaces=await foursquareSearch(String(payload?.query||textQuery),effectiveDestination,options);answered.push('foursquare')}catch(error:any){providerErrors.push({provider:'foursquare',message:error?.message||String(error),code:error?.code||'PROVIDER_ERROR'})}}
...
if(ttl>0&&!searchEmpty)store(key,result,ttl);return{data:result,cache:{hit:false,key,ttlMs:searchEmpty?0:ttl,skippedEmpty:searchEmpty,forced:forceRefresh}};}
