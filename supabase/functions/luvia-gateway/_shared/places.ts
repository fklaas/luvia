...
const searchFields=(options:any={})=>{const viewport=Boolean(options?.locationRestriction?.rectangle||options?.locationBias?.rectangle);if(!viewport)return SEARCH_FIELDS;const fields=[VIEWPORT_SEARCH_FIELDS];if(options?.openNow||Number(options?.minRating)>0||(Array.isArray(options?.priceLevels)&&options.priceLevels.length))fields.push(VIEWPORT_ENTERPRISE_FIELDS);if(options?.vegetarianOnly)fields.push(VIEWPORT_ATMOSPHERE_FIELDS);return fields.join(',');};
