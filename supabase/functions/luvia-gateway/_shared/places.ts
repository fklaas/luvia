import {FOURSQUARE_API_VERSION,FOURSQUARE_MAPPING_VERSION,FOURSQUARE_RESTAURANT_CATEGORY_ID,FOURSQUARE_PRO_FIELDS,FOURSQUARE_SEARCH_FIELDS,FOURSQUARE_DETAILS_FIELDS,normalizeFoursquarePlace,boundedFoursquareError} from './foursquare-place-mapping.ts';
...
async function foursquareWithFieldFallback(path:string,params:Record<string,unknown>,fallbackFields:string[]){
  try{return await foursquare(path,params)}catch(error:any){
    ...
    return foursquare(path,{...params,fields:fallbackFields.join(',')});
  }
}
...
function foursquareCategoryFilter(options:any){const explicit=Array.isArray(options?.foursquareCategoryIds)?options.foursquareCategoryIds.join(','):String(options?.foursquareCategoryIds||'');const safe=explicit.split(',').map((value:string)=>value.trim()).filter((value:string)=>/^\d+$/.test(value)).slice(0,12);if(safe.length)return safe.join(',');return options?.strictTypeFiltering===true&&String(options?.includedType||'').toLowerCase()==='restaurant'?FOURSQUARE_RESTAURANT_CATEGORY_ID:undefined;}
...
if(action==='places.health'){result={status:'ok',service:'multi-provider-places-gateway',version:'4.29.0',configured:Boolean(getKey()||getFoursquareKey()),providers:{google:{configured:Boolean(getKey())},foursquare:{configured:Boolean(getFoursquareKey()),apiVersion:FOURSQUARE_API_VERSION,mappingVersion:FOURSQUARE_MAPPING_VERSION,coordinateSchema:'top-level-latitude-longitude',premiumFieldsOptional:true,categoryFilteredSearch:true,adaptiveDestinationRadius:true}},metrics:{...metrics},cache:{entries:cache.size}};return{data:result,cache:{hit:false,key:null,ttlMs:0}};}
