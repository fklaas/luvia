...
import {osmDietaryConfiguration,osmDietarySearch,osmPlacesSearch} from './osm-dietary.ts';
...
const CATEGORY_HEALTH_TYPES=Object.freeze([
  ['food','Restaurants Cafés Bars Essen','restaurant',['restaurant','cafe','bakery','bar','meal_takeaway','vegetarian_restaurant','vegan_restaurant'],[],false,3000],
  ['activities','Aktivitäten Erlebnisse Freizeit','',['amusement_park','amusement_center','aquarium','bowling_alley','escape_room','fitness_center','gym','playground','skating_rink','spa','sports_activity_location','stadium','swimming_pool','water_park','zoo','tourist_attraction'],['hospital','store','locality','restaurant','cafe','bakery','meal_takeaway','food_court','catering','lodging','hotel','hostel'],false,3000],
  ['themeparks','Freizeitparks Themenparks Erlebnisparks','amusement_park',['amusement_park','amusement_center','water_park'],['hospital','store','locality','restaurant','cafe','bakery','meal_takeaway','catering','lodging','hotel'],true,15000],
  ['wellness','Wellness Spa Sauna Erholung','spa',['spa'],['hospital','locality','restaurant','cafe','bakery','meal_takeaway','catering','lodging','hotel'],true,15000],
  ['water','Wassererlebnisse Schwimmen Strand Wassersport','',['water_park','swimming_pool','beach','marina'],['hospital','locality','restaurant','cafe','bakery','meal_takeaway','catering','lodging','hotel'],false,5000],
  ['sights','Sehenswürdigkeiten Wahrzeichen besondere Orte','tourist_attraction',['tourist_attraction','historical_landmark','monument','observation_deck'],['restaurant','hospital'],false,5000],
  ['photo','Fotospots besondere Perspektiven Aussicht Architektur','tourist_attraction',['tourist_attraction','historical_landmark','monument','observation_deck','park','garden','natural_feature'],['hospital'],false,5000],
  ['culture','Museen Kino Theater Kultur','',['museum','movie_theater','art_gallery','performing_arts_theater','concert_hall'],['hospital','restaurant'],false,5000],
  ['nature','Parks Gärten Natur Erholung','park',['park','garden','beach','hiking_area','natural_feature'],['store','hospital','lodging','accommodation','hotel','hostel','motel','guest_house','bed_and_breakfast','apartment','vacation_rental','holiday_home','resort_hotel','campground','spa'],false,3000],
  ['shopping','Shopping Märkte besondere Geschäfte','',['shopping_mall','market','store','clothing_store','department_store'],['hospital'],false,3000],
  ['malls','Einkaufszentren Shopping Center','shopping_mall',['shopping_mall','department_store'],['hospital'],true,15000],
  ['nightlife','Bars Pubs Cocktailbars Weinbars Lounges Clubs Diskotheken Live-Musik Karaoke Comedy Casinos Nachtleben','',['nightlife_spot','bar','pub','cocktail_bar','wine_bar','night_club','dance_club','discotheque','lounge_bar','concert_hall','live_music_venue','music_venue','jazz_club','comedy_club','karaoke_bar','casino'],['hospital','locality'],false,5000],
  ['practical','Apotheke Supermarkt Parkplatz Toilette Ladestation','',['pharmacy','supermarket','parking','electric_vehicle_charging_station','gas_station','atm','laundry'],['tourist_attraction'],false,5000],
  ['accommodation','Hotels Unterkünfte Hostels Apartments','lodging',['lodging','hotel','hostel','motel','bed_and_breakfast','guest_house','resort_hotel','campground','apartment','vacation_rental','holiday_home'],['restaurant','hospital','locality'],false,15000]
]);
const CATEGORY_HEALTH_PROBES=Object.fromEntries(CATEGORY_HEALTH_TYPES.map(([category,query,includedType,includedTypes,excludedTypes,strictTypeFiltering,radius])=>[
  `category-${category}-scharbeutz`,
  Object.freeze({query,destination:Object.freeze({...SCHARBEUTZ_CUISINE_DESTINATION,searchRadiusMeters:radius}),options:Object.freeze({providers:Object.freeze(['auto']),category,includedType,includedTypes:Object.freeze(includedTypes),excludedTypes:Object.freeze(excludedTypes),strictTypeFiltering,strictDestination:true,maxResultCount:50,maxDistanceMeters:radius})})
]));
const HEALTH_PROBES=Object.freeze({
  ...CUISINE_HEALTH_PROBES,
  ...CATEGORY_HEALTH_PROBES,
  ...
  'beach-supplies-scharbeutz':Object.freeze({query:'Strandbedarf Surf Shop',destination:Object.freeze({name:'Scharbeutz',countryCode:'DE',location:Object.freeze({latitude:54.0214,longitude:10.7536}),canonicalCity:Object.freeze({name:'Scharbeutz'}),searchRadiusMeters:15000}),options:Object.freeze({providers:Object.freeze(['auto']),category:'shopping',includedType:'sporting_goods_store',includedTypes:Object.freeze(['sporting_goods_store','toy_store','department_store']),strictTypeFiltering:true,strictDestination:true,maxResultCount:50,maxDistanceMeters:15000})}),
  ...
  'shopping-scharbeutz':Object.freeze({query:'Geschäfte Märkte Kaufhäuser Einkaufszentren',destination:Object.freeze({name:'Scharbeutz',countryCode:'DE',location:Object.freeze({latitude:54.0214,longitude:10.7536}),canonicalCity:Object.freeze({name:'Scharbeutz'}),searchRadiusMeters:3000}),options:Object.freeze({providers:Object.freeze(['auto']),category:'shopping',includedType:'store',includedTypes:Object.freeze(['store','shopping_mall','department_store','market','supermarket','clothing_store','gift_shop','sporting_goods_store']),strictTypeFiltering:false,strictDestination:true,maxResultCount:50,maxDistanceMeters:3000})}),
  'nightlife-scharbeutz':Object.freeze({query:'Bars Clubs Diskotheken Nachtleben',destination:Object.freeze({name:'Scharbeutz',countryCode:'DE',location:Object.freeze({latitude:54.0214,longitude:10.7536}),canonicalCity:Object.freeze({name:'Scharbeutz'}),searchRadiusMeters:5000}),options:Object.freeze({providers:Object.freeze(['auto']),category:'nightlife',includedType:'bar',includedTypes:Object.freeze(['bar','pub','night_club','cocktail_bar','wine_bar','live_music_venue']),strictTypeFiltering:false,strictDestination:true,maxResultCount:50,maxDistanceMeters:5000})})
});
...
const accommodationIdentity=(place:any)=>[place?.primaryType,place?.primary_type,...(place?.types||[])].map(normalizedIdentityText).some((type:string)=>['lodging','accommodation','hotel','hostel','motel','guest house','bed and breakfast','apartment','vacation rental','holiday home','resort hotel','campground'].includes(type));
function canonicalDuplicate(left:any,right:any){
  ...
  if(meters!=null&&(meters<=25||meters<=350&&accommodationIdentity(left)&&accommodationIdentity(right)))return true;
...
export async function enrichExactGoogleMedia(place:any){
  if(!place||place.photos?.length||!getKey()||!exactMediaName(place.name)||!coordinate(place.location))return place;
  metrics.providers.google.exactMediaAttempts++;
  try{
    const center=coordinate(place.location),body=cleanObject({textQuery:String(place.name),languageCode:'de',regionCode:String(place.countryCode||'DE'),maxResultCount:3,locationBias:{circle:{center,radius:1000}}});
    const raw=await google('/places:searchText',{method:'POST',body:JSON.stringify(body)},'places.id,places.displayName,places.location,places.photos');
    const candidates=(raw.places||[]).map((candidate:any)=>normalizedPlace(candidate)).filter((candidate:any)=>exactMediaCandidate(place,candidate));
    const match=candidates.find((candidate:any)=>candidate.photos?.length)||null;
    if(!match){metrics.providers.google.exactMediaMisses++;return place}
    metrics.providers.google.exactMediaMatches++;
    const googleId=String(match.id||'').replace(/^places\//,''),entityReference=String(place.providerPlaceId||place.id||'');
    return{...place,photos:match.photos.slice(0,3).map((photo:any)=>({...photo,provider:'google',entityReference,linkedEntityReference:`google:${googleId}`,verified:true})),providerRefs:{...(place.providerRefs||{}),google:googleId},evidence:[...(place.evidence||[]),{provider:'google',kind:'exact-name-and-coordinate-photo-match',providerPlaceId:googleId,maxDistanceMeters:120}]};
  }catch{metrics.providers.google.exactMediaFailures++;return place}
}
...
async function enrichExactSelectedMedia(place:any){
  if(!place||place.photos?.length)return place;
  const google=await enrichExactGoogleMedia(place);
  return google?.photos?.length?google:enrichExactFoursquareMedia(google);
}
...
async function geoapifyPlaceDetails(rawId:string,options:any={}){
  ...
  const seed=options?.providerPlaceSeed&&typeof options.providerPlaceSeed==='object'?options.providerPlaceSeed:null,seedId=String(seed?.providerPlaceId||seed?.id||'').replace(/^places\//,''),exactSeed=seed&&seedId===rawId?seed:null;
  const seedMediaKey=exactSeed?hash({id:seedId,photos:(exactSeed.photos||[]).map((photo:any)=>photo?.uri||photo?.name||photo?.photoReference||''),linked:exactSeed.raw?.wiki_and_media||null}):'none';
  const cacheKey=`geoapify-details-media-v4:${rawId}:${options.enrichMedia===true}:${seedMediaKey}`,hit=cached(cacheKey);if(hit)return hit;
  ...
  const linked=await enrichLinkedMedia(exactPlace),enriched=options.enrichMedia===true&&!linked.photos?.length?await enrichExactSelectedMedia(linked):linked;store(cacheKey,enriched,30*60_000);return enriched;
}
...
function postProcessPlaces(places:any[],destination:any,options:any){
  ...
  if(options?.strictTypeFiltering===true&&requestedTypes.length){const accepted=[...new Set(requestedTypes)];list=list.filter(place=>{const types=[place?.primaryType,...(Array.isArray(place?.types)?place.types:[]),...(Array.isArray(place?.providerNativeTypes)?place.providerNativeTypes:[])].map(value=>String(value||'').toLowerCase().replace(/[^a-z0-9]+/g,'_').replace(/^_+|_+$/g,'')).filter(Boolean);return accepted.some(rule=>rule==='vegetarian_restaurant'&&place?.features?.servesVegetarianFood===true||rule==='vegan_restaurant'&&place?.features?.servesVeganFood===true||types.some(type=>type===rule||type.startsWith(`${rule}_`)||type.endsWith(`_${rule}`)||rule==='restaurant'&&type.endsWith('_restaurant')||rule==='store'&&type.endsWith('_store')))});}
...
if(action==='places.health'){
  ...
  if(probe){
    try{
      const response=await placesAction('places.text-search',{query:probe.query,destination:probe.destination,options:{providers:['auto'],maxResultCount:20,strictDestination:true,languageCode:'de',regionCode:'DE',...(probe.options||{})}});
      const places=(response.data?.places||[]).slice(0,12).map((place:any)=>({providerPlaceId:String(place?.providerPlaceId||place?.id||'').slice(0,160),name:String(place?.name||place?.displayName||'').slice(0,160),primaryType:String(place?.primaryType||'').slice(0,120),primaryTypeLabel:String(place?.primaryTypeLabel||'').slice(0,160),provider:String(place?.provider||'').slice(0,80),providerNativeTypes:(place?.providerNativeTypes||[]).slice(0,12).map((value:any)=>String(value).slice(0,120)),providerPrimaryFoodTypes:(place?.raw?.foodTypes||[]).filter((value:any)=>value?.primary===true).slice(0,8).map((value:any)=>String(value?.name||value?.id||'').slice(0,120)),types:(place?.types||[]).slice(0,12).map((value:any)=>String(value).slice(0,120)),servesVegetarianFood:place?.features?.servesVegetarianFood===true,servesVeganFood:place?.features?.servesVeganFood===true,photoCount:Array.isArray(place?.photos)?place.photos.length:0,mediaStatus:String(place?.mediaStatus||'search-result').slice(0,80),mediaProviders:[...new Set((place?.photos||[]).map((photo:any)=>String(photo?.provider||place?.provider||'unknown').slice(0,40)))],location:coordinate(place?.location)}));
      diagnosticProbe={key:requestedProbe,status:'ok',query:probe.query,destination:probe.destination.name,providers:response.data?.providers||null,mediaCoverage:{total:places.length,seeded:places.filter((place:any)=>place.photoCount>0).length,missing:places.filter((place:any)=>place.photoCount===0).length,providers:[...new Set(places.flatMap((place:any)=>place.mediaProviders))]},places};
    }catch(error:any){
