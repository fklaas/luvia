...
const CUISINE_HEALTH_PROBES=Object.fromEntries(CUISINE_HEALTH_TYPES.map(([slug,label,type])=>[
  `${slug}-scharbeutz`,
  Object.freeze({query:`${label} Restaurant Scharbeutz`,destination:SCHARBEUTZ_CUISINE_DESTINATION,options:Object.freeze({providers:Object.freeze(['vegetarian_restaurant','vegan_restaurant'].includes(type)?['geoapify','google','foursquare']:['auto']),category:'food',includedType:type,includedTypes:Object.freeze([type]),strictPlaceType:type,strictTypeFiltering:true,strictDestination:true,maxResultCount:50,maxDistanceMeters:5000,vegetarianOnly:type==='vegetarian_restaurant'})})
]));
...
async function geoapifyPlaceDetails(rawId:string,options:any={}){
  ...
  const cacheKey=`geoapify-details-media-v2:${rawId}:${options.enrichMedia===true}`,hit=cached(cacheKey);if(hit)return hit;
...
if(action==='places.health'){
  ...
  if(probe){
    try{
      ...
      diagnosticProbe={key:requestedProbe,status:'failed',query:probe.query,destination:probe.destination.name,error:{code:String(error?.code||'PROBE_FAILED').slice(0,80),message:String(error?.message||'Diagnose fehlgeschlagen.').slice(0,240)},providerErrors:(error?.providerErrors||[]).slice(0,4).map((item:any)=>({provider:String(item?.provider||'unknown').slice(0,40),code:String(item?.code||'PROVIDER_ERROR').slice(0,80),reason:String(item?.reason||'unknown').slice(0,80),message:String(item?.message||'Provider fehlgeschlagen.').slice(0,240)})),places:[]};
    }
  }
  result={status:'ok',service:'multi-provider-places-gateway',version:'4.37.6-dietary-conflict-guard',configured:Boolean(getGeoapifyKey()||getKey()||getFoursquareKey()||Deno.env.get('TOMTOM_API_KEY')||Deno.env.get('HERE_API_KEY')),providerOrder:'free_budget_cascade',providers:{geoapify:{configured:Boolean(getGeoapifyKey()),priority:'primary',coordinateSchema:'top-level-latitude-longitude'},tomtom:{configured:Boolean(Deno.env.get('TOMTOM_API_KEY')),priority:'automatic_fallback',credentialExposure:false},here:{configured:Boolean(Deno.env.get('HERE_API_KEY')),priority:'automatic_fallback',credentialExposure:false},google:{configured:Boolean(getKey()),priority:'bounded_dietary_evidence_opt_in'},foursquare:{configured:Boolean(getFoursquareKey()),priority:'on_demand_dietary_fallback_and_exact_selected_media',apiVersion:FOURSQUARE_API_VERSION,mappingVersion:FOURSQUARE_MAPPING_VERSION,coordinateSchema:'top-level-latitude-longitude',premiumFieldsOptional:true,categoryFilteredSearch:'explicit-reviewed-taxonomy-only',postRetrievalCategoryEvidence:true,adaptiveDestinationRadius:true,exactMediaIdentity:'normalized_name_and_max_120m',aliasMediaIdentity:'contained_distinctive_name_and_max_25m',photoEndpoint:'one_popular_photo_after_exact_identity'}},diagnosticProbe,availableDiagnosticProbes:Object.keys(HEALTH_PROBES),metrics:{...metrics},cache:{entries:cache.size}};return{data:result,cache:{hit:false,key:null,ttlMs:0}};
}
if(action==='places.text-search'){
  ...
  if(wantsLegacy&&!processed.length){
    ...
    if(providers.includes('google')&&getKey()){
      ...
      catch(error:any){fallbackReason=isQuotaError(error)?'google_quota':'google_failed';providerErrors.push({provider:'google',message:error?.message||String(error),code:error?.code||'PROVIDER_ERROR',reason:error?.reason||null})}
