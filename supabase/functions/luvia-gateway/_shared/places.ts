...
async function foursquareSearch(query:string,destination:any,options:any){const anchor=searchAnchor(destination,options);const near=destination?.canonicalCity?.name||destination?.name||'';const params:any={query,limit:Math.min(20,Number(options?.maxResultCount||10)),sort:'RELEVANCE',fields:FOURSQUARE_SEARCH_FIELDS.join(',')};if(anchor)params.ll=`${anchor.latitude},${anchor.longitude}`;else if(near)params.near=near;if(Number(options?.maxDistanceMeters)>0)params.radius=Math.min(100000,Number(options.maxDistanceMeters));if(options?.openNow)params.open_now=true;const raw=await foursquareWithFieldFallback('/places/search',params,[...FOURSQUARE_PRO_FIELDS,'distance']);const rows=raw.results||raw.places||[];return rows.map((place:any)=>normalizeFoursquarePlace(place,{evidenceKind:'place-search'}));}
...
if(action==='places.health'){result={status:'ok',service:'multi-provider-places-gateway',version:'4.28.8',configured:Boolean(getKey()||getFoursquareKey()),providers:{google:{configured:Boolean(getKey())},foursquare:{configured:Boolean(getFoursquareKey()),apiVersion:FOURSQUARE_API_VERSION,mappingVersion:FOURSQUARE_MAPPING_VERSION,coordinateSchema:'top-level-latitude-longitude',premiumFieldsOptional:true}},metrics:{...metrics},cache:{entries:cache.size}};return{data:result,cache:{hit:false,key:null,ttlMs:0}};}
