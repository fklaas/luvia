...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  const bookingType=clean(booking.booking_type).toLowerCase(),hotel=['hotel','accommodation','lodging'].includes(bookingType),admission=['activity','attraction','culture','event'].includes(bookingType);const request=booking.request||{};
  const intro=bookingType==='restaurant'?'ich möchte gerne einen Tisch in Ihrem Restaurant reservieren.':hotel?'ich möchte gerne die Verfügbarkeit und den Gesamtpreis für einen Aufenthalt anfragen.':admission?'ich möchte gerne Verfügbarkeit, Einlassbedingungen und Preise für den gewünschten Besuch anfragen.':'ich möchte gerne eine Buchung anfragen.';
  const lines=['Guten Tag,','',intro,'',`${hotel?'Anreise':'Datum'}: ${startDate?fDate(startDate):'noch offen'}`];if(startDate&&!hotel)lines.push(`Uhrzeit: ${fTime(startDate)}`);if(hotel&&booking.end_at)lines.push(`Abreise: ${fDate(new Date(booking.end_at))}`);if(hotel){lines.push(`Zimmer: ${request.rooms||request.roomCount||1}`,`Erwachsene: ${request.adults||booking.party_size||1}`);if(Number(request.children||0)>0)lines.push(`Kinder: ${request.children}${Array.isArray(request.childAges)&&request.childAges.length?` (Alter: ${request.childAges.join(', ')})`:''}`);if(request.roomType)lines.push(`Zimmertyp: ${clean(request.roomType)}`);if(request.board)lines.push(`Verpflegung: ${clean(request.board)}`)}else lines.push(`Personen: ${booking.party_size||1}`);lines.push(`Name: ${requesterName}`);if(occasion&&occasion!=='Kein besonderer Anlass')lines.push(`Anlass: ${occasion}`);if(note)lines.push('',`Hinweis: ${note}`);lines.push('',hotel?'Bitte nennen Sie Verfügbarkeit, Gesamtpreis, Zahlungs- und Stornobedingungen.':admission?'Bitte nennen Sie Verfügbarkeit, Ticket-/Reservierungspflicht, Gesamtpreis und Stornobedingungen.':'Bitte bestätigen Sie uns kurz, ob die Buchung möglich ist.','','Vielen Dank und freundliche Grüße','',requesterName,'Buchungsanfrage über Luvia');
  const subject=`${hotel?'Aufenthaltsanfrage':admission?'Ticket-/Reservierungsanfrage':'Buchungsanfrage'} · ${booking.title}`;const fromDefault=clean(Deno.env.get('BOOKING_EMAIL_FROM')||'Luvia Booking <booking@booking.myluvia.app>');
