...
Deno.serve(async(req:Request)=>{
  ...
  if(PUBLIC.has(action))return jsonResponse(200,{ok:true,data:{service:'luvia-intelligence',status:'ok',version:'4.38.0',build:'13.38.0',core:'4.38.0',authenticated:Boolean(userId),...modelDiagnostics(),capabilities:listCapabilities(),calendarEvidence:{provider:'OpenHolidays API',schoolRegions:'DE-16',officialReference:'KMK'},structuredOutput:{incompleteDetection:true,tripComposeTokens:16000,travelPromise:true,uncertaintyMap:true,bookingOrder:true,neighborhoodRecommendation:true},privacy:{store:false,promptsLogged:false,minimumNecessaryContext:true}},meta:{requestId:id}},cors);
