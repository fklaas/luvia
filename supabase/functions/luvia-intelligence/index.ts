...
Deno.serve(async(req:Request)=>{
  ...
  if(PUBLIC.has(action))return jsonResponse(200,{ok:true,data:{service:'luvia-intelligence',status:'ok',version:'4.38.2',build:'13.38.2',core:'4.38.2',authenticated:Boolean(userId),...modelDiagnostics(),capabilities:listCapabilities(),calendarEvidence:{provider:'OpenHolidays API',schoolRegions:'DE-16',officialReference:'KMK'},structuredOutput:{incompleteDetection:true,structuredOutputEscalation:true,tripComposeTokens:16000,planningDialogueTokens:6000,adaptiveQualityCascade:true,travelPromise:true,uncertaintyMap:true,bookingOrder:true,neighborhoodRecommendation:true},privacy:{store:false,promptsLogged:false,minimumNecessaryContext:true}},meta:{requestId:id}},cors);
