...
import { sanitize, sanitizeTripPayload, byteLength, safetyIdentifier } from './policies/privacy.ts';
import { modelDiagnostics, runOpenAI } from './providers/openai.ts';
import { recordUsage } from './telemetry/usage.ts';
import { WEB_RESEARCH_CAPABILITY, WEB_RESEARCH_POLICY, researchEnabled } from './providers/web-research.ts';
...
Deno.serve(async(req:Request)=>{
  ...
  if(byteLength(body)>(String(body.action||'').startsWith('trip.plan-workflow.')?1_500_000:180_000))return errorResponse(413,'PAYLOAD_TOO_LARGE','Der KI-Kontext ist zu groß.',id,cors);
  ...
  const requestedTier=String(body.payload?.tier||definition.tier);const tier=(['fast','default','deep'].includes(requestedTier)?requestedTier:definition.tier) as 'fast'|'default'|'deep';const sanitizeInput=capabilityId.startsWith('trip.')?sanitizeTripPayload:sanitize;const input=sanitizeInput(body.payload?.input||{}),context=sanitize(body.payload?.context||{});const started=performance.now();
  try{
