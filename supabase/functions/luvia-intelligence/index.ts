...
import { checkpointTripPlanWorkflow, readTripPlanJob, readTripPlanWorkflow, startTripPlanJob, startTripPlanWorkflow, TRIP_WORKFLOW_BUDGET } from './jobs/trip-plan.ts';
...
Deno.serve(async(req:Request)=>{
  ...
  if(PUBLIC.has(action))return jsonResponse(200,{ok:true,data:{service:'luvia-intelligence',status:'ok',version:'4.40.2',build:'13.40.2',core:'4.40.2',authenticated:Boolean(userId),...modelDiagnostics(),capabilities:listCapabilities(),calendarEvidence:{provider:'OpenHolidays API',schoolRegions:'DE-16',officialReference:'KMK'},structuredOutput:{incompleteDetection:true,structuredOutputEscalation:true,tripComposeTokens:12000,tripDayRepairTokens:6000,planningDialogueTokens:6000,destinationInspirationTier:'fast',destinationInspirationTokens:1200,adaptiveQualityCascade:true,compactTripCompose:true,targetedDayRepair:true,resumableTripWorkflow:true,resumableTripJobs:true,singleActiveCapabilityJob:true,workflowBudget:TRIP_WORKFLOW_BUDGET,jobCapabilities:['planning.dialogue','trip.compose','trip.compose-day-repair','trip.audit'],workflowPhases:['understanding','candidates','itinerary','repair','audit','ready-for-review'],jobTtlHours:24,repairTiers:['fast','default','deep'],travelPromise:true,uncertaintyMap:true,bookingOrder:true,neighborhoodRecommendation:true,semanticConflictModerator:true,movementScope:true,visiblePlanningAssumptions:true,deferredLiveChecks:true,spatialDiversityAudit:true},privacy:{providerStore:false,transientJobStore:true,jobTtlHours:24,completedInputDiscarded:true,promptsLogged:false,minimumNecessaryContext:true}},meta:{requestId:id}},cors);
