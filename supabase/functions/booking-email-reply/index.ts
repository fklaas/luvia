...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  const {data:threadLookup,error:threadError}=await userClient.from('booking_email_threads').select('*').eq('booking_id',bookingId).maybeSingle();
  if(threadError)return json({error:'EMAIL_THREAD_LOOKUP_FAILED',details:threadError.message},500);
  let thread:any=threadLookup||null;
  const threadBootstrapAllowed=!thread&&['modify','cancel'].includes(action);
  if(!thread&&!threadBootstrapAllowed)return json({ok:false,expected:true,error:'EMAIL_THREAD_REQUIRED'},200);
  const intendedRecipient=clean(booking.contact?.email).toLowerCase();
  ...
  if(!thread&&threadBootstrapAllowed){
   const inboundDomain=clean(Deno.env.get('BOOKING_INBOUND_DOMAIN')||'booking.myluvia.app');
   const replyAlias=`booking-${booking.id}@${inboundDomain}`;
   const {data:createdThread,error:createThreadError}=await admin.from('booking_email_threads').upsert({booking_id:booking.id,trip_id:booking.trip_id,transport_provider:'resend',reply_alias:replyAlias,state:'awaiting_reply',last_activity_at:new Date().toISOString(),updated_at:new Date().toISOString()},{onConflict:'booking_id'}).select('*').single();
   if(createThreadError)return json({error:'MUTATION_EMAIL_THREAD_BOOTSTRAP_FAILED',details:createThreadError.message},500);
  ...
  if(existing)return json({ok:true,idempotent:true,message:existing,bookingId,threadId:thread.id,threadBootstrapped,mutationThreadBootstrap,mode,version:VERSION});
  const headers:Record<string,string>={};
  if(!mutationThreadBootstrap&&clean(lastMessage?.message_id_header))headers['In-Reply-To']=clean(lastMessage.message_id_header);
  if(!mutationThreadBootstrap&&clean(lastMessage?.references_header))headers['References']=clean(lastMessage.references_header);
  else if(!mutationThreadBootstrap&&clean(lastMessage?.message_id_header))headers['References']=clean(lastMessage.message_id_header);
  const resendBody:any={from:fromDefault,to:[actualRecipient],subject,text:bodyText,reply_to:thread.reply_alias,tags:[{name:'booking_id',value:booking.id},{name:'booking_thread_id',value:thread.id},{name:'booking_action',value:action||'reply'}]};
  ...
  if(!resendResponse.ok){if(mutationThreadBootstrap)await admin.from('booking_email_threads').update({state:'delivery_failed',last_activity_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq('id',thread.id);return json({error:'RESEND_REPLY_FAILED',message:'Der E-Mail-Anbieter hat die Antwort abgelehnt.',details:resendPayload,resendStatus:resendResponse.status,from:fromDefault,intendedRecipient,actualRecipient,threadBootstrapped,mutationThreadBootstrap},502);}
  const templateKey=mutationThreadBootstrap?`booking.mutation.${action}.v1`:'booking.thread.reply.v1';
  const {data:storedMessage,error:messageError}=await userClient.rpc('luvia_booking_record_message',{p_booking_id:booking.id,p_direction:'outbound',p_channel:'email',p_transport_provider:'resend',p_sender:fromDefault,p_recipient:actualRecipient,p_intended_recipient:intendedRecipient,p_actual_recipient:actualRecipient,p_subject:subject,p_body_text:bodyText,p_template_key:templateKey,p_provider_message_id:resendPayload.id,p_provider_thread_id:thread.id,p_delivery_status:'sent',p_idempotency_key:idempotencyKey,p_metadata:{mode,replyTo:thread.reply_alias,emailReplyV1:true,mutationThreadBootstrap,threadCreatedForMutation:threadBootstrapped,verifiedCandidateId:verified.candidateId||null,action:action||null,intelligenceId:intelligenceId||null},p_raw_payload:{resend:{id:resendPayload.id}}});
  if(messageError)return json({error:'REPLY_MESSAGE_RECORD_FAILED',details:messageError.message,providerReference:resendPayload.id,threadBootstrapped},500);
  await admin.from('booking_messages').update({email_thread_id:thread.id,correlation_method:mutationThreadBootstrap?'outbound_mutation_thread_bootstrap':'outbound_thread_reply'}).eq('id',storedMessage.id);
  ...
  return json({ok:true,version:VERSION,provider:'resend',providerReference:resendPayload.id,message:storedMessage,intelligence,booking:bookingAfter,threadId:thread.id,threadBootstrapped,mutationThreadBootstrap,replyTo:thread.reply_alias,mode,intendedRecipient,actualRecipient});
