-- Luvia v13.54.3 / Core 4.54.3
-- Failed Status Signal Recovery & Idempotent Retry Fix
...
begin
    ...
    if found then
      ...
      set provider_id=coalesce(nullif(lower(trim(coalesce(p_provider_id,''))),''),provider_id),
          ...
          evidence=coalesce(evidence,'{}'::jsonb)||coalesce(p_evidence,'{}'::jsonb)||jsonb_build_object(
            ...
            'retryCoreVersion','4.54.3'
          ),
          received_at=now(),
          resolution_state='received',
          resolution_reason='RETRYING_PREVIOUSLY_UNAPPLIED_SIGNAL'
  ...
  if v_source in ('provider_webhook','provider_api','provider_polling') then
    ...
    exception when others then
      ...
      set resolution_state='ignored',resolution_reason=sqlerrm,
          evidence=coalesce(evidence,'{}'::jsonb)||jsonb_build_object('lastRetryFailedAt',now(),'retryCoreVersion','4.54.3')
      where id=s.id returning * into s;
      return jsonb_build_object('duplicate',false,'applied',false,'retried',v_retry_existing,'signal',to_jsonb(s));
    end;
  ...
  set resolution_state='applied',resolution_reason=case when v_retry_existing then 'RETRIED_AUTHORITATIVE_SIGNAL_APPLIED' else 'AUTHORITATIVE_SIGNAL_APPLIED' end,
      ...
      evidence=coalesce(evidence,'{}'::jsonb)||jsonb_build_object('retryRecovered',v_retry_existing,'retryCoreVersion','4.54.3')
  where id=s.id returning * into s;
...
begin
  ...
  v_result:=public.luvia_booking_ingest_status_signal_internal(
    ...
    coalesce(r.evidence,'{}'::jsonb)||jsonb_build_object(
      ...
      'trustedInternalBridgeVersion','4.54.3',
  ...
  set resolution_state=case when coalesce((v_result->>'applied')::boolean,false) then 'applied' else 'ignored' end,
      ...
      evidence=evidence||jsonb_build_object('signalRetryAttempted',v_retry_signal,'signalRetryCoreVersion','4.54.3')
  where id=r.id returning * into r;
...
is 'Protected database-only Booking Status V2 resolver. Previously ignored/failed trusted provider signals may be retried only when never applied; applied signals remain strictly idempotent.';

comment on function public.luvia_booking_reprocess_provider_status_receipt_internal(uuid)
is 'Reprocesses provider receipts and can recover a previously emitted but never-applied trusted provider signal without creating a duplicate signal.';
