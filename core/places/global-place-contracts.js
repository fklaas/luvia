(() => {
'use strict';
const VERSION='4.63.0-provider-dietary-evidence';
...
const FILTER_DEFINITIONS=Object.freeze({
  ...
  food:Object.freeze({label:'Restaurant & Küche',types:Object.freeze([['restaurant','Restaurant'],['cafe','Café'],['bar','Bar'],['bakery','Bäckerei'],['meal_takeaway','Imbiss & Take-away'],['food_court','Food Court'],['fine_dining_restaurant','Fine Dining']]),cuisines:Object.freeze([['italian_restaurant','Italienisch'],['german_restaurant','Deutsch'],['mediterranean_restaurant','Mediterran'],['greek_restaurant','Griechisch'],['french_restaurant','Französisch'],['spanish_restaurant','Spanisch'],['indian_restaurant','Indisch'],['asian_restaurant','Asiatisch'],['chinese_restaurant','Chinesisch'],['japanese_restaurant','Japanisch'],['thai_restaurant','Thailändisch'],['vietnamese_restaurant','Vietnamesisch'],['korean_restaurant','Koreanisch'],['mexican_restaurant','Mexikanisch'],['middle_eastern_restaurant','Nahöstlich'],['lebanese_restaurant','Libanesisch'],['turkish_restaurant','Türkisch'],['vegetarian_restaurant','Vegetarisch'],['vegan_restaurant','Vegan']]),subtypes:Object.freeze([]),facts:Object.freeze(['openNow','rating45','vegetarian','reservable','accessible','priceLevel'])}),
...
const FILTER_TYPE_ALIASES=Object.freeze({
  italian_restaurant:['italienisch','italian'],german_restaurant:['deutsch','german'],mediterranean_restaurant:['mediterran','mediterranean'],greek_restaurant:['griechisch','greek'],french_restaurant:['franzosisch','french'],spanish_restaurant:['spanisch','spanish'],indian_restaurant:['indisch','indian'],asian_restaurant:['asiatisch','asian'],chinese_restaurant:['chinesisch','chinese'],japanese_restaurant:['japanisch','japanese'],thai_restaurant:['thailandisch','thai'],vietnamese_restaurant:['vietnamesisch','vietnamese'],korean_restaurant:['koreanisch','korean'],mexican_restaurant:['mexikanisch','mexican'],middle_eastern_restaurant:['nahostlich','nahost','middle eastern','arabisch','arabian'],lebanese_restaurant:['libanesisch','lebanese'],turkish_restaurant:['turkisch','turkish'],vegetarian_restaurant:['vegetarisch','vegetarian'],vegan_restaurant:['vegan'],
  restaurant:['restaurant'],cafe:['cafe'],bar:['bar'],bakery:['backerei','bakery'],meal_takeaway:['imbiss','take away','takeaway'],food_court:['food court'],fine_dining_restaurant:['fine dining'],apartment:['apartment','ferienwohnung'],vacation_rental:['ferienhaus','vacation rental'],campground:['camping','campground'],night_club:['nachtclub','nightclub','club','diskothek','disco'],lounge_bar:['lounge'],live_music_venue:['live musik','livemusik'],movie_theater:['kino','cinema'],performing_arts_theater:['theater'],art_gallery:['galerie','gallery'],observation_deck:['aussicht','aussichtsplattform'],historical_landmark:['historisch','historische'],shopping_mall:['shopping center','einkaufszentrum'],clothing_store:['mode','bekleidung'],department_store:['kaufhaus'],electric_vehicle_charging_station:['ladesaule','ladestation']
});
...
function filterIntent(text='',categoryKey=''){
  const raw=fold(text),q=` ${raw.replace(/[^a-z0-9]+/g,' ')} `,definition=FILTER_DEFINITIONS[categoryKey]||{},available=[...(definition.types||[]),...(definition.subtypes||[]),...(definition.cuisines||[])].filter(([value])=>Boolean(value));
  const queryTokens=new Set(words(raw).flatMap(token=>[token,stem(token)]));
  const contains=term=>{const normalized=fold(term).replace(/[^a-z0-9]+/g,' ').trim();if(!normalized)return false;if(normalized.includes(' '))return q.includes(` ${normalized} `);return queryTokens.has(normalized)||queryTokens.has(stem(normalized))};
  let includedTypes=available.filter(([value,label])=>[label,typeKey(value).replace(/_/g,' '),...(FILTER_TYPE_ALIASES[value]||[])].some(contains)).map(([value])=>value);
  const cuisineTypes=new Set((definition.cuisines||[]).map(([value])=>value)),specific=includedTypes.filter(value=>value!=='restaurant'&&value!=='bar');
  if(specific.some(value=>cuisineTypes.has(value)))includedTypes=includedTypes.filter(value=>value!=='restaurant');
  if(categoryKey==='nightlife'&&specific.length)includedTypes=includedTypes.filter(value=>value!=='bar');
  includedTypes=[...new Set(includedTypes)];
  const vegan=/\bvegan\b/.test(q),vegetarian=vegan||/\bvegetar(?:isch|ian)\w*\b/.test(q),accessible=/\b(?:barrierefrei|rollstuhl\w*|wheelchair|accessible)\b/.test(q),reservable=/\b(?:reservier\w*|reservation|buchbar|tisch reservieren)\b/.test(q),openNow=/\b(?:jetzt geoffnet|gerade geoffnet|aktuell geoffnet|open now|jetzt offen)\b/.test(q),rating45=/(?:mindestens|ab|uber)\s*4[,.]5|4[,.]5\s*(?:sterne|stars)|best(?:e|en)?\s+bewertung/.test(raw),nearby=/\b(?:in der nahe|nahe gelegen|nearby|um die ecke|nachst(?:e|en|er))\b/.test(q);
  if(vegan&&available.some(([value])=>value==='vegan_restaurant')&&!includedTypes.includes('vegan_restaurant'))includedTypes.push('vegan_restaurant');
  else if(vegetarian&&available.some(([value])=>value==='vegetarian_restaurant')&&!includedTypes.includes('vegetarian_restaurant'))includedTypes.push('vegetarian_restaurant');
  const priceLevels=[];
  if(['gunstig','preiswert','budget','inexpensive','billig'].some(term=>queryTokens.has(term))||/(^|\s)€(\s|$)/.test(raw))priceLevels.push('PRICE_LEVEL_INEXPENSIVE');
  if(/(^|\s)€€(\s|$)/.test(raw)||/\b(?:moderat|moderate|mittler(?:e|es|en) preis)\b/.test(q))priceLevels.push('PRICE_LEVEL_MODERATE');
  if(/(^|\s)€€€(\s|$)/.test(raw)||/\b(?:gehoben|expensive|teuer)\b/.test(q))priceLevels.push('PRICE_LEVEL_EXPENSIVE');
  if(/(^|\s)€€€€(\s|$)/.test(raw)||/\b(?:luxus|luxurios|very expensive)\b/.test(q))priceLevels.push('PRICE_LEVEL_VERY_EXPENSIVE');
  const distanceMatch=raw.match(/\b(\d+(?:[,.]\d+)?)\s*(km|kilometer|m|meter)\b/),requestedDistance=distanceMatch?Number(distanceMatch[1].replace(',','.'))*(distanceMatch[2].startsWith('k')?1000:1):null,maxDistanceMeters=Number.isFinite(requestedDistance)?Math.max(250,Math.min(50000,Math.round(requestedDistance))):(nearby?5000:null),sortBy=/\b(?:nachste|nachsten|kurzeste entfernung|distance)\b/.test(q)?'distance':rating45?'rating':'relevance';
  return Object.freeze({explicit:Boolean(includedTypes.length||vegetarian||accessible||reservable||openNow||rating45||priceLevels.length||maxDistanceMeters),includedType:includedTypes.length===1?includedTypes[0]:'',includedTypes:Object.freeze(includedTypes),vegetarianOnly:vegetarian&&!vegan,accessibleOnly:accessible,reservableOnly:reservable,openNow,minRating:rating45?4.5:null,priceLevels:Object.freeze(priceLevels),maxDistanceMeters,sortBy,source:'places-filter-contract-v1'});
}
