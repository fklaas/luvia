(() => {
  ...
  const state={root:null,trip:null,phase:'entry',input:'',category:null,activeGoal:null,pendingGoals:[],question:null,answer:'',results:[],visibleResultCount:INITIAL_VISIBLE_RESULTS,loading:false,error:null,details:new Map(),rejected:new Set(),known:new Map(),sequence:0,lastAction:null,lastError:null,lastStartedAt:null,lastCompletedAt:null};
  ...
  function planningDialog(place){
    ...
    modal.querySelector('form').onsubmit=async e=>{e.preventDefault();const submit=e.submitter;submit.disabled=true;try{const fd=new FormData(e.currentTarget),date=fd.get('date'),time=fd.get('time')||'12:00',plannedAt=new Date(`${date}T${time}:00`).toISOString(),entity=await importEntity(place,'planned'),tp={id:entity?.tripPlaceId},p={id:entity?.id};if(!tp.id)throw new Error('Place-Verknüpfung konnte nicht erstellt werden.');const def=CATEGORY_DEFS[state.activeGoal?.category]||CATEGORY_DEFS.activities;const activeTripId=tripId(state.trip);const fields={planned_at:plannedAt,notes:clean(fd.get('note')),place_name:place.name};await placesContract().plan({tripId:activeTripId,placeType:def.type,tripPlaceId:tp.id,placeId:p.id,fields});await placesContract().updateLifecycle(tp.id,'planned',{}, {tripId:activeTripId});await window.LuviaJourneyContractV1?.commands?.hydrate?.(activeTripId);window.dispatchEvent(new CustomEvent('luvia:place-plan-changed',{detail:{tripId:activeTripId,type:def.type,tripPlaceId:tp.id,placeId:p.id,providerPlaceId:providerId(place),lifecycle:'planned',plannedAt,fields}}));window.dispatchEvent(new CustomEvent('luvia:places-lifecycle-changed',{detail:{tripId:activeTripId,tripPlaceId:tp.id,placeId:p.id,providerPlaceId:providerId(place),lifecycle:'planned',plannedAt}}));window.dispatchEvent(new CustomEvent('luvia:timeline-invalidated',{detail:{tripId:activeTripId,type:def.type,tripPlaceId:tp.id}}));window.dispatchEvent(new CustomEvent('luvia:in-window-data-changed',{detail:{tripId:activeTripId,placeType:def.type,tripPlaceId:tp.id}}));window.dispatchEvent(new CustomEvent('luvia:dashboard-widget-refresh',{detail:{id:'today'}}));mounted.close('saved');toast('Zur Timeline hinzugefügt.');await continueFlow(place)}catch(err){toast(err.message||'Planung fehlgeschlagen.',true)}finally{submit.disabled=false}}
  }
  ...
  function reason(place,index){const opening=(place.currentOpeningHours?.openNow??place.openNow)===true?' Der Ort ist aktuell geöffnet.':'';return index===0?`Dieser Ort passt nach der gemeinsamen Auswertung eurer aktuellen Suche, des Reisekompasses und der belegbaren Ortsdaten am besten zu euch.${opening}`:`Auch diese Alternative erfüllt mehrere belegte Punkte aus eurer Suche und eurem Reisekompass.${opening}`;}function reasonList(place){const items=[...new Set((place._luviaReasons||[]).map(x=>clean(x)).filter(Boolean))].slice(0,4);return items.length?`<div class="places-final-reasons">${items.map(x=>`<p>${esc(x)}</p>`).join('')}</div>`:''}
  ...
  async function mount(root,trip){state.root=root;state.trip=trip;state.phase='entry';state.input='';state.category=null;state.activeGoal=null;state.question=null;state.results=[];state.visibleResultCount=INITIAL_VISIBLE_RESULTS;state.error=null;loadLocal();await loadKnown();render();return true}
  function unmount(){planningHandle?.close('unmount');state.sequence++;state.root=null;state.trip=null;state.results=[];state.details.clear()}
