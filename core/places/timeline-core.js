(() => {
'use strict';
const VERSION='5.8.0-visit-owner-management';
const root=window;
...
function closeSurface(surface,reason='owner'){const target=surfaceHandles.get(surface)||surface;if(target?.close)return target.close(reason);if(target?.remove){target.remove();return true}return false}
...
function ownerCoordinates(place={}){const latitude=Number(place.latitude??place.coordinates?.latitude??place.location?.latitude),longitude=Number(place.longitude??place.coordinates?.longitude??place.location?.longitude);return Number.isFinite(latitude)&&Number.isFinite(longitude)?{latitude,longitude}:null}
...
function visitEntry(r){const p=state.places[r.place_id]||{},name=state.members[r.participant_id]||'Mitreisender',correction=r.correction&&typeof r.correction==='object'?r.correction:{};return enrichOwnerPlace({id:`visit:${r.id}`,rowId:r.id,sourceKey:r.id,sourceRevision:String(r.updated_at||correction._ownerRevision||r.created_at||r.arrived_at||'')||null,tripId:r.trip_id,placeId:r.place_id,tripPlaceId:r.trip_place_id,providerPlaceId:p.provider_place_id,entityType:p.primary_type||correction.placeType||'place',kind:'visited',title:p.name||correction.placeName||'Besuchter Ort',startAt:r.arrived_at,endAt:r.left_at,durationMinutes:Math.max(5,Math.round(Number(r.duration_seconds||0)/60)),automatic:Boolean(r.is_automatic),source:'gps',participantId:r.participant_id,participantName:name,metadata:{...correction,visitEvidence:{detectionSource:r.detection_source,isConfirmed:Boolean(r.is_confirmed),gpsAccuracyMeters:r.gps_accuracy_meters,minimumDistanceMeters:r.minimum_distance_meters}},icon:typeIcon(p.primary_type)},p)}
...
function emit(reason='cloud'){state.days=buildDays(state.entries);state.lastUpdatedAt=new Date().toISOString();const snap=snapshot();listeners.forEach(fn=>{try{fn(snap)}catch{}});window.dispatchEvent(new CustomEvent('luvia:timeline-changed',{detail:{...snap,projectionReason:reason}}));if(reason==='cloud')window.dispatchEvent(new CustomEvent('luvia:timeline-cloud-changed',{detail:snap}))}
function projectPlaceData(source=root.LuviaTripPlaceData?.snapshot?.()){
 const sourceTripId=source?.tripId||null;if(!sourceTripId||state.tripId&&String(state.tripId)!==String(sourceTripId))return snapshot();
 state.tripId=sourceTripId;
 const placeData=(root.LuviaTripPlaceData?.dateEntries?.()||[]).filter(entry=>entry.placeType!=='mobility').map(dataEntry);
 state.entries=dedupe([...state.entries.filter(entry=>entry.source!=='place-data'),...placeData]);
...
async function hydrate(tripId=activeTripId()){
 ...
 try{
  ...
  // place_visits is the canonical Places-owner truth for confirmed visits.
  // Legacy place_visited events are therefore not painted as a second moment.
  const allowedEvents=new Set(['manual_note','memory','trip_moment','photo_memory']);ownerEventRows=(eventRows.data||[]).filter(row=>allowedEvents.has(row.event_type));
  const validVisits=(visitRows.data||[]).filter(x=>linked.has(x.place_id)&&x.is_confirmed===true&&x.arrived_at&&Number(x.duration_seconds||0)>=300&&['visited','left'].includes(x.state));
...
async function openPhotoMemory(e,node){
 closeSurface(node,'open-photo-memory');photoMemoryHandle?.close('replace');
 ...
 const ui=LuviaUI;if(!ui?.adopt)throw new Error('Overlay Host v1 Legacy Adoption ist noch nicht bereit.');let mounted=null;
 mounted=ui.adopt(modal,{name:'journey.timeline-photo-memory',kind:'sheet',content:modal.querySelector('.lv-photo-memory-sheet'),closeSelector:'[data-close-photo-memory]',initialFocus:'[data-close-photo-memory].lv-timeline-close',label:'Reise-Fotomoment',onClose:()=>{if(photoMemoryHandle?.id===mounted.id)photoMemoryHandle=null}});surfaceHandles.set(modal,mounted);photoMemoryHandle=mounted;
...
function openPlanningEditor({title,icon='',definitions=[],fields={},eyebrow='Datum und Uhrzeit',description='Datum und Uhrzeit werden direkt in eure gemeinsame Reiseplanung übernommen.',submitLabel='Änderungen speichern',save,optimistic=true}={},onDone){
 planningEditorHandle?.close('replace');
 ...
 const ui=LuviaUI;if(!ui?.adopt)throw new Error('Overlay Host v1 Legacy Adoption ist noch nicht bereit.');let mounted=null;
 mounted=ui.adopt(node,{name:'journey.timeline-planning-editor',kind:'sheet',content:form,closeSelector:'[data-close-place-plan]',initialFocus:'input',label:title||'Timeline-Eintrag bearbeiten',onClose:()=>{if(planningEditorHandle?.id===mounted.id)planningEditorHandle=null}});surfaceHandles.set(node,mounted);planningEditorHandle=mounted;
 const close=()=>mounted.close('owner');
 form?.addEventListener('submit',event=>{event.preventDefault();if(submit?.disabled)return;const updates={};try{
   ...
   if(submit){submit.disabled=true;submit.textContent='Wird synchronisiert …'}if(message)message.textContent='';
   const task=save(updates);
   if(optimistic){close();onDone?.(updates);root.dispatchEvent(new CustomEvent('luvia:timeline-mutation-pending',{detail:{title,updates}}))}
   Promise.resolve(task).then(()=>{if(!optimistic){if(message)message.textContent='Gespeichert.';setTimeout(()=>{close();onDone?.(updates)},120)}root.LuviaUIKit?.toast?.('Änderung synchronisiert.',{type:'success'})}).catch(error=>{if(!optimistic&&submit){submit.disabled=false;submit.textContent=submitLabel}if(!optimistic&&message)message.textContent=error?.message||'Die Änderungen konnten nicht gespeichert werden.';root.LuviaUIKit?.toast?.(`${error?.message||'Die Änderungen konnten nicht gespeichert werden.'} Der vorherige Timeline-Stand wurde wiederhergestellt.`,{type:'error'})})
...
function editPlaceDates(e,onDone){let context;try{context=placeDataContext(e)}catch(error){window.LuviaUIKit?.toast?.(error.message,{type:'error'});return null}return openPlanningEditor({title:context.title,icon:context.icon,definitions:context.definitions,fields:context.fields,description:context.placeType==='accommodation'?'Check-in und Check-out werden direkt in eure gemeinsame Reiseplanung übernommen.':'Datum und Uhrzeit werden sofort in eurem Tagesplan vorgemerkt und anschließend mit der Cloud abgeglichen.',save:async updates=>{const writer=window.LuviaPlaceCollections?.saveDateFields;if(typeof writer==='function')await writer({tripId:context.tripId,placeType:context.placeType,tripPlaceId:context.tripPlaceId,placeId:context.placeId,fields:updates});else await window.LuviaTripPlaceData.upsert({tripId:context.tripId,tripPlaceId:context.tripPlaceId,placeId:context.placeId,placeType:context.placeType,fields:updates});window.dispatchEvent(new CustomEvent('luvia:place-plan-changed',{detail:{tripId:context.tripId,type:context.placeType,tripPlaceId:context.tripPlaceId}}));window.dispatchEvent(new CustomEvent('luvia:in-window-data-changed',{detail:{tripId:context.tripId,placeType:context.placeType,tripPlaceId:context.tripPlaceId}}))}},onDone)}
function editLegacySchedule(e,onDone){const rowId=uuid(e?.rowId),trip=uuid(e?.tripId||state.tripId||activeTripId());if(!rowId||!trip){window.LuviaUIKit?.toast?.('Der Cloud-Schlüssel dieses Planeintrags fehlt. Bitte die Timeline neu laden.',{type:'error'});return null}return openPlanningEditor({title:e.title||'Datum und Uhrzeit',icon:e.icon||'',definitions:[{key:'planned_at',label:'Datum und Uhrzeit'}],fields:{planned_at:e.startAt},save:async updates=>{const parts=localParts(updates.planned_at);if(!parts.date||!parts.time)throw new Error('Datum und Uhrzeit dürfen bei diesem älteren Planeintrag nicht leer sein.');const target=state.entries.find(item=>item.id===e.id),before=target?.startAt;if(target){target.startAt=updates.planned_at;emit('owner-local-projection')}try{const {error}=await client().from('trip_schedule_events').update({event_date:parts.date,start_time:parts.time,updated_at:new Date().toISOString()}).eq('id',rowId).eq('trip_id',trip);if(error)throw error;queueMicrotask(()=>hydrate(trip).catch(()=>{}));window.dispatchEvent(new CustomEvent('luvia:place-plan-changed',{detail:{tripId:trip,type:e.entityType,tripPlaceId:e.tripPlaceId}}));window.dispatchEvent(new CustomEvent('luvia:in-window-data-changed',{detail:{tripId:trip,placeType:e.entityType,tripPlaceId:e.tripPlaceId}}))}catch(error){if(target){target.startAt=before;emit('owner-local-projection')}throw error}}},onDone)}
...
function popup(date){
 ...
 items.filter(item=>item.providerPlaceId).forEach(item=>window.LuviaPlaceDetails?.prepare?.(item.providerPlaceId,{regionCode:'DE',photoLimit:1,seedPlace:state.places[item.placeId]||null}).catch(()=>{}));
 dayHandle?.close('replace');
 const node=document.createElement('div');node.className='lv-timeline-modal';
 node.innerHTML=`<div class="lv-timeline-backdrop" data-close-timeline></div><section class="lv-timeline-sheet"><button data-close-timeline class="lv-timeline-close">×</button><span class="lv-kicker">${new Date(`${date}T12:00:00`).toLocaleDateString('de-DE',{weekday:'long',day:'2-digit',month:'long'})}</span><h2>Euer Tag</h2><div class="lv-day-timeline">${items.length?items.map((e,i)=>{const memory=e.kind==='photo_memory';return `<article class="lv-day-entry ${memory?'is-photo-memory':''}"><div class="lv-day-time">${fmtTime(e.startAt)}</div><div class="lv-day-line"><i></i></div><button type="button" class="lv-day-place" data-open-entry="${e.id}"><span>${e.icon}</span><div><strong>${e.title}</strong><small>${stayText(e,items)}</small>${memory?'<em>Bestätigter Fotomoment</em>':e.source==='gps'?`<em>GPS · ${e.participantName}</em>`:''}</div></button><div class="lv-day-entry-actions"><button type="button" data-open-entry="${e.id}">${memory?'Erinnerung öffnen':'Place öffnen'}</button>${['schedule','place-data'].includes(e.source)?`<button type="button" data-edit-entry="${e.id}">Datum und Uhrzeit ändern</button>`:''}</div>${i<items.length-1?`<div class="lv-day-transfer">↳ Weiter zum nächsten ${memory?'Moment':'Ort'}</div>`:''}</article>`}).join(''):'<p>Noch keine Einträge für diesen Tag.</p>'}</div></section>`;
 const ui=LuviaUI;if(!ui?.adopt)throw new Error('Overlay Host v1 Legacy Adoption ist noch nicht bereit.');let mounted=null;
 mounted=ui.adopt(node,{name:'journey.timeline-day',kind:'sheet',content:node.querySelector('.lv-timeline-sheet'),closeSelector:'[data-close-timeline]',initialFocus:'[data-open-entry],[data-close-timeline].lv-timeline-close',label:'Timeline-Tagesablauf',onClose:()=>{if(dayHandle?.id===mounted.id)dayHandle=null}});surfaceHandles.set(node,mounted);dayHandle=mounted;
 node.querySelectorAll('[data-open-entry]').forEach(x=>x.onclick=ev=>{ev.preventDefault();ev.stopPropagation();const entry=items.find(value=>value.id===x.dataset.openEntry);if(entry){if(entry.kind==='photo_memory')openPhotoMemory(entry,mounted);else openPlace(entry,mounted)}});
 node.querySelectorAll('[data-edit-entry]').forEach(x=>x.onclick=ev=>{ev.preventDefault();ev.stopPropagation();const entry=items.find(value=>value.id===x.dataset.editEntry);if(entry)editEntry(entry,updates=>{mounted.close('refresh');const next=Object.values(updates||{}).find(Boolean)||entry.startAt;popup(dayKey(next))})});
 return node
}
