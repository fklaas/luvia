...
async function send(input={}){const bookingId=clean(input.bookingId||input.booking_id);if(!bookingId)throw new Error('Booking-ID fehlt.');const c=await client();if(!c)throw new Error('Supabase ist nicht bereit.');const occasion=clean(input.occasion),note=clean(input.note),legacyNote=occasion&&occasion!=='Kein besonderer Anlass'?[`Anlass: ${occasion}`,note].filter(Boolean).join('\n'):note;const {data,error}=await c.functions.invoke('booking-email-send',{body:{bookingId,userApproved:true,requesterName:input.requesterName||undefined,note:legacyNote||undefined,occasion:occasion||undefined,testRecipient:input.testRecipient||undefined,idempotencyKey:input.idempotencyKey||undefined}});if(error)throw error;return data;}
