...
async function activationControl(providerId,{enabled,confirmAdminControl=false,approvedUntil=null,reason=''}={}){if(!clean(providerId))throw new Error('Provider fehlt.');return invoke('set_activation_control',providerId,{enabled:Boolean(enabled),confirmAdminControl:Boolean(confirmAdminControl),approvedUntil:approvedUntil||null,reason:String(reason||'').slice(0,240)});}
async function controlStatus(providerId){if(!clean(providerId))throw new Error('Provider fehlt.');return invoke('control_status',providerId);}
async function readiness(){const c=await client();if(!c)throw new Error('Supabase ist nicht bereit.');const {data,error}=await c.from('booking_provider_connection_readiness_v6').select('*').order('provider_id');if(error)throw error;return data||[];}
window.LuviaBookingProviderConnections=Object.freeze({version:VERSION,health,probe,activate,orchestrate,activationControl,controlStatus,readiness});
})();
