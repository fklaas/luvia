...
async function readiness(){const c=await client();if(!c)throw new Error('Supabase ist nicht bereit.');const {data,error}=await c.from('booking_provider_connection_readiness_v4').select('*').order('provider_id');if(error)throw error;return data||[];}
window.LuviaBookingProviderConnections=Object.freeze({version:VERSION,health,probe,activate,readiness});
})();
