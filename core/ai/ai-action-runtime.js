...
function previewPayload(payload={}){const allowed=['tripId','entryId','visitId','recoveryId','bookingId','providerPlaceId','placeId','placeType','name','title','date','time','startAt','durationMinutes','routeBufferMinutes','previousRouteBufferMinutes','changeKind','routeMode','routeModeLabel','routeSource','routeDataAge','routeLiveStatus','partySize','reason','status','category'];return Object.fromEntries(allowed.filter(key=>payload[key]!=null&&payload[key]!=='').map(key=>[key,payload[key]]))}
...
function confirmationPreview(definition,payload={}){
  ...
  if(definition.id==='journey.entry.schedule'&&payload.changeKind==='route-buffer')return{...base,changes:{routeBufferMinutes:payload.routeBufferMinutes},before:{routeBufferMinutes:payload.previousRouteBufferMinutes},routeEvidence:{mode:payload.routeModeLabel||payload.routeMode||null,source:payload.routeSource||null,dataAge:payload.routeDataAge||null,liveStatus:payload.routeLiveStatus||'unknown'}};
...
function visitEntryCandidate(entry,activeTripId){
  ...
  return{tripId:clean(entry.tripId)||activeTripId,visitId,placeId:clean(visit.placeId||entry.placeId||entry.place?.id)||null,tripPlaceId:clean(visit.tripPlaceId||entry.tripPlaceId||entry.trip_place_id)||null,providerPlaceId:clean(visit.providerPlaceId||entry.providerPlaceId||entry.provider_place_id||entry.place?.providerPlaceId)?.replace(/^places\//,'')||null,name:aliases[0]||'Bestätigter Besuch',aliases:[...new Set(aliases)],startAt:clean(visit.arrivedAt||entry.startAt),durationMinutes:Math.max(5,Math.round(Number(visit.durationSeconds||Number(entry.durationMinutes||5)*60)/60)),expectedRevision:revision};
}
...
function visitPlaceIdentityValues(source={}){return [...new Set([source.placeId,source.place_id,source.tripPlaceId,source.trip_place_id,source.providerPlaceId,source.provider_place_id,source.id,source.place?.id,source.place?.placeId,source.place?.tripPlaceId,source.place?.providerPlaceId].map(value=>clean(value).replace(/^places\//,'')).filter(Boolean))]}
function visitPlaceAliases(source={}){return [...new Set([source.name,source.title,source.placeName,source.displayName?.text,source.displayName,source.place?.name,source.place?.title,source.place?.placeName].map(value=>typeof value==='object'?'':clean(value)).filter(Boolean))]}
function hasSpecificVisitAlias(candidate={}){return(candidate.aliases||[]).some(alias=>mutationNameTokens(alias).length>0)}
function mergeVisitPlaceIdentity(candidate,place){
  if(!place)return candidate;const ownerAliases=visitPlaceAliases(place),aliases=[...new Set([...(candidate.aliases||[]),...ownerAliases].map(clean).filter(Boolean))],name=ownerAliases.find(alias=>mutationNameTokens(alias).length>0)||aliases.find(alias=>mutationNameTokens(alias).length>0)||candidate.name||'Bestätigter Besuch';return{...candidate,placeId:candidate.placeId||clean(place.placeId||place.id)||null,tripPlaceId:candidate.tripPlaceId||clean(place.tripPlaceId||place.trip_place_id)||null,providerPlaceId:candidate.providerPlaceId||clean(place.providerPlaceId||place.provider_place_id)?.replace(/^places\//,'')||null,name,aliases};
}
function matchVisitPlace(candidate,places=[]){const ids=new Set(visitPlaceIdentityValues(candidate));if(!ids.size)return null;return places.find(place=>visitPlaceIdentityValues(place).some(id=>ids.has(id)))||null}
async function enrichVisitCandidatesFromPlacesOwner(candidates,activeTripId){
  let enriched=uniqueVisitCandidates(candidates),local=[];
  for(const candidate of enriched){if(!candidate.placeId)continue;try{const place=placesContract().reads?.getPlace?.(candidate.placeId);if(place)local.push(place)}catch{}}
  try{const listed=placesContract().reads?.listPlaces?.({tripId:activeTripId})||[];local.push(...(Array.isArray(listed)?listed:listed?.places||[]))}catch{}
  enriched=enriched.map(candidate=>mergeVisitPlaceIdentity(candidate,matchVisitPlace(candidate,local)));
  if(enriched.every(hasSpecificVisitAlias))return enriched;
  try{const response=await Promise.resolve(placesContract().reads?.listSaved?.({tripId:activeTripId})||[]),saved=Array.isArray(response)?response:response?.places||[];enriched=enriched.map(candidate=>mergeVisitPlaceIdentity(candidate,matchVisitPlace(candidate,saved)))}catch{}
  return enriched;
}
function namedVisitCandidate(candidates,intent,message){
  const unique=uniqueVisitCandidates(candidates),matching=unique.filter(candidate=>{const aliases=(candidate.aliases?.length?candidate.aliases:[candidate.name]).map(name=>({...candidate,name}));return Boolean(namedMutationCandidate(aliases,intent,message))});
  if(matching.length===1)return matching[0];
  if(matching.length>1){const ordered=[...matching].sort((left,right)=>Date.parse(left.startAt||left.removedAt||0)-Date.parse(right.startAt||right.removedAt||0)),source=clean(message||intent?.clause);if(/\b(?:erste[nrsm]?|frühere[nrsm]?|fruehere[nrsm]?|first)\b/iu.test(source))return ordered[0];if(/\b(?:zweite[nrsm]?|second)\b/iu.test(source))return ordered[1]||null;if(/\b(?:letzte[nrsm]?|spätere[nrsm]?|spaetere[nrsm]?|last)\b/iu.test(source))return ordered.at(-1);return null}
...
async function semanticVisitMutationPreview(message,compiled,options={}){
  ...
  if(actionId==='journey.visit.restore'){
    const recoveries=placesContract().reads?.visitRecoveries?.()||[],candidates=await enrichVisitCandidatesFromPlacesOwner(uniqueVisitCandidates(recoveries.filter(item=>!item.tripId||clean(item.tripId)===activeTripId).map(item=>({tripId:item.tripId||activeTripId,recoveryId:clean(item.recoveryId),visitId:clean(item.visitId)||null,placeId:clean(item.placeId)||null,tripPlaceId:clean(item.tripPlaceId)||null,providerPlaceId:clean(item.providerPlaceId)?.replace(/^places\//,'')||null,name:clean(item.title)||'Bestätigter Besuch',aliases:[clean(item.title)].filter(Boolean),expectedRevision:clean(item.expectedRevision),removedAt:clean(item.removedAt)})).filter(item=>item.recoveryId&&item.expectedRevision)),activeTripId);if(!candidates.length)throw runtimeError('AI_VISIT_RECOVERY_NOT_FOUND','Für diese Reise ist kein wiederherstellbarer bestätigter Besuch belegt. Es wurde nichts verändert.',{actionId,tripId:activeTripId});const target=namedVisitCandidate(candidates,intent,options.sourceMessage||message);if(!target)throw runtimeError('AI_VISIT_TARGET_AMBIGUOUS','Mehrere entfernte Besuche kommen infrage. Bitte nenne den Ort und bei mehreren Besuchen dort „erster“, „zweiter“ oder „letzter“.',{actionId,tripId:activeTripId,candidateCount:candidates.length});
    return prepare(actionId,{...target,readbackRequired:true},{userGesture:true,surface:options.surface||'global-chat'}).result;
  }
  const projection=await Promise.resolve(journeyContract().reads?.snapshot?.({trip})||{}),entries=journeyEntries(projection),candidates=await enrichVisitCandidatesFromPlacesOwner(uniqueVisitCandidates(entries.map(entry=>visitEntryCandidate(entry,activeTripId)).filter(Boolean)),activeTripId);if(!candidates.length)throw runtimeError('AI_VISIT_NOT_FOUND','Für diese Reise ist aktuell kein bestätigter Besuch belegt. Geplante Orte sind davon getrennt; es wurde nichts verändert.',{actionId,tripId:activeTripId});const target=namedVisitCandidate(candidates,intent,options.sourceMessage||message);if(!target)throw runtimeError('AI_VISIT_TARGET_AMBIGUOUS','Mehrere bestätigte Besuche kommen infrage. Bitte nenne den Ort und bei mehreren Besuchen dort „erster“, „zweiter“ oder „letzter“.',{actionId,tripId:activeTripId,candidateCount:candidates.length});
...
async function dayResult(request){
  ...
  const routeUncertainty=includePlanningDetails&&routeEnabled&&typeof reads.routeUncertainty==='function'?days.flatMap(day=>(day.entries||[]).slice(1).map((entry,index)=>{const previous=day.entries[index],metadata=entry.metadata||{},projection=reads.routeUncertainty({baseMinutes:entry.transferMinutes||metadata.transferMinutes||previous?.transferMinutes||20,routeMode:entry.routeMode||metadata.routeMode||'walking',travelSpeed:metadata.travelSpeed||'balanced',providerConfidence:entry.routeConfidence??metadata.routeConfidence,weatherRisk:metadata.weatherRisk,disruptionRisk:metadata.disruptionRisk,seasonRisk:metadata.seasonRisk,timeOfDayRisk:metadata.timeOfDayRisk,evidence:entry.routeEvidence||metadata.routeEvidence||metadata.routeUncertainty?.evidence||[]});const currentBuffer=Object.hasOwn(metadata,'routeBufferMinutes')?Number(metadata.routeBufferMinutes):null;return{date:day.date,from:previous?.title||'Vorheriger Reisemoment',to:entry.title||'Nächster Reisemoment',toEntryId:entry.id||null,startAt:entry.startAt||null,durationMinutes:Number(entry.durationMinutes)||60,expectedRevision:entry.sourceRevision||null,currentBufferMinutes:Number.isFinite(currentBuffer)?currentBuffer:null,...projection}})):[];
  const routeBufferActions=routeUncertainty.filter(route=>route.evidenceCoverage>0&&route.toEntryId&&route.startAt&&route.expectedRevision&&route.recommendedBufferMinutes>=5&&route.currentBufferMinutes!==route.recommendedBufferMinutes).slice(0,5).map(route=>({actionId:'journey.entry.schedule',label:`${route.recommendedBufferMinutes} Min. Puffer prüfen`,payload:{tripId:activeTripId,entryId:route.toEntryId,name:route.to,startAt:route.startAt,durationMinutes:route.durationMinutes,routeBufferMinutes:route.recommendedBufferMinutes,previousRouteBufferMinutes:route.currentBufferMinutes,changeKind:'route-buffer',routeMode:route.routeMode,routeModeLabel:route.routeModeLabel,routeSource:route.sourceSummary,routeDataAge:route.dataAgeLabel,routeLiveStatus:route.liveStatusLabel,expectedRouteEvidenceSignature:route.evidenceSignature,expectedRevision:route.expectedRevision}}));
  const rehearsals=includePlanningDetails&&rehearsalEnabled&&typeof reads.rehearseDay==='function'?days.map(day=>({date:day.date,...reads.rehearseDay({entries:day.entries||[],travelSpeed:'balanced',contextEvidence:[...(Array.isArray(day.contextEvidence)?day.contextEvidence:[]),...(Array.isArray(projection?.contextEvidence)?projection.contextEvidence:[])],now:request.context?.now||new Date().toISOString()})})):[];
...
async function invokeOwner(definition,payload,idempotencyKey){
  ...
  else if(definition.id==='journey.entry.schedule'){result=await journeyContract().commands.editEntry(payload.entryId,{startAt:payload.startAt,durationMinutes:payload.durationMinutes,routeBufferMinutes:payload.routeBufferMinutes,changeKind:payload.changeKind,routeMode:payload.routeMode,expectedRouteEvidenceSignature:payload.expectedRouteEvidenceSignature,expectedRevision:payload.expectedRevision,...(payload.expectedConflictSignature!=null?{expectedConflictSignature:payload.expectedConflictSignature}:{}),conflictsAccepted:payload.conflictsAccepted===true,confirmed:true,operationId:idempotencyKey});message=payload.changeKind==='route-buffer'?'Der belegte Zeitpuffer wurde beim Journey Owner gespeichert und erneut gelesen.':'Der Timeline-Moment wurde beim zuständigen Owner geändert und erneut gelesen.'}
