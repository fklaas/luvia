...
async function placeDiscoveryResult(request,options={}){
  ...
  const ownerReads=rejectedProviderPlaceIds=>Promise.allSettled(categories.map(category=>placesContract().reads.recommend({tripId:tripId(trip),trip,text:input.query,query:input.query,category,destination:searchDestination,destinationContext,limit:Math.min(3,Math.max(1,Number(input.limit||3))),candidateLimit:50,queryVariantLimit:3,requirePreferenceEvidence:true,fastPath:Boolean(options.viewport),fastQueryLimit:1,maxDistanceMeters:options.viewport?.radiusMeters,preferences:effectivePreferences,profileContext,preferenceMode:profileFallbackUsed?'confirmed-profile-fallback':'explicit-request-over-confirmed-profile',strictPlaceType,rejectedProviderPlaceIds,spatialConstraints,diversity:{minimumQueryVariants:1,targetCandidates:3,rotateAcrossQueries:true}})));
