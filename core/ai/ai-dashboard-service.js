(() => {
  ...
  function modalShell({name='intelligence.dashboard',kicker,title,body,className='',initialFocus=null}){
    const ui=LuviaUI;if(!ui?.mount)throw new Error('Overlay Host v1 ist noch nicht bereit.');const content=document.createElement('section');content.className=`luv-ai-proposal ${className}`.trim();content.innerHTML=`<span class="luv-ai-kicker">${esc(kicker)}</span><h2>${esc(title)}</h2>${body}`;return ui.mount({name,kind:'dialog',content,className:'luv-ai-proposal-overlay',closeSelector:'[data-ai-close]',initialFocus}).overlay;
  }
  function askModal(){
    const overlay=modalShell({name:'intelligence.ask',kicker:'Mit Luvia sprechen',title:'Was soll Luvia für eure Reise durchdenken?',className:'luv-ai-chat',initialFocus:'textarea',body:'<textarea rows="4" placeholder="Zum Beispiel: Was passt heute noch zwischen Museum und Abendessen?"></textarea><div data-ai-answer></div><div class="luv-ai-proposal-actions"><button type="button" data-ai-close>Schließen</button><button type="button" data-ai-send>Fragen</button></div>'});
