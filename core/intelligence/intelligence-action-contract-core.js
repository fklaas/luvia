...
const VERSION='1';
const RUNTIME_VERSION='1.17.0-route-buffer-confirmation';
...
function validateActionInput(actionId,input={},context={}){
  ...
  if(action.id==='journey.entry.schedule'){
    ...
    const routeBufferMinutes=value.routeBufferMinutes==null?null:Number(value.routeBufferMinutes);if(routeBufferMinutes!=null&&(!Number.isInteger(routeBufferMinutes)||routeBufferMinutes<5||routeBufferMinutes>90))issue('range','routeBufferMinutes','Der Zeitpuffer muss zwischen 5 und 90 Minuten liegen.');
    const changeKind=text(value.changeKind)||'schedule';if(!['schedule','route-buffer'].includes(changeKind))issue('enum','changeKind','Die Art der Timeline-Änderung ist unbekannt.');
    if(changeKind==='route-buffer'&&!text(value.expectedRouteEvidenceSignature))issue('required','expectedRouteEvidenceSignature','Der geprüfte Routenbeleg fehlt.');
    if(!text(value.expectedRevision))issue('required','expectedRevision','Der geprüfte Owner-Stand fehlt.');
    return finish({tripId:text(value.tripId)||text(context.tripId)||null,entryId:text(value.entryId),name:text(value.name)||null,startAt:text(value.startAt),durationMinutes,routeBufferMinutes,previousRouteBufferMinutes:value.previousRouteBufferMinutes==null?null:Number(value.previousRouteBufferMinutes),changeKind,routeMode:text(value.routeMode)||null,routeModeLabel:text(value.routeModeLabel)||null,routeSource:text(value.routeSource)||null,routeDataAge:text(value.routeDataAge)||null,routeLiveStatus:text(value.routeLiveStatus)||null,expectedRouteEvidenceSignature:text(value.expectedRouteEvidenceSignature)||null,expectedRevision:text(value.expectedRevision),expectedConflictSignature:value.expectedConflictSignature==null?null:text(value.expectedConflictSignature),conflictsAccepted:value.conflictsAccepted===true});
  }
...
const ACTIONS=Object.freeze([
  ...
  {id:'journey.entry.schedule',owner:'journey',ownerContract:'journey.v1',ownerMethod:'commands.editEntry',effect:'WRITE',risk:'R2',confirmation:'EXPLICIT',resultKind:'receipt',reversible:true,idempotency:'REQUIRED',compensation:'journey.entry.schedule',permissions:['journey.write','trip.member'],label:'Zeit oder Puffer bestätigen',description:'Ändert einen revisionsgeprüften Timeline-Moment oder seinen belegten Zeitpuffer über den zuständigen Journey- oder Places-Owner.',consequence:'Ändert Zeitpunkt, Dauer oder Zeitpuffer erst nach Vorher/Neu-Vorschau, Belegprüfung und ausdrücklicher Bestätigung.'},
