...
function planningDialogue(value={}){
  ...
  // These are planning requirements, not a display shortlist. The model response
  // already has an output budget; truncating arrays here silently drops wishes.
  const items=input=>(Array.isArray(input)?input:[]).map(item).filter(entry=>entry.key&&entry.label);
  ...
  const conflictAssessment={status:['none','tradeoff','blocked'].includes(value.conflictAssessment?.status)?value.conflictAssessment.status:'none',summary:text(value.conflictAssessment?.summary),tensions:(Array.isArray(value.conflictAssessment?.tensions)?value.conflictAssessment.tensions:[]).map(tension=>({id:text(tension?.id),label:text(tension?.label),reason:text(tension?.reason),decisionNeeded:tension?.decisionNeeded===true,variants:(Array.isArray(tension?.variants)?tension.variants:[]).map(variant=>({id:text(variant?.id),label:text(variant?.label),preserves:text(variant?.preserves),relaxes:text(variant?.relaxes),effect:text(variant?.effect)}))}))};
  return{understanding:text(value.understanding),goals:(Array.isArray(value.goals)?value.goals:[]).map(goal=>({type:text(goal?.type,'open'),label:text(goal?.label),hardConstraints:items(goal?.hardConstraints),softPreferences:items(goal?.softPreferences),timeWindow:goal?.timeWindow?{label:text(goal.timeWindow.label),start:text(goal.timeWindow.start),end:text(goal.timeWindow.end),flexible:Boolean(goal.timeWindow.flexible)}:null,source:text(goal?.source,'ai')})).filter(goal=>goal.label),hardConstraints:items(value.hardConstraints),softPreferences:items(value.softPreferences),conflictAssessment,followUpQuestion:question,summary:{headline:text(value.summary?.headline,'So habe ich euch verstanden'),intro:text(value.summary?.intro),goalLabels:list(value.summary?.goalLabels,Infinity),hardLabels:list(value.summary?.hardLabels,Infinity),softLabels:list(value.summary?.softLabels,Infinity)},unknowns:list(value.unknowns,Infinity),confidence:number(value.confidence,0,1,.5)};
}
