...
const VERSION='1';
const RUNTIME_VERSION='1.1.0-trip-composition';
...
const DOMAINS=Object.freeze([
  {id:'trip',description:'Reiseauftrag und vollständige, noch ungespeicherte Reiseentwürfe.',capabilities:['planning.dialogue','trip.compose'],tools:['trip.current','preferences.current','travel.context'],events:[],contracts:{sourceContract:'trip.v1',owner:'trip',placesRemainAuthoritative:true,writesRequireConfirmation:true}},
...
function tripItinerary(value={}){return{title:text(value.title),summary:text(value.summary),days:(Array.isArray(value.days)?value.days:[]).slice(0,366).map((day,index)=>({date:text(day?.date),label:text(day?.label,`Tag ${index+1}`),theme:text(day?.theme),entries:(Array.isArray(day?.entries)?day.entries:[]).slice(0,8).map(entry=>({providerPlaceId:text(entry?.providerPlaceId),time:text(entry?.time),durationMinutes:number(entry?.durationMinutes,30,720,90),category:text(entry?.category),reason:text(entry?.reason),confidence:number(entry?.confidence,0,1,.5)})).filter(entry=>entry.providerPlaceId)})),alternatives:list(value.alternatives,20),uncoveredRequirements:list(value.uncoveredRequirements,20),warnings:list(value.warnings,20),confidence:number(value.confidence,0,1,.5)}}
