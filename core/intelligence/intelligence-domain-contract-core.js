...
const VERSION='1';
const RUNTIME_VERSION='1.2.0-trip-quality-audit';
...
const CAPABILITIES=Object.freeze([
  ...
  {id:'trip.compose',tier:'deep',mode:'READ',schema:'trip_itinerary',tools:['trip.current','preferences.current','travel.context'],timeoutMs:45000,cacheTtlMs:0,description:'Komponiert aus einem semantischen Reiseauftrag und bereits belegten Places einen vollständigen, bestätigungspflichtigen Tagesentwurf.'},
  {id:'trip.audit',tier:'deep',mode:'READ',schema:'trip_quality_audit',tools:[],timeoutMs:30000,cacheTtlMs:0,description:'Prüft einen vollständigen Reiseentwurf unabhängig auf Logik, Abdeckung, Belastung, räumliche Kohärenz und unbelegte Aussagen.'},
...
const DOMAINS=Object.freeze([
  {id:'trip',description:'Reiseauftrag und vollständige, noch ungespeicherte Reiseentwürfe.',capabilities:['planning.dialogue','trip.compose','trip.audit'],tools:['trip.current','preferences.current','travel.context'],events:[],contracts:{sourceContract:'trip.v1',owner:'trip',placesRemainAuthoritative:true,writesRequireConfirmation:true}},
...
function tripItinerary(value={}){return{title:text(value.title),summary:text(value.summary),days:(Array.isArray(value.days)?value.days:[]).slice(0,366).map((day,index)=>({date:text(day?.date),label:text(day?.label,`Tag ${index+1}`),theme:text(day?.theme),role:['arrival','full','departure','day-trip'].includes(day?.role)?day.role:'full',entries:(Array.isArray(day?.entries)?day.entries:[]).slice(0,8).map(entry=>({providerPlaceId:text(entry?.providerPlaceId),time:text(entry?.time),durationMinutes:number(entry?.durationMinutes,30,720,90),category:text(entry?.category),reason:text(entry?.reason),confidence:number(entry?.confidence,0,1,.5)})).filter(entry=>entry.providerPlaceId)})),alternatives:list(value.alternatives,20),backupOptions:(Array.isArray(value.backupOptions)?value.backupOptions:[]).slice(0,40).map(item=>({dayDate:text(item?.dayDate),forProviderPlaceId:text(item?.forProviderPlaceId),providerPlaceId:text(item?.providerPlaceId),trigger:text(item?.trigger),reason:text(item?.reason)})).filter(item=>item.dayDate&&item.forProviderPlaceId&&item.providerPlaceId),uncoveredRequirements:list(value.uncoveredRequirements,20),warnings:list(value.warnings,20),confidence:number(value.confidence,0,1,.5)}}
function tripQualityAudit(value={}){return{readyForReview:value.readyForReview===true,score:number(value.score,0,100,0),headline:text(value.headline),dimensions:(Array.isArray(value.dimensions)?value.dimensions:[]).slice(0,12).map(item=>({id:text(item?.id),label:text(item?.label),score:number(item?.score,0,100,0),status:['pass','attention','blocked'].includes(item?.status)?item.status:'attention',summary:text(item?.summary)})).filter(item=>item.id&&item.label),issues:(Array.isArray(value.issues)?value.issues:[]).slice(0,30).map(item=>({code:text(item?.code),severity:item?.severity==='blocked'?'blocked':'attention',dayDate:text(item?.dayDate),providerPlaceIds:list(item?.providerPlaceIds,12),message:text(item?.message),suggestedRepair:text(item?.suggestedRepair)})).filter(item=>item.code&&item.message),strengths:list(value.strengths,12),repairInstructions:list(value.repairInstructions,20),confidence:number(value.confidence,0,1,.5)}}
