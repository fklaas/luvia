...
const VERSION='2.1.0-verified-travel-windows';
...
function requestedPeriodText(time={}){return clean([time.season,time.month,time.dateFlexibility,...(time.requestedWindows||[]).flatMap(item=>[item?.label,item?.value])].filter(Boolean).join(' ')).toLowerCase()}
...
function projectTripBrief(input={},response={}){
  ...
  travelOrder.time.suggestedWindows=suggestHolidayTravelWindows(travelOrder.time,matchingHolidayEvidence);
