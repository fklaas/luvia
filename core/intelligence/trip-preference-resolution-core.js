...
function experienceSelectionPolicy(input={}){
  ...
  return immutable({version:2,requestedCategories:requested,excludedCategories:excluded,shoppingStyle,explicitScope:requested.length>0,allowUnrequestedCategories:false,balanceMeans:'daily rhythm within the requested experiences, not equal shares of every POI category'});
}
function tripPlaceExperienceFit(input={}){
  const place=input.place||{},policy=input.policy||experienceSelectionPolicy(input),aliases={active:'activities',family:'activities',activity:'activities',malls:'shopping'},canonical=value=>aliases[value]||value,token=value=>clean(value).normalize('NFKD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,'_').replace(/^_+|_+$/g,'');
  ...
  if(has(['bar','pub','beach_bar','beer_bar','cocktail_bar','sports_bar','tiki_bar','wine_bar','lounge','lounge_bar','taproom','biergarten','night_club','nightclub','dance_club','discotheque','disco','jazz_club','karaoke_bar','live_music_venue','music_venue','concert_hall','comedy_club','casino']))categories.push('nightlife');
  if(has(['museum','art_gallery','movie_theater','performing_arts_theater','cultural_center']))categories.push('culture');
  if(has(['historical_landmark','historical_monument','monument','church','cathedral','observation_deck']))categories.push('sights');
  if(has(['park','garden','park_recreation_area','national_park','hiking_area','nature_reserve','natural_feature']))categories.push('nature');
  if(has(['bowling_alley','escape_room','amusement_center','minigolf','fitness_center','gym','swimming_pool','water_sports_center','water_park','amusement_park','aquarium','zoo','skating_rink','golf_course','stadium']))categories.push('activities');
  if(has(['amusement_park','water_park','theme_park']))categories.push('themeparks');
  if(has(['spa','sauna','wellness','wellness_center','thermal_bath']))categories.push('wellness');
  ...
  if(!functional&&described){if(broadSight)categories.push('sights');if(broadActivity)categories.push('activities');if(broadNightlife)categories.push('nightlife');}
  ...
  const evidencedCategories=[...new Set(categories)],matchedCategories=evidencedCategories.filter(category=>policy.requestedCategories.includes(category)&&!policy.excludedCategories.includes(category)),retrievalBacked=!retrievalCategory||evidencedCategories.includes(retrievalCategory),eligibleByCategory=matchedCategories.length>0&&retrievalBacked,unresolvedBroad=((broadSight&&!specificCategories.has('sights'))||(broadActivity&&!specificCategories.has('activities'))||(broadNightlife&&!specificCategories.has('nightlife')))&&!described&&!researchCategory;
  // Specific negative evidence beats a misleading broad retrieval label.
  const reason=functional&&!researchCategory?'functional-place-not-visitor-experience':shop&&!shoppingCentre&&policy.shoppingStyle==='centres'&&!matchedCategories.length?'individual-shop-not-shopping-centre':!retrievalBacked?'retrieval-category-unverified':unresolvedBroad?'ambiguous-provider-identity':!matchedCategories.length?'outside-requested-experiences':'';
  return immutable({eligible:functional&&!researchCategory?false:policy.explicitScope?eligibleByCategory:!unresolvedBroad,matchedCategories,evidencedCategories,shoppingCentre,needsEvidence:unresolvedBroad||functional&&!researchCategory,evidenceStrength:researchCategory?'researched':described&&(broadSight||broadActivity||broadNightlife)?'described-broad':'canonical-type',reason});
}
...
function projectTripBrief(input={},response={}){
  ...
  for(const item of constraints){
    ...
    if(['experiencewish','avoidexperience','discoverystyle','eveningstyle'].includes(key)){
      travelOrder.experiences=travelOrder.experiences||{wishes:[],avoid:[],discoveryStyle:'',eveningStyle:''};
      if(key==='experiencewish')travelOrder.experiences.wishes.push(clean(item.value));
      if(key==='avoidexperience')travelOrder.experiences.avoid.push(clean(item.value));
      if(key==='discoverystyle')travelOrder.experiences.discoveryStyle=clean(item.value);
      if(key==='eveningstyle')travelOrder.experiences.eveningStyle=clean(item.value);
      handled=true;
    }
    ...
    if(['mealtiming','mainmealtiming','restauranttiming'].includes(key)&&['flexible','lunch','dinner','both'].includes(value)){preferences.mealTiming=value;handled=true;}
  ...
  preferences.mealTiming=['flexible','lunch','dinner','both'].includes(preferences.mealTiming)?preferences.mealTiming:'flexible';
  ...
  const around=(kind,value,fallbackStart,fallbackEnd)=>{const minutes=time(value)?Number(value.slice(0,2))*60+Number(value.slice(3)):null,clock=number=>`${String(Math.floor(Math.max(0,Math.min(1439,number))/60)).padStart(2,'0')}:${String(Math.max(0,Math.min(1439,number))%60).padStart(2,'0')}`;return{kind,windowStart:minutes==null?fallbackStart:clock(minutes-30),windowEnd:minutes==null?fallbackEnd:clock(minutes+120),venueMode:'place',importance:'preferred'}};
  const mealSlots=[];
  if(preferences.mealTiming==='lunch'||preferences.mealTiming==='both')mealSlots.push(around('lunch','','12:00','14:30'));
  if(preferences.mealTiming==='dinner'||preferences.mealTiming==='both')mealSlots.push(around('dinner','','18:30','21:30'));
  if(travelOrder.rhythm.breakfastTime)mealSlots.push(around('breakfast',travelOrder.rhythm.breakfastTime,'07:30','10:30'));
  if(travelOrder.rhythm.lunchWindow){const values=travelOrder.rhythm.lunchWindow.match(/(?:[01]\d|2[0-3]):[0-5]\d/g)||[];mealSlots.splice(0,mealSlots.length,...mealSlots.filter(item=>item.kind!=='lunch'));mealSlots.push({kind:'lunch',windowStart:values[0]||'12:00',windowEnd:values[1]||'14:30',venueMode:'place',importance:'preferred'});}
  if(travelOrder.rhythm.dinnerTime){mealSlots.splice(0,mealSlots.length,...mealSlots.filter(item=>item.kind!=='dinner'));mealSlots.push(around('dinner',travelOrder.rhythm.dinnerTime,'18:30','21:30'));}
  travelOrder.rhythm.mealPlan={mode:mealSlots.length?'selected':'flexible',slots:mealSlots.sort((left,right)=>left.windowStart.localeCompare(right.windowStart))};
  ...
  if(!travelOrder.geography.movementStyle)travelOrder.geography.movementStyle=preferences.mobility.find(value=>['near_walk','city_transit','wide_taxi_car'].includes(value))||'';
  ...
  for(const key of ['adults','children'])if(base[key]!=null&&base[key]!==''&&Number.isInteger(Number(base[key]))&&Number(base[key])>=0)travelOrder.travelers[key]=Number(base[key]);
  ...
  // Trip context overrides soft profile defaults without changing the profile.
  // Zero children is explicit information; null remains genuinely unknown.
  if(travelOrder.travelers.children===0){
    const childContext=/^(traveling_with_children|baby|toddler|children|family_friendly|stroller|kinderwagen)$/;
    const withoutChildren=values=>list(values).filter(value=>!childContext.test(value));
    requestPreferences.familyPreferences={...(requestPreferences.familyPreferences||{}),needs:withoutChildren(requestPreferences.familyPreferences?.needs),travelingWithChildren:false,stroller:false};
    requestPreferences.family=withoutChildren(requestPreferences.family);
    requestPreferences.travelStyles=withoutChildren(requestPreferences.travelStyles);
  }
  if(preferences.budgetLevel)requestPreferences.budgetPreference=({economy:'low',balanced:'medium',generous:'high',open:'open'})[preferences.budgetLevel]||preferences.budgetLevel;
  if(preferences.pace)requestPreferences.travelPace=preferences.pace;
  const experienceChoices={hands_on:'Selbst etwas ausprobieren und mitmachen',food_discovery:'Lokale Küche und Märkte entdecken',city_stories:'Stadtviertel, Architektur und Geschichten',nature_time:'Bewusst Zeit draußen verbringen',live_evenings:'Lebendige Abende und Live-Erlebnisse',playful:'Spielerische gemeinsame Erlebnisse'};
  travelOrder.experiences=travelOrder.experiences||{wishes:[],avoid:[],discoveryStyle:'',eveningStyle:''};
  travelOrder.experiences.wishes=unique([...travelOrder.experiences.wishes,...(base.experiences||[]).map(value=>experienceChoices[value]).filter(Boolean)]);
  travelOrder.experiences.avoid=unique(travelOrder.experiences.avoid);
  travelOrder.group.preferenceCoverage={status:input.participantPlan==='invite-after-creation'?'awaiting-invitees':'organizer-input',knownParty:travelOrder.travelers.description||null,otherMembersConsulted:false,revisionPolicy:'propose-changes-preserve-confirmed-and-booked',membershipOwner:'collaboration',profileOwner:'identity'};
  ...
  const explicitKeys=new Set(constraints.map(item=>norm(item.key))),planningAssumptions=[
    ...
    ...(preferences.interests.includes('food')?[{id:'meal-timing',label:'Restaurants',value:{flexible:'Flexibel im Tagesverlauf',lunch:'Eher mittags',dinner:'Eher abends',both:'Mittags und abends'}[preferences.mealTiming],source:explicitKeys.has('mealtiming')||base.mealTiming&&base.mealTiming!=='flexible'?'trip-input':'luvia-default',editable:true}]:[]),
