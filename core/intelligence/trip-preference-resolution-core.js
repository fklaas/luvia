...
function projectTripBrief(input={},response={}){
  ...
  const needsHolidayWindow=travelOrder.travelers.schoolHolidayRequired&&matchingHolidayEvidence.length&&!confirmedHolidayCovered,holidayQuestion=travelOrder.travelers.schoolHolidayRequired&&!travelOrder.travelers.schoolHolidayRegion?{id:'school-holiday-region',text:'Für welches Bundesland oder welche Schulregion gelten eure Ferien?',reason:'Nur damit kann Luvia einen groben Zeitraum mit echten Ferienterminen abgleichen.'}:needsHolidayWindow?{id:'school-holiday-period',text:travelOrder.time.suggestedWindows.length?'Welcher bestätigte Ferienzeitraum passt für euch?':'Für den genannten groben Zeitraum wurde noch kein passendes bestätigtes Ferienfenster gefunden.',reason:travelOrder.time.confirmedStart?'Der bisher gewählte Zeitraum liegt nicht vollständig in den bestätigten Schulferien.':'Luvia plant erst weiter, wenn ihr ein belegtes Zeitfenster gewählt habt.',options:travelOrder.time.suggestedWindows}:null,requestedQuestion=needsRequestedWindow?{id:'requested-travel-period',text:`Welcher Zeitraum im ${MONTH_LABELS[requestedMonthYear(travelOrder.time).month-1]||'gewünschten Zeitraum'} passt für euch?`,reason:'Alle Optionen folgen eurem genannten Monat und eurer Reisedauer. Wetter, Flugpreise und Verfügbarkeit werden erst mit echten Quellen bewertet.',options:travelOrder.time.suggestedWindows}:null,modelQuestion=data.followUpQuestion?.text&&!questionsConfirmedDuration(data.followUpQuestion,confirmedNights,requestedNights)?{id:'model-follow-up',text:clean(data.followUpQuestion.text).slice(0,300),reason:clean(data.followUpQuestion.reason).slice(0,300)}:null,question=holidayQuestion||requestedQuestion||modelQuestion;
  return immutable({owner:'intelligence',contractId:'intelligence.v1',kind:'trip-planning-brief',source:'ai',understanding:correctConfirmedDurationNarrative(data.understanding,confirmedNights,requestedNights).slice(0,1200),goals:goals.map(goal=>({type:clean(goal.type),label:clean(goal.label).slice(0,180)})),travelOrder,calendarEvidence,applied:uniqueLabels(applied),unresolved:uniqueLabels(unresolved),unknowns:(data.unknowns||[]).slice(0,20).map(item=>clean(item).slice(0,180)),question,tripPreferences:preferences,requestPreferences,policy,modelConfidence:Math.max(0,Math.min(1,Number(data.confidence)||0)),automaticPlanningAllowed:!unresolved.some(item=>item.hard),confirmationRequired:true,automaticMutation:false});
}
...
function confirmTripBriefWindow(brief={},selection={}){
  ...
  if(['school-holiday-period','requested-travel-period'].includes(next.question?.id))next.question=null;
