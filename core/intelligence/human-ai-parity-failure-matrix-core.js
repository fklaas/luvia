var LuviaHumanAIParityFailureMatrixCoreV1=(()=>{
  ...
  function query(rows=[],filters={}){
    const search=text(filters.search).toLocaleLowerCase('de-DE'),category=text(filters.category),release=text(filters.releaseStatus),failure=text(filters.failureId),dimensionName=text(filters.dimension),dimensionStatus=text(filters.dimensionStatus),limit=Math.max(1,Math.min(Number(filters.limit)||rows.length,329));
