(() => {
  ...
  async function composeTripItinerary(input = {}) {
    ...
    const repairInstructions=(Array.isArray(input.repairInstructions)?input.repairInstructions:[]).map(item=>clean(item,300)).filter(Boolean).slice(0,20),composeTier=repairInstructions.length?'deep':'fast';
    const response=await run('trip.compose',{
      ...
      repairInstructions
    },{tier:composeTier,fallback:false,context:{surface:'trip-composer',purpose:repairInstructions.length?'repair-complete-trip-itinerary':'complete-trip-itinerary',qualityLane:repairInstructions.length?'sol-repair':'luna-draft'}});
