(() => {
  ...
  async function composeTripItinerary(input = {}) {
    ...
    const planningDayCount=Math.max(1,Math.min(366,Array.isArray(input.days)?input.days.length:1)),planningCandidateLimit=Math.max(160,planningDayCount*9),candidateInput=(Array.isArray(input.candidates)?input.candidates:[]).slice(0,planningCandidateLimit).map(item=>{
