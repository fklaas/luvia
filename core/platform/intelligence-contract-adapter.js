(() => {
  ...
  async function composeTripItinerary(input = {}) {
    ...
    const candidateInput=(Array.isArray(input.candidates)?input.candidates:[]).slice(0,160).map(item=>{
    ...
    if(existing&&requestedRepairDates.length){
      ...
      for(const repairPolicy of repairPolicies){
        const availableCandidates=candidateInput.filter(item=>!usedIds.has(item.providerPlaceId)),neighboringDays=[...preservedDays,...replacements.values()].map(day=>({date:clean(day?.date,10),theme:clean(day?.theme,160),energy:day?.balance?.energy||'balanced',providerPlaceIds:(day?.entries||[]).map(entry=>clean(entry?.providerPlaceId,240).replace(/^places\//,'')).filter(Boolean)}));
        ...
        const repairResponse=await run('trip.compose-day-repair',{surface:'trip-composer',retryGeneration:Math.max(0,Math.round(Number(input.retryGeneration)||0)),locale:input.locale||'de-DE',timeZone:input.timeZone||input.destination?.timezone||'',travelOrder:order,planningPolicy,destination:input.destination||null,travelPromise:existing.travelPromise||{},days:[repairPolicy],neighboringDays,excludedProviderPlaceIds:[...usedIds],candidateCatalog:availableCandidates,evidenceCatalog,repairInstructions:dayInstructions.length?dayInstructions:repairInstructions,planningContract:{...planningContract,dayPolicies:[repairPolicy],repairOnly:true,preserveOtherDays:true}},{tier:qualityAttempt>=3?'deep':'default',fallback:false,workflowId:input.workflowId||null,context:{surface:'trip-composer',purpose:'repair-failed-trip-day',qualityLane:qualityAttempt>=3?'sol-day-repair':'terra-day-repair'}});
        ...
        const repairValue=repairResponse?.data||repairResponse?.result||repairResponse,day=(Array.isArray(repairValue?.days)?repairValue.days:[]).find(item=>clean(item?.date,10)===repairPolicy.date);
      ...
      const response=await run('trip.compose',{
        surface:'trip-composer',retryGeneration:Math.max(0,Math.round(Number(input.retryGeneration)||0)),locale:input.locale||'de-DE',timeZone:input.timeZone||input.destination?.timezone||'',
  ...
  async function auditTripItinerary(input = {}) {
    ...
    const response=await run('trip.audit',{surface:'trip-composer',retryGeneration:Math.max(0,Math.round(Number(input.retryGeneration)||0)),locale:input.locale||'de-DE',timeZone:input.timeZone||input.destination?.timezone||'',travelOrder:input.brief?.travelOrder||{},planningPolicy:input.brief?.policy||{},destination:input.destination||null,planningContract:{dayPolicies:itinerary.dayPolicies||[],allDaysExactlyOnce:true,useOnlyCandidateIds:true,preserveAllHardConstraints:true,travelPromiseRequired:true,explicitDeliberateFreeTime:true,wholeTripBalance:true,uncertaintyMapRequired:true,verifiedClaimsRequireEvidence:true,bookingDependenciesRequired:true,neighborhoodRecommendationRequired:true,clusterDaysGeographically:true,arrivalDepartureAware:true,prepareBackupOptions:true},itinerary,evidenceCatalog:itinerary.evidenceCatalog||[],candidateCatalog:(Array.isArray(input.candidates)?input.candidates:[]).slice(0,160).map(item=>({providerPlaceId:String(item?.providerPlaceId||item?.provider_place_id||item?.id||'').replace(/^places\//,''),name:String(item?.name||''),category:String(item?.requestCategory||item?.category||''),coordinates:item?.coordinates||item?.location||null}))},{tier:'default',fallback:false,workflowId:input.workflowId||null,context:{surface:'trip-composer',purpose:'independent-trip-quality-audit',qualityLane:'terra-audit'}});
