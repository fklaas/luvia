  ...
  async function composeTripItinerary(input = {}) {
    ...
    if(!contractIssues.length&&(supportingDetailsChanged||(unusedCandidates.length&&!(value?.backupOptions||[]).some(validBackup)))){
      ...
      try{
        ...
        const warning='Der Tagesplan ist vollständig; Plan-B-Alternativen und abhängige Buchungshinweise werden bei der nächsten Prüfung ergänzt.';
        value={...value,...(supportingDetailsChanged?{backupOptions:[],uncertaintyMap:[],bookingOrder:[],neighborhoodRecommendation:{}}:{}),warnings:[...new Set([...(value?.warnings||[]),warning])],supportingDetailsErrorCode:clean(error?.code||'TRIP_SUPPORT_UNAVAILABLE',80)};
      }
    }
