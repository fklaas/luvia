(() => {
  ...
  async function composeTripItinerary(input = {}) {
    ...
    const repairInstructions=(Array.isArray(input.repairInstructions)?input.repairInstructions:[]).map(item=>clean(item,300)).filter(Boolean).slice(0,20),qualityAttempt=Math.max(1,Math.min(3,Math.round(Number(input.qualityAttempt)||1))),composeTier=!repairInstructions.length?'fast':qualityAttempt>=3?'deep':'default';
    const response=await run('trip.compose',{
      ...
      repairInstructions
    },{tier:composeTier,fallback:false,context:{surface:'trip-composer',purpose:repairInstructions.length?'repair-complete-trip-itinerary':'complete-trip-itinerary',qualityLane:composeTier==='fast'?'luna-draft':composeTier==='default'?'terra-repair':'sol-last-resort'}});
    ...
    const value=response?.data||response?.result||response,planDays=Array.isArray(value?.days)?value.days:[],allowed=new Map(candidateInput.map(item=>[item.providerPlaceId,item])),allowedEvidence=new Set(evidenceCatalog.map(item=>item.id)),used=new Set(),byDate=new Map(planDays.map(item=>[clean(item?.date,10),item])),byLabel=new Map(planDays.map(item=>[clean(item?.label,80),item]));
    ...
    return immutable({owner:'intelligence',contractId:'intelligence.v1',kind:'ai-trip-itinerary',source:'ai',title:clean(value?.title,160),summary:clean(value?.summary,800),travelPromise,days,dayPolicies:dayInput,uncertaintyMap,bookingOrder,neighborhoodRecommendation,evidenceCatalog,alternatives,backupOptions,uncoveredRequirements:(value?.uncoveredRequirements||[]).map(item=>clean(item,240)).filter(Boolean).slice(0,20),warnings:(value?.warnings||[]).map(item=>clean(item,240)).filter(Boolean).slice(0,20),confidence:Math.max(0,Math.min(1,Number(value?.confidence)||0)),candidateCount:candidateInput.length,automaticMutation:false,confirmationRequired:true});
  }
