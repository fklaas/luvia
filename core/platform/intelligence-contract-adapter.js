(() => {
  ...
  function experienceSelectionPolicy(input={}){return immutable(preferenceResolver().experienceSelectionPolicy(input));}
  function tripPlaceExperienceFit(input={}){return immutable(preferenceResolver().tripPlaceExperienceFit(input));}
  function tripExperienceAlignment(input={}){return immutable(preferenceResolver().tripExperienceAlignment(input));}
  ...
  async function interpretTripBrief(input = {}) {
    const request={
      ...
      interpretationContract:{purpose:'Interpret only this new trip. Never inherit a different active trip. Respect explicit profile restrictions. Return German text. Preserve every requested hard constraint, including unsupported ones. A follow-up answer resolves its original question.',
        ...
        constraintKeys:{destination:'named destination or region',destinationScope:'city|region|country|coast|open',countryPreference:'named country or abroad',timeWindow:'original approximate period',season:'spring|summer|autumn|winter',month:'named month',year:'four digit year',durationDays:'integer calendar days only when the user says days',durationNights:'integer nights only when the user says nights',dateFlexibility:'fixed|days|weeks|open',travelers:'short group description',adults:'integer',children:'integer',childAges:'comma-separated ages',schoolHolidayRequired:'true|false',schoolHolidayRegion:'governing country/state/region',accommodation:'requested kind',transport:'requested mode',departureOrigin:'named home city, station or airport',maximumFlightMinutes:'integer only when explicitly stated',flightPreference:'short|medium|long|open or description',arrivalTime:'HH:mm local time',departureTime:'HH:mm local time',arrivalRecoveryMinutes:'integer',departureBufferMinutes:'integer',category:'one goal type',excludeCategory:'one goal type',categoryMix:'balanced|favorites|surprising',pace:'slow|balanced|active',budgetLevel:'economy|balanced|generous|open',tripBudget:'amount as stated',dailyBudget:'amount as stated',currency:'ISO code or stated currency',splurgeDay:'special higher-budget moment',dietary:'requirement',accessibility:'requirement',mobility:'requirement',maximumPerDay:'1|2|3|4',notBefore:'HH:mm',notAfter:'HH:mm',wakeTime:'HH:mm',bedTime:'HH:mm',mealTiming:'flexible|lunch|dinner|both',breakfastTime:'HH:mm',lunchWindow:'time range as stated',dinnerTime:'HH:mm',napWindow:'time range as stated',energyPattern:'morning|balanced|evening or description',jetLagSensitivity:'low|medium|high',freeTimePercent:'0-100',maximumTransferMinutes:'integer',dayTripRadiusKm:'integer',baseLocation:'named area',spatialClustering:'true|false',weatherFallback:'true|false',indoorOutdoorBalance:'description',planBPerDay:'true|false',reservationStyle:'early|flexible|minimal or description',bookingDeadline:'date or relative period',mustReserve:'specific wish',groupDecisionMode:'consensus|majority|individual-turns or description',fairnessRequired:'true|false',memberPriority:'person and wish',evidenceFreshness:'description',recheckBeforeDays:'integer',mustDo:'specific wish',exclude:'specific exclusion'},
        experienceKeys:{shoppingStyle:'centres by default; specific_shops only for an explicit kind of shop; mixed only when requested',experienceWish:'specific activity or experience, not a generic POI category',avoidExperience:'experience the party dislikes',discoveryStyle:'familiar highlights|local discovery|surprise, only if stated',eveningStyle:'dinner|live music|bars|dancing|quiet, only if stated'},
        rule:'Output adults and children as separate integer constraints whenever the party is explicit; no children means children=0, never merely a group description. The current trip overrides profile defaults about children, stroller, budget and pace; preserve medical and dietary restrictions. Extract a category goal for EACH positively requested experience, including named must-dos. Negative wishes belong only in excludeCategory/avoidExperience; do not add them to mustDo. A balanced pace never implies culture, nature or sightseeing. General shopping means shopping centres (shoppingStyle=centres); only an explicit wish for a type of shop means specific_shops. Preserve the kind of shop in experienceWish. If activities are ambiguous and no activity tastes are known, ask one short question about water/sport, games/action, shows or workshops instead of silently choosing parks. A beach day is water with a mustDo for real beach time, never a request for parks. Old town/harbour exploration is sights/water, not automatically museums or art galleries. Nightlife is its own category even when also in mustDo. Lively evenings are an explicit evening wish: preserve that wording as eveningStyle and include a nightlife goal without inventing a preference for clubs or alcohol. Wanting to try something personally means an activities goal plus experienceWish; a park visit or sightseeing alone does not meet it. Interpret optional tripPreferences.experiences as taste, without assuming tastes from age. When invitees are still missing, do not claim their preferences or agreement are known. Use canonical keys when applicable. Keep month, year and stated duration as separate constraints so Luvia can offer exact date windows before planning. Put concrete wishes such as a named museum, beach day, child-friendly activity, restaurant style or shopping wish into mustDo/category constraints instead of generalizing them away. Other constraints retain descriptive keys. Never discard unsupported requirements or invent destinations, dates, holiday periods or place facts.'}
    };
  ...
  function experienceEvidence(place){
    ...
    const families=[['beach',['beach']],['harbour',['marina']],['park-garden',['park','garden','park_recreation_area']],['museum-gallery',['museum','art_gallery']],['monument',['monument','historical_monument','historical_landmark']],['market',['market']],['shopping-centre',['shopping_mall']],['individual-shop',['clothing_store','department_store','store']],['food',['restaurant','cafe','bakery','meal_takeaway']],['live-evening',['live_music_venue','concert_hall','performing_arts_theater','comedy_club']],['bar-club',['bar','pub','cocktail_bar','wine_bar','night_club','jazz_club','karaoke_bar']],['participatory',['sports_activity_location','bowling_alley','escape_room','amusement_center','minigolf','swimming_pool','amusement_park']],['animal-experience',['aquarium','zoo']]];
    const id=String(place.providerPlaceId||place.id||'').replace(/^places\//,''),research=place.tripWebResearch||{},researchKind=research.providerPlaceId===id&&/^https?:\/\//i.test(String(research.source?.url||''))&&research.description?`researched-${String(place.requestCategory||place.category||'experience')}`:'';
    const kinds=[...families.filter(([,values])=>types.some(type=>values.includes(type)||values.includes('restaurant')&&type.endsWith('_restaurant'))).map(([key])=>key),...(researchKind?[researchKind]:[])];
    const warnings=[];
    ...
    return {kinds:[...new Set(kinds.length?kinds:['unspecified'])],identityWarnings:warnings,providerNativeTypes:(place.providerNativeTypes||[]).slice(0,12),description:String(research.description||place.description||place.editorialSummary?.text||place.editorialSummary||'').slice(0,420),researchSource:researchKind?{url:String(research.source.url).slice(0,1200),retrievedAt:String(research.source.retrievedAt||'').slice(0,40),availability:'unverified'}:null,address:String(place.formattedAddress||place.address||'').slice(0,220)};
  }
  const EXPERIENCE_QUALITY={
    selection:'Only requested experiences belong in the itinerary. Balanced means a sensible daily rhythm, not adding parks, culture or monuments. Generic shopping means shopping centres; individual stores require a specific shopping wish. Use evidenceAlignment to detect missing wishes even when the prose sounds appealing.',
  ...
  async function composeTripItinerary(input = {}) {
    ...
    const repairSafeTravelPromise=order=>{
      const labels=[...new Set((order.categories||[]).map(item=>clean(item?.label||item?.category,120)).filter(Boolean))].slice(0,6),freeTimePercent=Number(order.rhythm?.freeTimePercent),commitments=labels.map(label=>`Der Plan deckt ${label} ab.`);
      commitments.push('Alle Reisetage bleiben vollständig und in einem ausgewogenen Rhythmus geplant.');
      if((order.mustDo||[]).length)commitments.push('Die ausdrücklich gesetzten Muss-Wünsche bleiben Prüfmaßstab für den unabhängigen Reise-Audit.');
      return {summary:labels.length?`Der reparierte Tagesplan verbindet ${labels.join(', ')} mit bewusstem Freiraum; Plan-B- und Buchungshinweise werden noch aktualisiert.`:'Der reparierte Tagesplan folgt weiterhin eurem bestätigten Reiseauftrag; Plan-B- und Buchungshinweise werden noch aktualisiert.',commitments:[...new Set(commitments)].slice(0,12),deliberateFreeTime:Number.isFinite(freeTimePercent)?`${Math.max(0,Math.min(80,freeTimePercent))}% des verfügbaren Tagesrahmens bleiben als sichtbarer Freiraum geplant.`:'Bewusste Pausen und spontane Zeit bleiben im Tagesplan sichtbar.',exclusions:[...new Set((order.exclusions||[]).map(item=>clean(item,180)).filter(Boolean))].slice(0,12)};
    };
    ...
    const selectionPolicy=experienceSelectionPolicy(input),planningDayCount=Math.max(1,Math.min(366,Array.isArray(input.days)?input.days.length:1)),planningCandidateLimit=4000,candidateInput=(Array.isArray(input.candidates)?input.candidates:[]).filter(item=>tripPlaceExperienceFit({place:item,policy:selectionPolicy}).eligible).slice(0,planningCandidateLimit).map(item=>{
    ...
    const order=input.brief?.travelOrder||{},planningPolicy=input.brief?.policy||{},rhythm=order.rhythm||{},logistics=order.logistics||{},pace=String(input.brief?.tripPreferences?.pace||input.tripPreferences?.pace||'balanced'),baseTarget=pace==='active'?4:pace==='slow'?2:3,maximumMomentsPerDay=Math.max(1,Math.min(4,Number(planningPolicy.maximumPerDay)||4)),baseStart=minute(planningPolicy.notBefore)??minute(rhythm.dayStart)??570,baseEnd=minute(planningPolicy.notAfter)??minute(rhythm.dayEnd)??1260,arrivalTime=minute(logistics.arrival?.localTime),departureTime=minute(logistics.departure?.localTime),mealPlan=rhythm.mealPlan&&typeof rhythm.mealPlan==='object'?rhythm.mealPlan:{mode:'flexible',slots:[]},mealSlots=(Array.isArray(mealPlan.slots)?mealPlan.slots:[]).map(slot=>({kind:['breakfast','lunch','dinner'].includes(slot?.kind)?slot.kind:'',windowStart:clean(slot?.windowStart,5),windowEnd:clean(slot?.windowEnd,5),venueMode:['place','open','accommodation'].includes(slot?.venueMode)?slot.venueMode:'place',importance:slot?.importance==='required'?'required':'preferred'})).filter(slot=>slot.kind&&minute(slot.windowStart)!=null&&minute(slot.windowEnd)>minute(slot.windowStart));
    const dayInput=rawDays.map((day,index)=>{
      ...
      if(role==='departure'){target=departureTime==null?1:Math.max(1,baseTarget-1);if(departureTime!=null)notAfter=Math.min(notAfter,departureTime-Math.max(60,Number(logistics.departure?.bufferMinutes)||120));}
      if(notAfter-notBefore<90)target=0;else target=Math.min(target,Math.max(1,Math.floor((notAfter-notBefore+60)/150)));
      const availableMealSlots=mealSlots.filter(slot=>minute(slot.windowEnd)>notBefore&&minute(slot.windowStart)<notAfter).map(slot=>({...slot,windowStart:clock(Math.max(notBefore,minute(slot.windowStart))),windowEnd:clock(Math.min(notAfter,minute(slot.windowEnd)))})).filter(slot=>minute(slot.windowEnd)>minute(slot.windowStart));
      return {...day,role,minimumMoments:target,targetMoments:target,...(role==='full'?{longStayAlternative:{minimumMoments:Math.max(1,target-1),minimumAnchorMinutes:180,minimumTotalMinutes:target*60,minimumFreeMinutes:90}}:{}),notBefore:clock(notBefore),notAfter:clock(Math.max(notBefore,notAfter)),mealSlots:availableMealSlots,freeTimePercent:Math.max(0,Math.min(70,Number(rhythm.freeTimePercent??planningPolicy.freeTimePercent)||0))};
    });
    ...
    let value,missingRepairDates=[],supportingDetailsChanged=false,repairedOriginalPlaceIds=[],repairedDayTokens=[];
    if(existing&&requestedRepairDates.length){
      ...
      for(const repairPolicy of repairPolicies){
        ...
        repairedDayTokens.push(repairPolicy.date,repairPolicy.label);for(const entry of replacedDay?.entries||[]){const id=clean(entry?.providerPlaceId,240).replace(/^places\//,'');if(id)repairedOriginalPlaceIds.push(id);}
    ...
    const days=dayInput.map((expected,index)=>{
      ...
      const entries=(Array.isArray(source.entries)?source.entries:[]).slice(0,maximumMomentsPerDay).map(entry=>{
        ...
        const mealSlot=candidate.category==='food'&&(expected.mealSlots||[]).length?(expected.mealSlots||[]).find(slot=>start!=null&&start>=minute(slot.windowStart)&&start<minute(slot.windowEnd)):null;
        if(candidate.category==='food'&&(expected.mealSlots||[]).length&&!mealSlot){contractIssues.push({code:'TRIP_ITINERARY_MEAL_WINDOW_VIOLATION',dayDate:expected.date,message:`${candidate.name} liegt nicht in eurem gewählten Essenszeitfenster am ${expected.label}.`});return null;}
        if(mealSlot&&candidate.facts?.features?.[mealSlot.kind]===false){contractIssues.push({code:'TRIP_ITINERARY_MEAL_SERVICE_CONTRADICTION',dayDate:expected.date,message:`${candidate.name} bietet laut Ortsdaten ${mealSlot.kind==='breakfast'?'Frühstück':mealSlot.kind==='lunch'?'Mittagessen':'Abendessen'} nicht an.`});return null;}
        ...
        return {providerPlaceId,time:expected.date?time:'',durationMinutes,category:candidate.category,mealSlot:mealSlot?.kind||null,reason:clean(entry?.reason,500),certainty:certainty==='verified'&&!evidenceRefs.length?'modelled':certainty,evidenceRefs,confidence:Math.max(0,Math.min(1,Number(entry?.confidence)||0))};
      }).filter(Boolean);
    ...
    if(!contractIssues.length&&(supportingDetailsChanged||(unusedCandidates.length&&!(value?.backupOptions||[]).some(validBackup)))){
      ...
      const supportingWarning='Der Tagesplan ist vollständig; Plan-B-Alternativen und abhängige Buchungshinweise werden bei der nächsten Prüfung ergänzt.',supportingRequirement='Plan-B-, Wege- und Buchungshinweise werden nach der Tagesreparatur erneut geprüft.',normalizeSupportingText=value=>clean(value).toLocaleLowerCase('de-DE').replace(/[^a-z0-9äöüß]+/g,' ').trim(),removedIds=[...new Set(repairedOriginalPlaceIds)].filter(id=>!used.has(id)),removedPlaceTokens=removedIds.flatMap(id=>[id,candidateInput.find(item=>item.providerPlaceId===id)?.name]).map(normalizeSupportingText).filter(token=>token.length>=4),repairedDayTextTokens=repairedDayTokens.map(normalizeSupportingText).filter(token=>token.length>=4),routeWords=/\b(weg|wege|route|fahrt|transfer|öffnung|öffnungszeit|buchung)\b/,retainedRequirements=(value?.uncoveredRequirements||[]).filter(item=>{const text=normalizeSupportingText(item);return text&&text!==normalizeSupportingText(supportingRequirement)&&(!supportingDetailsChanged||!removedPlaceTokens.some(token=>text.includes(token)))&&(!supportingDetailsChanged||!(routeWords.test(text)&&repairedDayTextTokens.some(token=>text.includes(token))));});
      try{
        ...
        value={...value,backupOptions:support.backupOptions||[],alternatives:support.alternatives||value.alternatives,uncertaintyMap:supportingDetailsChanged?(support.uncertaintyMap||[]):support.uncertaintyMap?.length?support.uncertaintyMap:value.uncertaintyMap,bookingOrder:support.bookingOrder||value.bookingOrder,neighborhoodRecommendation:support.neighborhoodRecommendation||value.neighborhoodRecommendation,uncoveredRequirements:[...new Set([...retainedRequirements,...(support.uncoveredRequirements||[])])],warnings:[...new Set([...(value?.warnings||[]).filter(item=>clean(item,240)!==supportingWarning),...(support.warnings||[])])],supportingDetailsErrorCode:null};
      }catch(error){
        if(supportingDetailsChanged)travelPromise=repairSafeTravelPromise(order);
        if(supportingDetailsChanged)retainedRequirements.push(supportingRequirement);
        value={...value,...(supportingDetailsChanged?{backupOptions:[],uncertaintyMap:[],bookingOrder:[],neighborhoodRecommendation:{},uncoveredRequirements:[...new Set(retainedRequirements)]}:{}),warnings:[...new Set([...(value?.warnings||[]),supportingWarning])],supportingDetailsErrorCode:clean(error?.code||'TRIP_SUPPORT_UNAVAILABLE',80)};
      }
    }
    ...
    return immutable({owner:'intelligence',contractId:'intelligence.v1',kind:'ai-trip-itinerary',source:'ai',title:clean(value?.title,160),summary:clean(value?.summary,800),travelPromise,days,dayPolicies:dayInput,uncertaintyMap,bookingOrder,neighborhoodRecommendation,evidenceCatalog,alternatives,backupOptions,contractIssues,uncoveredRequirements:(value?.uncoveredRequirements||[]).map(item=>clean(item,240)).filter(Boolean).slice(0,20),warnings:(value?.warnings||[]).map(item=>clean(item,240)).filter(Boolean).slice(0,20),supportingDetailsErrorCode:clean(value?.supportingDetailsErrorCode,80)||null,confidence:Math.max(0,Math.min(1,Number(value?.confidence)||0)),candidateCount:candidateInput.length,automaticMutation:false,confirmationRequired:true});
  }
  ...
  async function planTripPlaceSearch(input={}){
    ...
    return immutable({source:'ai',plans:(response.data?.searchPlans||[]).slice(0,6).map(plan=>{
      const includedTypes=[...new Set((plan.includedTypes||[]).filter(type=>allowed.has(type)))].slice(0,4);
      const category=definitions.slice().sort((left,right)=>right.types.filter(type=>includedTypes.includes(type)).length-left.types.filter(type=>includedTypes.includes(type)).length).find(item=>item.types.some(type=>includedTypes.includes(type)))?.category||'';
      return {query:String(plan.query||'').trim().slice(0,200),targetName:String(plan.targetName||'').trim().slice(0,200)||null,includedTypes,category:String(category),reason:String(plan.reason||'').slice(0,200)};
    }).filter(plan=>plan.query&&plan.category&&plan.includedTypes.length)});
  }
  ...
  async function researchTripExperiences(input={}){
    ...
    const response=await run('discovery.web-research',{surface:'trip-composer',destination:{name:input.destination?.name,countryCode:input.destination?.countryCode},needs:(input.needs||[]).slice(0,6).map(item=>({category:String(item.category||''),query:String(item.query||'').slice(0,200)}))},{tier:'fast',fallback:false,workflowId:input.workflowId,context:{surface:'trip-composer',purpose:'missing-experience-research'}});
  ...
  async function rankTripReserve(input={}){
    const selectionPolicy=experienceSelectionPolicy(input),candidates=(input.candidates||[]).filter(place=>tripPlaceExperienceFit({place,policy:selectionPolicy}).eligible).slice(0,16).map(place=>({providerPlaceId:String(place.providerPlaceId||place.id||'').replace(/^places\//,''),name:place.name,category:place.requestCategory||place.category,experienceEvidence:experienceEvidence(place),features:place.features||{},coordinates:place.coordinates||place.location}));
    ...
    const response=await run('discovery.rank',references.encode({referenceSet:references.fingerprint,surface:'trip-composer',domain:'trip',currentMoment:{surface:'trip-composer'},contract:{travelOrder:input.travelOrder||{},operation:input.mode,rejectionReason:String(input.rejectionReason||''),previousRejections:input.rejections||[],experienceQuality:EXPERIENCE_QUALITY,experienceSelectionPolicy:experienceSelectionPolicy(input),selectedMoment:input.current||null,day:input.day||null,instruction:'Rank only the supplied verified reserve. Address the exact rejection reason, not merely similarity. Omit unsuitable options; do not fill five slots. For no interest offer a different experience consistent with the travel order. Explain in one short German sentence (at most 16 words) what differs and fits. Preserve confirmed constraints. Do not claim opening hours, transport times, live prices or suitability without evidence.'},candidates}),{tier:'fast',fallback:false,context:{surface:'trip-composer',purpose:'trip-reserve-choice'}});
  ...
  async function auditTripItinerary(input = {}) {
    ...
    const evidenceAlignment=tripExperienceAlignment(input),categoryLabels={food:'Essen & Trinken',culture:'Kultur',sights:'Sehenswürdigkeiten',nature:'Natur',water:'Wasser & Strand',activities:'Aktivitäten',shopping:evidenceAlignment.policy.shoppingStyle==='centres'?'Einkaufszentren':'Shopping',nightlife:'Nachtleben',wellness:'Wellness',photo:'Fotospots',themeparks:'Freizeitparks'};
    // A model score cannot turn absent requested venues into fulfilled wishes.
    // Preserve legacy drafts without a semantic travel order; they can be reinterpreted explicitly.
    if(Array.isArray(input.brief?.travelOrder?.categories)&&evidenceAlignment.policy.explicitScope){
      for(const category of evidenceAlignment.missing)issues.push({code:'REQUESTED_EXPERIENCE_MISSING',severity:'blocked',dayDate:'',providerPlaceIds:[],category,message:`${categoryLabels[category]||category} ist im Plan noch nicht durch einen passenden Ort abgedeckt.`,suggestedRepair:evidenceAlignment.needsResearch.includes(category)?`Gezielt ${categoryLabels[category]||category} recherchieren; keine Ersatzkategorie einplanen.`:`Einen passenden belegten Ort für ${categoryLabels[category]||category} aus der Reserve einplanen.`});
      for(const item of evidenceAlignment.outside.slice(0,8))issues.push({code:'UNREQUESTED_EXPERIENCE',severity:'blocked',dayDate:item.dayDate,providerPlaceIds:[item.providerPlaceId],message:`${item.name} passt nach den Ortsbelegen nicht zu den gewählten Erlebnisbereichen.`,suggestedRepair:'Diesen Eintrag durch einen passenden Ort ersetzen; die anderen gültigen Tage erhalten.'});
      if(evidenceAlignment.missing.length||evidenceAlignment.outside.length){promiseAssessment.kept=false;promiseAssessment.summary='Die tatsächlichen Orte decken den Reiseauftrag noch nicht vollständig ab.';promiseAssessment.missedCommitments=[...new Set([...promiseAssessment.missedCommitments,...evidenceAlignment.missing.map(id=>categoryLabels[id]||id)])];}
    }
    ...
    return immutable({owner:'intelligence',contractId:'intelligence.v1',kind:'ai-trip-quality-audit',source:'ai',evidenceAlignment,readyForReview:blocked.length===0&&(promiseAssessment.kept||onlyDeferred),score:Math.max(0,Math.min(100,Math.round(Number(value?.score)||0))),headline:String(evidenceAlignment.missing.length||evidenceAlignment.outside.length?'Die Ortsauswahl erfüllt eure Erlebniswünsche noch nicht vollständig.':value?.headline||'Qualitätscheck des Reiseentwurfs').trim().slice(0,240),promiseAssessment,dimensions,issues,strengths:[...new Set((value?.strengths||[]).map(item=>String(item||'').trim()).filter(Boolean))].slice(0,12),repairInstructions:[...new Set((value?.repairInstructions||[]).map(item=>String(item||'').trim()).filter(Boolean))].slice(0,20),confidence:Math.max(0,Math.min(1,Number(value?.confidence)||0)),automaticMutation:false});
  }
  ...
  const api = Object.freeze({
    ...
    reads: Object.freeze({
      ...
      tripPlaceExperienceFit,
      tripExperienceAlignment,
      rankPlaceCandidates,
      composeDayGuidance,
