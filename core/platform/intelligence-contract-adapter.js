(() => {
  ...
  async function composeTripItinerary(input = {}) {
    ...
    if(!contractIssues.length&&unusedCandidates.length&&!(value?.backupOptions||[]).some(validBackup)){
      ...
      value={...value,backupOptions:support.backupOptions||[],alternatives:support.alternatives||value.alternatives,uncertaintyMap:support.uncertaintyMap?.length?support.uncertaintyMap:value.uncertaintyMap,bookingOrder:support.bookingOrder||value.bookingOrder,neighborhoodRecommendation:support.neighborhoodRecommendation||value.neighborhoodRecommendation};
    }
